...
* ``TENSOR_FIXED_SHAPE`` — all slots are zero-initialised at construction and
...
data is touched.  If you do not need this runtime type check, use one of the
typed wrappers (e.g. :ref:`float_array`) which hard-code the dtype and hide it
...
.. code-block:: c
   ...
   /* Create a 3x4 matrix of floats (TENSOR_FIXED_SHAPE). */
   size_t shape[] = { 3, 4 };
...
.. note::
   ...
   as :ref:`float_array` or :ref:`float_matrix`.  The generic API is intended
   for code that must operate on tensors of arbitrary or runtime-determined
   dtype, such as container libraries, serialisers, or generic algorithms.
...
* **HeapAllocator**
  ...
  * frequent resizing of ``TENSOR_DYNAMIC_1D`` tensors
...
* **ArenaAllocator**
  ...
  * intermediate tensors in algorithms
  ...
  construction.  ``TENSOR_DYNAMIC_1D`` tensors that grow frequently may leave
...
* **PoolAllocator / SlabAllocator**
  ...
  * many small tensors of identical rank and shape
  ...
  These allocators are not well suited for ``TENSOR_DYNAMIC_1D`` tensors that
...
The following operations are only valid for ``TENSOR_DYNAMIC_1D`` tensors.
Calling them on a ``TENSOR_FIXED_SHAPE`` tensor returns ``ILLEGAL_STATE``.
...
for ``TENSOR_FIXED_SHAPE`` and ``len`` for ``TENSOR_DYNAMIC_1D``.
N-dimensional index access is only valid for ``TENSOR_FIXED_SHAPE`` tensors;
calling either ``_nd`` variant on a ``TENSOR_DYNAMIC_1D`` tensor returns
``ILLEGAL_STATE``.
