...
static inline void simd_fill_int32(int32_t* data, size_t count, int32_t value) {
    ...
    while (i < count) {
        ...
        svint32_t vv = svdup_n_s32(value);
        svst1_s32(pg, data + i, vv);
        i += svcntw();
    }
}
...
static inline bool simd_int32_arrays_equal(const int32_t* a,
    ...
    while (i < count) {
        svbool_t pg = svwhilelt_b32(i, count);
        svint32_t av = svld1_s32(pg, a + i);
        svint32_t bv = svld1_s32(pg, b + i);
        svbool_t cmp = svcmpeq_s32(pg, av, bv);
...
static inline size_t simd_count_nonzero_int32(const int32_t* data,
    ...
    while (i < count) {
        ...
        svbool_t nz = svcmpne_n_s32(pg, v, 0);
