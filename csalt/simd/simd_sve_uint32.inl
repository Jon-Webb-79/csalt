...
static inline void simd_fill_uint32(uint32_t* data, size_t count, uint32_t value) {
    ...
    while (i < count) {
        ...
        svuint32_t vv = svdup_n_u32(value);
        svst1_u32(pg, data + i, vv);
        i += svcntw();
    }
}
...
static inline bool simd_uint32_arrays_equal(const uint32_t* a,
    ...
    while (i < count) {
        svbool_t pg = svwhilelt_b32(i, count);
        svuint32_t av = svld1_u32(pg, a + i);
        svuint32_t bv = svld1_u32(pg, b + i);
        svbool_t cmp = svcmpeq_u32(pg, av, bv);
...
static inline size_t simd_count_nonzero_uint32(const uint32_t* data,
    ...
    while (i < count) {
        svbool_t pg = svwhilelt_b32(i, count);
        svuint32_t v = svld1_u32(pg, data + i);
        svbool_t nz = svcmpne_n_u32(pg, v, 0u);

        nnz += (size_t)svcntp_b32(pg, nz);
        i += svcntw();
    }
