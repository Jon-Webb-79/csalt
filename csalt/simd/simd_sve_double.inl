/* simd_sve_double.inl
   SVE helpers for double operations (vector-length agnostic).
   Compile with -march=armv8.2-a+sve (or newer). Requires: <arm_sve.h>
*/
...
static inline double simd_sum_f64_sve(const double* x, size_t n) {
...
static inline double simd_min_f64_sve(const double* x, size_t n) {
...
static inline double simd_max_f64_sve(const double* x, size_t n) {
    size_t i = 0; double m = x[0];
    while (i < n) {
        svbool_t pg = svwhilelt_b64((uint64_t)i, (uint64_t)n);
        svfloat64_t vx = svld1_f64(pg, &x[i]);
...
static inline double simd_dot_f64_sve(const double* a, const double* b, size_t n) {
    size_t i = 0; double sum = 0.0;
    while (i < n) {
        svbool_t pg = svwhilelt_b64((uint64_t)i, (uint64_t)n);
...
static inline double simd_mean_f64_sve(const double* x, size_t n) {
    return n ? simd_sum_f64_sve(x, n) / (double)n : 0.0;
}
static inline double simd_stdev_f64_sve(const double* x, size_t n) {
    if (n < 2) return 0.0;
    const double mean = simd_mean_f64_sve(x, n);
