...
static inline void simd_fill_int16(int16_t* data, size_t count, int16_t value) {
    ...
    while (i < count) {
        ...
        svint16_t vv = svdup_n_s16(value);
        svst1_s16(pg, data + i, vv);
        i += svcnth();
    }
}
...
static inline bool simd_int16_arrays_equal(const int16_t* a,
    ...
    while (i < count) {
        svbool_t pg = svwhilelt_b16(i, count);
        svint16_t av = svld1_s16(pg, a + i);
        svint16_t bv = svld1_s16(pg, b + i);
        svbool_t cmp = svcmpeq_s16(pg, av, bv);
...
static inline size_t simd_count_nonzero_int16(const int16_t* data,
    ...
    while (i < count) {
        svbool_t pg = svwhilelt_b16(i, count);
        svint16_t v = svld1_s16(pg, data + i);
        svbool_t nz = svcmpne_n_s16(pg, v, 0);
