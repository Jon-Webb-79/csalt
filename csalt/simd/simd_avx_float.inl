...
static size_t simd_lsearch_float(const float* data,
    ...
    const size_t elems_per_reg = 8u;   /* 256 bits / 32 bits per float */
    ...
    if (len >= elems_per_reg) {
        ...
        for (; i + elems_per_reg <= len; i += elems_per_reg) {
            ...
            /* _CMP_LE_OQ: ordered less-than-or-equal, quiet (no exception
             * on NaN) — NaN inputs produce false, never a spurious match */
