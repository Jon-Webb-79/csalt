...
static inline void simd_fill_uint64(uint64_t* data, size_t count, uint64_t value) {
    ...
    size_t i = 0u;
    __m128i vv = _mm_set1_epi64x((long long)value);
...
// --------------------------------------------------------------------------------
/* SSE2 has no _mm_cmpeq_epi64, so emulate it with 32-bit compares. */
static inline bool simd_uint64_arrays_equal(const uint64_t* a,
    ...
    for (; i < vec_end; i += 2u) {
        ...
        __m128i eq32 = _mm_cmpeq_epi32(av, bv);
        __m128i hi = _mm_shuffle_epi32(eq32, _MM_SHUFFLE(3, 3, 1, 1));
        __m128i eq64 = _mm_and_si128(eq32, hi);

        if (_mm_movemask_epi8(eq64) != 0xFFFF) {
            return false;
        }
    }
...
static inline size_t simd_count_nonzero_uint64(const uint64_t* data,
    ...
    for (; i < vec_end; i += 2u) {
        ...
        __m128i eq32 = _mm_cmpeq_epi32(v, zero);
        __m128i hi = _mm_shuffle_epi32(eq32, _MM_SHUFFLE(3, 3, 1, 1));
        __m128i eq64 = _mm_and_si128(eq32, hi);

        unsigned mask = (unsigned)_mm_movemask_epi8(eq64);
