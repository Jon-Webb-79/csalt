...
static inline void simd_fill_uint64(uint64_t* data, size_t count, uint64_t value) {
    ...
    while (i < count) {
        ...
        svuint64_t vv = svdup_n_u64(value);
        svst1_u64(pg, data + i, vv);
        i += svcntd();
    }
}
...
static inline bool simd_uint64_arrays_equal(const uint64_t* a,
    ...
    while (i < count) {
        svbool_t pg = svwhilelt_b64(i, count);
        svuint64_t av = svld1_u64(pg, a + i);
        svuint64_t bv = svld1_u64(pg, b + i);
        svbool_t eq = svcmpeq_u64(pg, av, bv);
...
static inline size_t simd_count_nonzero_uint64(const uint64_t* data,
    ...
    while (i < count) {
        ...
        svbool_t nz = svcmpne_n_u64(pg, v, 0u);
