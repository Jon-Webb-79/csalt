...
static inline void simd_fill_uint32(uint32_t* data, size_t count, uint32_t value) {
    ...
    size_t i = 0u;
    __m256i vv = _mm256_set1_epi32((int)value);
...
static inline bool simd_uint32_arrays_equal(const uint32_t* a,
    ...
    for (; i < vec_end; i += 8u) {
        ...
        __m256i cmp = _mm256_cmpeq_epi32(av, bv);
...
static inline size_t simd_count_nonzero_uint32(const uint32_t* data,
    ...
    for (; i < vec_end; i += 8u) {
        ...
        __m256i eq = _mm256_cmpeq_epi32(v, zero);
        unsigned mask = (unsigned)_mm256_movemask_epi8(eq);

        for (unsigned lane = 0u; lane < 8u; ++lane) {
            unsigned lane_mask = (mask >> (lane * 4u)) & 0xFu;
            if (lane_mask != 0xFu) ++nnz;
        }
    }
