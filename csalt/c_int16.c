...
#  include "simd_avx512_int16.inl"
#elif defined(__AVX2__)
#  include "simd_avx2_int16.inl"
#elif defined(__AVX__)
#  include "simd_avx_int16.inl"
#elif defined(__SSE4_1__)
#  include "simd_sse41_int16.inl"
#elif defined(__SSE3__)
#  include "simd_sse3_int16.inl"
#elif defined(__SSE2__)
#  include "simd_sse2_int16.inl"
#elif defined(__ARM_FEATURE_SVE2)
#  include "simd_sve2_int16.inl"
#elif defined(__ARM_FEATURE_SVE)
#  include "simd_sve_int16.inl"
#elif defined(__ARM_NEON)
#  include "simd_neon_int16.inl"
#else
#  include "simd_scalar_int16.inl"
#endif
// ================================================================================ 
// ================================================================================ 
...
error_code_t print_int16_array(const int16_array_t* array, FILE* stream) {
    ...
    const int16_t* data = (const int16_t*)array->base.data;
    ...
    for (size_t i = 0u; i < len; ++i) {
        char buf[16];
        int n = snprintf(buf, sizeof(buf), "%" PRId16, data[i]);
...
bool int16_array_equal(const int16_array_t* a,
    ...
    return simd_int16_arrays_equal(
        (const int16_t*)a->base.data,
        (const int16_t*)b->base.data,
        a->base.len
    );
}
// ================================================================================ 
...
typedef struct {
    ...
    size_t col;
    bool   first;
} _int16_dict_print_ctx_t;

static void _print_int16_dict_entry(const char* key,
                                    size_t      key_len,
                                    int16_t     value,
                                    void*       user_data) {
    _int16_dict_print_ctx_t* ctx = (_int16_dict_print_ctx_t*)user_data;
    char val_buf[16];
    int val_n = snprintf(val_buf, sizeof(val_buf), "%" PRId16, value);
...
error_code_t print_int16_dict(const int16_dict_t* dict, FILE* stream) {
    ...
    error_code_t err = foreach_int16_dict(dict, _print_int16_dict_entry, &ctx);
...
// ================================================================================
// ================================================================================
// int16_matrix_t — type-safe int16 wrapper implementations
//
// Every function below is a thin delegation to the corresponding generic
// matrix_t function, fixing the dtype to INT16_TYPE and converting between
// int16 / int16* and the void* interface expected by c_matrix.h.
// ================================================================================
// ================================================================================

// ================================================================================
// Internal helper: rewrap matrix_expect_t as int16_matrix_expect_t
// ================================================================================

static inline int16_matrix_expect_t _wrap_matrix_expect(matrix_expect_t e) {
    if (e.has_value) {
        return (int16_matrix_expect_t){
            .has_value = true,
            .u.value   = (int16_matrix_t*)e.u.value
        };
    }
    return (int16_matrix_expect_t){
        .has_value = false,
...
int16_matrix_expect_t init_int16_dense_matrix(size_t             rows,
    ...
    return _wrap_matrix_expect(
        init_dense_matrix(rows, cols, INT16_TYPE, alloc_v)
    );
}
...
int16_matrix_expect_t init_int16_coo_matrix(size_t             rows,
                                            ...
                                            allocator_vtable_t alloc_v) {
    return _wrap_matrix_expect(
        init_coo_matrix(rows, cols, capacity, INT16_TYPE, growth, alloc_v)
    );
}
...
int16_matrix_expect_t convert_int16_matrix(const int16_matrix_t* src,
    ...
    /* SIMD fast path: dense int16 -> CSR */
    if (src->format == DENSE_MATRIX && target == CSR_MATRIX) {
        ...
        data = (const int16_t*)src->rep.dense.data;

        /* Pass 1: count nonzeros (SIMD-accelerated) */
        nnz = simd_count_nonzero_int16(data, total);
        ...
        if (nnz > (SIZE_MAX / sizeof(int16_t))) {
            return (int16_matrix_expect_t){
                .has_value = false,
                .u.error   = LENGTH_OVERFLOW
            };
        ...
        dst = (int16_matrix_t*)mr.u.value;
        dst->rows      = src->rows;
        dst->cols      = src->cols;
        dst->dtype     = INT16_TYPE;
        dst->data_size = sizeof(int16_t);
        ...
        values_bytes  = nnz * sizeof(int16_t);
        ...
        if (nnz > 0u) {
            ...
            for (size_t i = 0u; i < src->rows; ++i) {
                const int16_t* row_data = data + (i * src->cols);
                k = simd_scatter_csr_row_int16(
...
int16_matrix_expect_t transpose_int16_matrix(const int16_matrix_t* src,
    ...
    int16_matrix_expect_t r = init_int16_dense_matrix(
        src->cols, src->rows, alloc_v
    );
    if (!r.has_value) return r;
 
    /* SIMD fast path: dense int16 transpose */
    const int16_t* sdata = (const int16_t*)src->rep.dense.data;
    int16_t*       ddata = (int16_t*)r.u.value->rep.dense.data;
    simd_transpose_int16(sdata, ddata, src->rows, src->cols);
...
error_code_t fill_int16_matrix(int16_matrix_t* mat,
    ...
    /* SIMD fast path: dense int16 fill */
    int16_t* data  = (int16_t*)mat->rep.dense.data;
    size_t count = mat->rows * mat->cols;
    simd_fill_int16(data, count, value);
    return NO_ERROR;
}
// ================================================================================
...
bool int16_matrix_is_zero(const int16_matrix_t* mat) {
    ...
    /* SIMD fast path: dense int16 */
    if (mat->format == DENSE_MATRIX) {
        const int16_t* data = (const int16_t*)mat->rep.dense.data;
        size_t count = mat->rows * mat->cols;
        return simd_is_all_zero_int16(data, count);
    }
...
bool int16_matrix_equal(const int16_matrix_t* a,
    ...
    if (a->dtype != INT16_TYPE || b->dtype != INT16_TYPE) return false;

    /* Fast path: both dense */
    if (a->format == DENSE_MATRIX && b->format == DENSE_MATRIX) {
        const int16_t* da = (const int16_t*)a->rep.dense.data;
        const int16_t* db = (const int16_t*)b->rep.dense.data;
        return simd_int16_arrays_equal(da, db, a->rows * a->cols);
    }
    ...
    if (a->format == COO_MATRIX && b->format == COO_MATRIX &&
        ...
        if (nnz > 0u) {
            ...
            if (!simd_int16_arrays_equal(
                    (const int16_t*)a->rep.coo.values,
                    (const int16_t*)b->rep.coo.values,
                    nnz)) {
                return false;
            }
        }
...
int16_matrix_expect_t init_int16_identity_matrix(size_t             n,
                                                 allocator_vtable_t alloc_v) {
    return _wrap_matrix_expect(
        init_identity_matrix(n, INT16_TYPE, alloc_v)
    );
}
...
int16_matrix_expect_t init_int16_row_vector(size_t             length,
                                            allocator_vtable_t alloc_v) {
    return _wrap_matrix_expect(
        init_row_vector(length, INT16_TYPE, alloc_v)
    );
}
...
int16_matrix_expect_t init_int16_col_vector(size_t             length,
                                            allocator_vtable_t alloc_v) {
    return _wrap_matrix_expect(
        init_col_vector(length, INT16_TYPE, alloc_v)
    );
}
...
static inline bool _int16_value_is_zero(int16_t value, int16_zero_fn is_zero) {
    if (is_zero != NULL) {
...
// ================================================================================
// Semantic-zero conversion for int16 matrices
// ================================================================================
...
int16_matrix_expect_t convert_int16_matrix_zero(const int16_matrix_t* src,
     ...
     * For all other cases, delegate to the normal int16 conversion path.
     */
    if (src->format != DENSE_MATRIX ||
        (target != COO_MATRIX && target != CSR_MATRIX && target != CSC_MATRIX)) {
        return convert_int16_matrix(src, target, alloc_v);
    }
    ...
    const int16_t* data  = (const int16_t*)src->rep.dense.data;
    size_t total       = src->rows * src->cols;
    size_t nnz         = 0u;

    /* Pass 1: count semantic nonzeros */
    for (size_t i = 0u; i < total; ++i) {
        if (!_int16_value_is_zero(data[i], is_zero)) {
            ++nnz;
        }
    }
    ...
    if (target == COO_MATRIX) {
        int16_matrix_expect_t r = init_int16_coo_matrix(
        ...
        int16_matrix_t* dst = r.u.value;

        for (size_t row = 0u; row < src->rows; ++row) {
            for (size_t col = 0u; col < src->cols; ++col) {
                int16_t value = data[row * src->cols + col];
                if (!_int16_value_is_zero(value, is_zero)) {
                    error_code_t ec = push_back_int16_coo_matrix(dst, row, col, value);
                    if (ec != NO_ERROR) {
                        return_int16_matrix(dst);
                        return (int16_matrix_expect_t){
                            .has_value = false,
                            .u.error   = ec
                        };
         ...
         */
        (void)sort_int16_coo_matrix(dst);
        return r;
    }
...
static error_code_t _print_int16_dense_matrix(const int16_matrix_t* mat,
                                              FILE*                 stream) {
    ...
    for (size_t i = 0u; i < mat->rows; ++i) {
        ...
        for (size_t j = 0u; j < mat->cols; ++j) {
            ...
            if (fprintf(stream, "%" PRId16, data[idx]) < 0) {
                return ILLEGAL_STATE;
            }
...
static error_code_t _print_int16_sparse_row_major(const int16_matrix_t* mat,
                                                  FILE*                 stream) {
    if (mat == NULL || stream == NULL) return NULL_POINTER;
    ...
    for (size_t i = 0u; i < mat->rows; ++i) {
        for (size_t j = 0u; j < mat->cols; ++j) {
            int16_t value = 0;
            error_code_t err = get_int16_matrix(mat, i, j, &value);
            if (err != NO_ERROR) return err;
...
static error_code_t _print_int16_coo_matrix(const int16_matrix_t* mat,
                                            FILE*                 stream) {
    if (mat == NULL || stream == NULL) return NULL_POINTER;
    if (mat->format != COO_MATRIX)     return ILLEGAL_STATE;
    return _print_int16_sparse_row_major(mat, stream);
}

// -----------------------------------------------------------------------------

static error_code_t _print_int16_csr_matrix(const int16_matrix_t* mat,
                                            FILE*                 stream) {
    if (mat == NULL || stream == NULL) return NULL_POINTER;
...
static error_code_t _print_int16_csc_matrix(const int16_matrix_t* mat,
                                            FILE*                 stream) {
    if (mat == NULL || stream == NULL) return NULL_POINTER;
...
error_code_t print_int16_matrix(const int16_matrix_t* mat, FILE* stream) {
    ...
    switch (mat->format) {
        case DENSE_MATRIX:
            return _print_int16_dense_matrix(mat, stream);

        case COO_MATRIX:
            return _print_int16_coo_matrix(mat, stream);

        case CSR_MATRIX:
            return _print_int16_csr_matrix(mat, stream);

        case CSC_MATRIX:
            return _print_int16_csc_matrix(mat, stream);
...
int16_expect_t int16_matrix_min(const int16_matrix_t* mat) {
    ...
    size_expect_t idx = matrix_min(mat, _cmp_int16, INT16_TYPE);
    if (!idx.has_value) {
        return (int16_expect_t){ .has_value = false, .u.error = idx.u.error };
    }
    ...
    switch (mat->format) {
        case DENSE_MATRIX: {
            int16_t val = mat->rep.dense.data[idx.u.value];
            return (int16_expect_t){ .has_value = true, .u.value = val };
        }

        case COO_MATRIX: {
            int16_t val = mat->rep.coo.values[idx.u.value];
            return (int16_expect_t){ .has_value = true, .u.value = val };
        }

        case CSR_MATRIX: {
            int16_t val = mat->rep.csr.values[idx.u.value];
            return (int16_expect_t){ .has_value = true, .u.value = val };
        }

        case CSC_MATRIX: {
            int16_t val = mat->rep.csc.values[idx.u.value];
            return (int16_expect_t){ .has_value = true, .u.value = val };
        }
