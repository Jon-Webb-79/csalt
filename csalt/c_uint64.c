...
static void _add_scalar_uint64(void* element, const void* scalar) {
    uint64_t*       e = (uint64_t*)element;
    const uint64_t* s = (const uint64_t*)scalar;
    *e += *s;
}
...
uint64_expect_t uint64_matrix_min(const uint64_matrix_t* mat) {
    ...
    size_expect_t idx = matrix_min(mat, _cmp_uint64, UINT64_TYPE);
    if (!idx.has_value) {
        return (uint64_expect_t){ .has_value = false, .u.error = idx.u.error };
    }
...
uint64_expect_t uint64_matrix_max(const uint64_matrix_t* mat) {
    ...
    size_expect_t idx = matrix_max(mat, _cmp_uint64, UINT64_TYPE);
    if (!idx.has_value) {
        return (uint64_expect_t){ .has_value = false, .u.error = idx.u.error };
    }
