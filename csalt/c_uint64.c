...
error_code_t uint64_tensor_bsearch(const uint64_tensor_t* t,
    ...
    while (low <= high) {
        ...
        get_uint64_tensor_index(t, mid, &test);
...
bracket_expect_t uint64_tensor_bbsearch(const uint64_tensor_t* t,
    ...
    get_uint64_tensor_index(t, 0u,       &first);
    get_uint64_tensor_index(t, len - 1u, &last);
...
bool uint64_tensors_equal(const uint64_tensor_t* one,
    ...
    if (meta) {
        for (uint64_t d = 0u; d < one->base->ndim; d++) {
    ...
    return memcmp(one->base->data,
                  two->base->data,
                  sizeof(uint64_t) * one->base->len) == 0;
}
// -------------------------------------------------------------------------------- 
...
error_code_t min_uint64_tensor(const uint64_tensor_t* t, uint64_t* value) {
    ...
    *value = UINT64_MAX;
    return simd_min_uint64((const uint64_t*)t->base->data, t->base->len, value);
}
// ================================================================================
// ================================================================================
// eof
