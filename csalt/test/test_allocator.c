...
static void test_dyn_alignment_rounds_up_and_floors_to_maxalign(void **state) {
    ...
    /* Round up (24 -> 32) */
    arena_expect_t a = init_dynamic_arena(/*bytes*/4096, /*resize*/false,
                                          /*min_chunk_in*/4096, /*base_align_in*/24); 
    arena_t* a1 = a.u.value;
    assert_non_null(a1);
    void_ptr_expect_t expect = alloc_arena(a1, 1, false);
    assert_true(expect.has_value);
    void* p1 = expect.u.value;
    assert_non_null(p1);
    ...
    arena_expect_t b =init_dynamic_arena(/*bytes*/4096, /*resize*/false,
                                          /*min_chunk_in*/4096, /*base_align_in*/1);  
    assert_true(b.has_value);
    arena_t *a2 = b.u.value;
    assert_non_null(a2);
    void_ptr_expect_t expect2 = alloc_arena(a2, 1, false);
    assert_true(expect2.has_value);
    void* p2 = expect2.u.value;
    assert_non_null(p2);
...
static void test_static_alignment_honored_even_with_unaligned_buffer(void **state) {
    ...
    arena_expect_t expect = init_static_arena((void*)unaligned, BUF - 1, /*alignment_in*/64); 
    assert_true(expect.has_value);
    arena_t *a = expect.u.value;
    assert_non_null(a);

    /* First allocation must respect 64-byte alignment */
    void_ptr_expect_t aexpect = alloc_arena(a, 1, false);
...
static void test_init_darena(void **state) {
	...
	(void) state;
    arena_expect_t expect = init_darena(1000, true);
    assert_true(expect.has_value);
...
static void test_init_darena_no_bytes(void **state) {
	...
	(void) state;
    arena_expect_t expect = init_darena(0, true);
    assert_true(expect.has_value);
    arena_t* arena = expect.u.value;
    size_t size = arena_size(arena);
    size_t alloc = arena_alloc(arena);
...
static void test_init_darena_large_chunk(void **state) {
	...
	(void) state;
    arena_expect_t expect = init_darena(4097, true);
    assert_true(expect.has_value);
    arena_t* arena = expect.u.value;
    size_t size = arena_size(arena);
    size_t alloc = arena_alloc(arena);
...
static void test_init_sarena(void **state) {
    ...
    (void) state;
    uint8_t buffer[4097];
    arena_expect_t expect = init_sarena(buffer, 4097);
    assert_true(expect.has_value);
    arena_t* arena = expect.u.value;
    size_t size = arena_size(arena);
    size_t alloc = arena_alloc(arena);
...
static void test_invalid_free_sarena(void **state) {
    ...
    (void) state;
    uint8_t buffer[400];
    arena_expect_t expect = init_sarena(buffer, 400);
    assert_true(expect.has_value);
    arena_t* arena = expect.u.value;
    free_arena(arena);
    size_t size = arena_size(arena);
...
static void test_alloc_darena(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_arena(arena, sizeof(test), false); 
    assert_true(aexpect.has_value);
    test* struct_val = (test*)aexpect.u.value;
    
    aexpect = alloc_arena(arena, sizeof(int), false); 
    assert_true(aexpect.has_value);
    int* value = (int*)aexpect.u.value;
    struct_val->one = 3.4f;
    struct_val->two = 3;
    *value = 4;
...
static void test_alloc_darena_zeroed(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_arena(arena, sizeof(test), true); 
    assert_true(aexpect.has_value);
    test* struct_val = aexpect.u.value;
    ...
    aexpect = alloc_arena(arena, sizeof(test), false); 
    assert_true(aexpect.has_value);
    int* value = (int*)aexpect.u.value;
...
static void test_alloc_darena_null_value(void **state) {
    (void) state;
    void_ptr_expect_t value = alloc_arena(NULL, sizeof(test), true); 
...
static void test_alloc_darena_zero_input(void **state) {
    ...
    void_ptr_expect_t expect = alloc_arena(arena, 0, true); 
...
static void test_alloc_sarena(void **state) {
    ...
    void_ptr_expect_t expect = alloc_arena(arena, sizeof(test), false); 
    assert_true(expect.has_value);
    test* struct_val = (test*)expect.u.value;

    expect = alloc_arena(arena, sizeof(int), false); 
    assert_true(expect.has_value);
    int* value = (int*)expect.u.value;
    struct_val->one = 3.4f;
    struct_val->two = 3;
    *value = 4;
    assert_int_equal(*value, 4);
...
static void test_alloc_sarena_zeroed(void **state) {
    ...
    void_ptr_expect_t expect = alloc_arena(arena, sizeof(test), true); 
    assert_true(expect.has_value);
    test* struct_val = (test*)expect.u.value;
    assert_non_null(struct_val);
...
static void* alloc_fit(struct arena_t *a, size_t want, size_t *taken) {
    while (want > 0) {
        void_ptr_expect_t expect = alloc_arena(a, want, false);
        if (expect.has_value) {
            if (taken) *taken = want; return expect.u.value;
            --want;
        }
    }
    if (taken) *taken = 0;
...
static void test_realloc_grows_when_allowed(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_arena(a, 16, false);
    assert_true(aexpect.has_value);
...
static void test_realloc_first_alloc_in_new_chunk_is_aligned_and_no_pad(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_arena(a, 8, false);
    assert_true(aexpect.has_value);
    size_t rem = arena_remaining(a);
    assert_true(rem > 0);
    size_t taken = 0;
...
static void test_is_arena_ptr_basic_hits_and_misses(void **state) {
    ...
    size_t n = 128;
    void_ptr_expect_t aexpect = alloc_arena(a, n, true);
    assert_true(aexpect.has_value);
    uint8_t *p = (uint8_t*)aexpect.u.value;
    assert_non_null(p);
...
static void test_is_arena_ptr_tail_fastpath(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_arena(a, 32, false);
    assert_true(aexpect.has_value);
    
    aexpect = alloc_arena(a, 64, false);
    assert_true(aexpect.has_value);
...
static void test_is_arena_ptr_sized_boundaries(void **state) {
    ...
    size_t n = 256;
    void_ptr_expect_t aexpect = alloc_arena(a, n, false);
    assert_true(aexpect.has_value);
    uint8_t *p = (uint8_t*)aexpect.u.value;
    assert_non_null(p);
...
static void test_is_arena_ptr_multichunk(void **state) {
    ...
    aexpect = alloc_arena(a, 64, false);
    assert_true(aexpect.has_value);
    uint8_t *p2 = (uint8_t*)aexpect.u.value;
    assert_non_null(p2);
    ...
    aexpect = alloc_arena(a, 8, false);
    assert_true(aexpect.has_value);
    uint8_t *p3 = (uint8_t*)aexpect.u.value;
    assert_non_null(p3);
...
static void test_reset_dynamic_trim_true_frees_extra_chunks_and_resets_usage(void **state) {
    ...
    aexpect = alloc_arena(a, 64, false);
    assert_true(aexpect.has_value);
    uint8_t *tail_ptr = (uint8_t*)aexpect.u.value;
    ...
    aexpect = alloc_arena(a, 16, false);
    assert_true(aexpect.has_value);
...
static void test_reset_static_zeroes_usage_ignores_trim(void **state) {
    ...
    /* Consume some space */
    void_ptr_expect_t aexpect = alloc_arena(a, 128, true);
    assert_true(aexpect.has_value);
    ...
    aexpect = alloc_arena(a, 128, false);
    assert_true(aexpect.has_value);
...
static void test_save_restore_same_chunk_pointer_roundtrip(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_arena(a, 128, false);
    assert_true(aexpect.has_value);
...
static void test_save_restore_second_chunk_trims_and_replays_allocation(void **state) {
    ...
    aexpect = alloc_arena(a, 32, false);
    assert_true(aexpect.has_value);
    void *X = aexpect.u.value;
    assert_non_null(X);
    ...
    aexpect = alloc_arena(a, 48, false);
    assert_true(aexpect.has_value);
    void* Y1 = aexpect.u.value;
    assert_non_null(Y1);
...
static void test_save_restore_static_rewinds_cursor(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_arena(a, 128, false);
    assert_true(aexpect.has_value);
    void *A = aexpect.u.value;
    assert_non_null(A);

    ArenaCheckPoint cp = save_arena(a);

    aexpect = alloc_arena(a, 96, false);
    assert_true(aexpect.has_value);
    void *B1 = aexpect.u.value;
    assert_non_null(B1);
...
static void test_restore_rejects_checkpoint_from_other_arena(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_arena(a1, 32, false);
    assert_true(aexpect.has_value);
...
static void test_realloc_grow_copies_and_zeroes_tail(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_arena(a, old_sz, false);
    assert_true(aexpect.has_value);
    uint8_t* oldp = (uint8_t*)aexpect.u.value;
...
static void test_init_arena_with_arena_basic(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_arena(sub, 256, false);
    assert_true(aexpect.has_value);
    void* p = aexpect.u.value;
    assert_non_null(p);
...
static void test_init_arena_with_arena_multiple_subs(void **state) {
    ...
    // Allocate from each
    void_ptr_expect_t aexpect = alloc_arena(sub1, 128, false);
    assert_true(aexpect.has_value);
    void* p1 = aexpect.u.value;

    aexpect = alloc_arena(sub2, 256, false);
    assert_true(aexpect.u.value);
    void* p2 = aexpect.u.value;

    aexpect = alloc_arena(sub3, 512, false);
    assert_true(aexpect.has_value);
    void* p3 = aexpect.u.value;
...
static void test_init_arena_with_arena_nested_subs(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_arena(level3, 256, false);
    assert_true(aexpect.has_value);
    void* p = aexpect.u.value;
    assert_non_null(p);
...
static void test_sub_arena_cannot_grow(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_arena(sub, rem, false);
    assert_true(aexpect.has_value);
    void* p1 = aexpect.u.value;
    assert_non_null(p1);
    
    // Next allocation should fail (no growth)
    aexpect = alloc_arena(sub, 1, false);
    assert_false(aexpect.has_value);
    
    // Parent can still allocate (it has capacity)
    aexpect = alloc_arena(parent, 1024, false);
    assert_true(aexpect.has_value);
...
static void test_sub_arena_inherits_alignment(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_arena(sub, 1, false);
    assert_true(aexpect.has_value);
    void* p = aexpect.u.value;
...
static void test_free_sub_arena_nulls_state(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_arena(sub, 128, false);
    assert_true(aexpect.has_value);
...
static void test_sub_arena_stats(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_arena(sub, 1024, false);
    assert_true(aexpect.has_value);
    void* p1 = aexpect.u.value; 

    aexpect = alloc_arena(sub, 2048, false);
    assert_true(aexpect.has_value);
    void* p2 = aexpect.u.value;
...
static void test_sub_arena_checkpoint_restore(void **state) {
    ...
    // Allocate some memory
    void_ptr_expect_t aexpect = alloc_arena(sub, 512, false);
    assert_true(aexpect.has_value);
    void* p1 = aexpect.u.value;
    ...
    // Allocate more
    aexpect = alloc_arena(sub, 1024, false);
    assert_true(aexpect.has_value);
    void* p2 = aexpect.u.value;
...
static void test_sub_arena_independent_allocations(void **state) {
    ...
    // Allocate from sub1
    void_ptr_expect_t aexpect = alloc_arena(sub1, 1024, false);
    assert_true(aexpect.has_value);
    void* p1 = aexpect.u.value;
    ...
    // Allocate from sub2
    aexpect = alloc_arena(sub2, 2048, false);
    assert_true(aexpect.has_value);
    void* p2 = aexpect.u.value;
...
static void test_return_arena_with_buddy_roundtrip(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_arena(arena, 128u, false);
    assert_true(aexpect.has_value);
    void *p1 = aexpect.u.value;

    aexpect = alloc_arena(arena, 64u, false);
    assert_true(aexpect.has_value);
    void *p2 = aexpect.u.value;
...
static arena_t* make_dynamic_arena(size_t seed_bytes) {
    // resize=rue, min_chunk=64 KiB, base_align = max_align_t
    arena_expect_t expect = init_dynamic_arena(seed_bytes, /*resize=*/true, /*min_chunk_in=*/1u<<16, /*base_align_in=*/alignof(max_align_t)); 
    assert_true(expect.has_value);
...
static arena_t* make_tiny_dynamic_arena(size_t seed_bytes) {
    ...
    arena_expect_t expect = init_dynamic_arena(seed_bytes, /*resize=*/false, /*min_chunk_in=*/0, /*base_align_in=*/alignof(max_align_t)); 
    assert_true(expect.has_value);
    return expect.u.value;
}
...
static arena_t* make_exhausted_dynamic_arena(size_t seed_bytes, size_t leave_bytes) {
    ...
    arena_expect_t expect = init_dynamic_arena(seed_bytes, /*resize=*/false, /*min_chunk_in=*/0,
    ...
    size_t rem = arena_remaining(a);
    if (rem > leave_bytes) {
        void_ptr_expect_t aexpect = alloc_arena(a, rem - leave_bytes, /*zeroed=*/false); 
        assert_true(aexpect.has_value);
    }
    return a;
}
...
static void test_init_pool_invalid_args(void **state) {
    ...
    pool_expect_t expect = init_pool_with_arena(NULL, /*block_size=*/64, /*alignment=*/0, 
    ...
    expect = init_pool_with_arena(a, /*block_size=*/0, /*alignment=*/0, 
                                  /*blocks_per_chunk=*/16, /*prewarm=*/false,
                                  /*enable reallocations*/ true); 
    assert_false(expect.has_value);
    free_arena(a);
    expect = init_pool_with_arena(a, /*block_size=*/64, /*alignment=*/0, 
...
static void test_init_pool_header_lives_in_arena(void **state) {
    ...
    pool_expect_t expect = init_pool_with_arena(a, /*block_size=*/64, /*alignment=*/0, 
                                    /*blocks_per_chunk=*/32, /*prewarm=*/false,
                                    /*enable reallocations*/ true); 
    assert_true(expect.has_value);
    pool_t* p = expect.u.value;
...
static void test_init_pool_alignment_and_stride_rules(void **state) {
    ...
    pool_expect_t expect = init_pool_with_arena(a, /*block_size=*/1, /*alignment=*/0, 
    ...
    size_t req_align = 64;
    expect = init_pool_with_arena(a, /*block_size=*/48, /*alignment=*/req_align, 
                                     /*blocks_per_chunk=*/8, /*prewarm=*/false,
                                     /*enable reallocations*/ true); 
    assert_true(expect.has_value);
    pool_t* q = expect.u.value;
    assert_non_null(q);
...
static void test_init_pool_prewarm_sets_blocks(void **state) {
    ...
    pool_expect_t expect = init_pool_with_arena(a, /*block_size=*/32, /*alignment=*/0, 
    ...
    void_ptr_expect_t aexpect = alloc_pool(p, false);
    assert_true(aexpect.has_value);
    void* x = aexpect.u.value;

    aexpect = alloc_pool(p, false);
    assert_true(aexpect.has_value);
    void* y = aexpect.u.value;
...
static void test_pool_reset_semantics(void **state) {
    ...
    // Alloc should still wora after reset (pool will grow again)
    aexpect = alloc_pool(p, false);
    assert_true(aexpect.has_value);
    free_arena(a);
}
...
static void test_init_dynamic_pool_invalid_args(void **state) {
    ...
    pool_expect_t expect = init_dynamic_pool(0, 0, 32, 8192, 4096, true, true); 
    assert_false(expect.has_value);

    expect = init_dynamic_pool(64, 0, 0, 8192, 4096, true, true); 
    assert_false(expect.has_value);

    expect = init_dynamic_pool(64, 0, 32, 0, 4096, true, true); 
    assert_false(expect.has_value);
    //free_pool(p);
}
// -------------------------------------------------------------------------------- 
...
static void test_init_dynamic_pool_overflow_guard(void **state) {
    ...
    pool_expect_t expect = init_dynamic_pool(8, 0, too_many, 8192, 4096, true, false); 
    assert_false(expect.has_value);
}
// -------------------------------------------------------------------------------- 
...
static void test_init_dynamic_pool_alignment_and_stride(void **state) {
    ...
    pool_expect_t expect = init_dynamic_pool(48, 64, 8, 8192, 4096, true, false); 
    assert_true(expect.has_value);
    pool_t *p = expect.u.value;
...
static void test_init_dynamic_pool_fixed_prewarm_exact(void **state) {
    ...
    pool_expect_t expect = init_dynamic_pool(
        /*block_size=*/64,
        /*alignment=*/0,
    ...
    void_ptr_expect_t aexpect;
    void* saved[16] = {0};
    for (size_t i = 0; i < blocks; ++i) {
        aexpect = alloc_pool(p, false);
        assert_true(aexpect.has_value);
        saved[i] = aexpect.u.value;
...
static void test_init_dynamic_pool_fixed_no_prewarm_first_alloc_fails(void **state) {
    ...
    pool_expect_t expect = init_dynamic_pool(64, 0, 8, 4096, 0, false, false); 
    assert_true(expect.has_value);
    pool_t *p = expect.u.value;
    assert_non_null(p);
...
static void test_init_dynamic_pool_grow_prewarm_and_reuse(void **state) {
    ...
    pool_expect_t expect = init_dynamic_pool(32, 16, 8, 8192, 4096, true, true);
    assert_true(expect.has_value);
    pool_t *p = expect.u.value;
...
static void test_init_dynamic_pool_grow_lazy_first_alloc(void **state) {
    ...
    pool_expect_t expect = init_dynamic_pool(64, 0, 4, 4096, 2048, true, false); 
    assert_true(expect.has_value);
    pool_t *p = expect.u.value;
    ...
    void_ptr_expect_t aexpect = alloc_pool(p, false);
    assert_true(aexpect.has_value);
    assert_non_null(aexpect.u.value);
...
static void test_init_dynamic_pool_tiny_seed_fails(void **state) {
    ...
    pool_expect_t expect = init_dynamic_pool(64, 0, 8, 32, 0, true, false); 
    assert_false(expect.has_value);
}
// // ================================================================================ 
// // ================================================================================ 
...
static void test_init_static_pool_invalid_args(void **state) {
    ...
    // NULL buffer
    pool_expect_t expect = init_static_pool(NULL, 1024, 64, 0); 
    assert_false(expect.has_value);

    // zero buffer_bytes
    static unsigned char buf1[128];
    expect = init_static_pool(buf1, 0, 64, 0); 
    assert_false(expect.has_value);
    expect = init_static_pool(buf1, 0, 64, 0);
    assert_false(expect.has_value);

    // zero block_size
    expect = init_static_pool(buf1, sizeof buf1, 0, 0); 
    assert_false(expect.has_value);
}
// -------------------------------------------------------------------------------- 
...
static void test_init_static_pool_alignment_and_stride(void **state) {
    ...
    pool_expect_t expect = init_static_pool(buf, sizeof buf, /*block_size=*/48, /*alignment=*/64); 
    assert_true(expect.has_value);
    pool_t* p = expect.u.value;
    assert_non_null(p);
...
static void test_init_static_pool_stride_minimum(void **state) {
    ...
    pool_expect_t expect = init_static_pool(buf, sizeof buf, /*block_size=*/1, /*alignment=*/0); 
    assert_true(expect.has_value);
    pool_t* p = expect.u.value;
...
static void test_init_static_pool_capacity_and_exhaustion(void **state) {
    ...
    pool_expect_t expect = init_static_pool(buf, sizeof buf, /*block_size=*/64, /*alignment=*/0); 
    assert_true(expect.has_value);
    pool_t* p = expect.u.value;
    assert_non_null(p);
...
static void test_init_static_pool_free_list_reuse(void **state) {
    ...
    pool_expect_t expect = init_static_pool(buf, sizeof buf, /*block_size=*/32, /*alignment=*/16); 
    assert_true(expect.has_value);
    pool_t* p = expect.u.value;
...
static void test_init_static_pool_one_block_only(void **state) {
    ...
    pool_expect_t expect = init_static_pool(buf, sizeof buf, /*block_size=*/4096, /*alignment=*/64); 
    assert_true(expect.has_value);
    pool_t* p = expect.u.value;
...
static void test_pool_alignment_matches_contract(void **state) {
    ...
    pool_expect_t pexpect = init_dynamic_pool(
        /*block_size=*/48,
        /*alignment=*/64,
        /*blocks_per_chunk=*/8,
        /*arena_seed_bytes=*/8192,
    ...
    assert_true(pexpect.has_value);
    pool_t* p = pexpect.u.value;
...
static void test_pool_bump_remaining_single_chunk(void **state) {
    ...
    pool_expect_t pexpect = init_static_pool(buf, sizeof buf, /*block_size=*/64, /*alignment=*/64); 
    assert_true(pexpect.has_value);
    pool_t *p = pexpect.u.value;
    assert_non_null(p);
    ...
    void_ptr_expect_t expect = alloc_pool(p, false);
    assert_true(expect.has_value);
    void *b0 = expect.u.value;

    expect = alloc_pool(p, false);
    assert_true(expect.has_value);
    void *b1 = expect.u.value;
...
static void test_pool_in_use_blocks_counts(void **state) {
    ...
    static unsigned char buf[16 * 1024] __attribute__((aligned(64)));
    pool_expect_t pexpect = init_static_pool(buf, sizeof buf, /*block_size=*/64, /*alignment=*/0); 
    assert_true(pexpect.has_value);
    pool_t *p = pexpect.u.value;
...
static void test_pool_owns_arena_and_mtype_static_vs_dynamic(void **state) {
    ...
    pool_expect_t pexpect = init_static_pool(sbuf, sizeof sbuf, /*block_size=*/64, /*alignment=*/0); 
    assert_true(pexpect.has_value);
    pool_t *ps = pexpect.u.value;
    assert_true(pool_owns_arena(ps));
    assert_int_equal(pool_mtype(ps), STATIC);
    free_pool(ps);

    // Dynamic 
    pexpect = init_dynamic_pool(
...
static void test_pool_save_restore_basic(void **state) {
    ...
    pool_expect_t pexpect = init_dynamic_pool(32, 4, 64, 4096, 4096, true, true); 
    assert_true(pexpect.has_value);
    pool_t* p = pexpect.u.value;
...
static void test_pool_restore_reuses_address(void **state) {
    ...
    pool_expect_t pexpect = init_dynamic_pool(64, 8, 128, 4096, 4096, true, true);
    assert_true(pexpect.has_value);
    pool_t* p = pexpect.u.value;
...
static void test_init_freelist_with_arena_null_arena(void **state) {
    ...
    freelist_expect_t fl = init_freelist_with_arena(NULL, 128u, 0u);
...
static void test_init_freelist_with_arena_basic(void **state) {
    ...
    arena_expect_t expect = init_static_arena(buffer, sizeof buffer,
                                              alignof(max_align_t));
    assert_true(expect.has_value);
    arena_t *arena = expect.u.value;

    freelist_expect_t fexpect = init_freelist_with_arena(arena, 512u, 0u); 
    assert_true(fexpect.has_value);
    freelist_t *fl = fexpect.u.value;
    ...
    void_ptr_expect_t vexpect = alloc_freelist(fl, request, false);
...
static void test_init_freelist_with_arena_alignment_normalization(void **state) {
    ...
    arena_expect_t aexpect = init_static_arena(buffer, sizeof buffer,
                                       alignof(max_align_t)); 
    assert_true(aexpect.has_value);
    arena_t *arena = aexpect.u.value;

    /* Intentionally non-power-of-two alignment request */
    size_t requested_alignment = 24u;

    freelist_expect_t fexpect = init_freelist_with_arena(arena, 256u, requested_alignment); 
    assert_true(fexpect.has_value);
    freelist_t *fl = fexpect.u.value;
    ...
    void_ptr_expect_t vexpect =  alloc_freelist(fl, 64u, false);
    assert_true(vexpect.has_value);
    void *p = vexpect.u.value;
    assert_non_null(p);
    assert_int_equal((uintptr_t)p % fl_align, 0u);
...
static void test_init_freelist_with_arena_insufficient_arena_space(void **state) {
    ...
    freelist_expect_t fexpect = init_freelist_with_arena(arena, avail + 1u, 0u); 
    assert_false(fexpect.has_value);
}
// -------------------------------------------------------------------------------- 
...
static void test_init_dynamic_freelist_basic(void **state) {
    ...
    freelist_expect_t fexpect = init_dynamic_freelist(512u, 0u, false); 
    assert_true(fexpect.has_value);
    freelist_t *fl = fexpect.u.value;
...
static void test_init_dynamic_freelist_alignment_normalization(void **state) {
    ...
    freelist_expect_t fexpect = init_dynamic_freelist(256u, requested_alignment, false); 
    assert_true(fexpect.has_value);
    freelist_t *fl = fexpect.u.value;
...
static void test_init_static_freelist_zero_bytes_invalid(void **state) {
    ...
    freelist_expect_t expect = init_static_freelist(buffer, 0u, 0u); 
    assert_false(expect.has_value);
}
// -------------------------------------------------------------------------------- 
...
static void test_init_static_freelist_too_small_buffer(void **state) {
    ...
    freelist_expect_t expect = init_static_freelist(buffer, sizeof buffer, 0u); 
    assert_false(expect.has_value);
}
// -------------------------------------------------------------------------------- 
...
static void test_init_static_freelist_alignment_normalization(void **state) {
    ...
    size_t requested_alignment = 24u;  /* intentionally non–power-of-two */
    freelist_expect_t fexpect = init_static_freelist(buffer, sizeof buffer,
                                                     requested_alignment); 
    assert_true(fexpect.has_value);
    freelist_t *fl = fexpect.u.value;
    ...
    void_ptr_expect_t vexpect = alloc_freelist(fl, request, false);
    assert_true(vexpect.has_value);
    void *p = fexpect.u.value;
    assert_false(is_freelist_ptr_sized(fl, p, request));
    assert_int_equal((uintptr_t)p % fl_align, 0u);
...
static void test_alloc_freelist_aligned_basic(void **state) {
    ...
    void_ptr_expect_t vexpect = alloc_freelist_aligned(fl, req_bytes, req_align, false); 
    assert_true(vexpect.has_value);
    void *p = vexpect.u.value;
...
static void test_alloc_freelist_aligned_zero_alignment_uses_base(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_freelist_aligned(fl, req_bytes, 0u, false); 
    assert_true(aexpect.u.value);
    void *p = aexpect.u.value;
...
static void test_alloc_freelist_aligned_non_power_of_two_align(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_freelist_aligned(fl, req_bytes, requested_alignment, false); 
    assert_true(aexpect.has_value);
    void *p = aexpect.u.value;
...
static void test_alloc_freelist_aligned_zeroed(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_freelist_aligned(fl, req_bytes, req_align, true); 
    assert_true(aexpect.has_value);
    void *p = aexpect.u.value;
...
static void test_alloc_freelist_aligned_null_freelist(void **state) {
    ...
    void_ptr_expect_t expect = alloc_freelist_aligned(NULL, 16u, 16u, false); 
    assert_false(expect.has_value);
}
// -------------------------------------------------------------------------------- 
...
static void test_alloc_freelist_aligned_zero_bytes(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_freelist_aligned(fl, 0u, 16u, false); 
    assert_false(aexpect.has_value);
}
// -------------------------------------------------------------------------------- 
...
static void test_realloc_freelist_null_freelist(void **state) {
    ...
    void_ptr_expect_t expect = realloc_freelist(NULL, NULL, 0u, 16u, false); 
    assert_false(expect.has_value);
}
// -------------------------------------------------------------------------------- 
...
static void test_realloc_freelist_null_ptr_allocates_and_zeroes(void **state) {
    ...
    void_ptr_expect_t aexpect = realloc_freelist(fl, NULL, 0u, new_size, true); 
    assert_true(aexpect.has_value);
    void *p = aexpect.u.value;
...
static void test_realloc_freelist_shrink_keeps_pointer(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_freelist(fl, old_size, false); 
    assert_true(aexpect.has_value);
    void *p = aexpect.u.value;
    ...
    aexpect = realloc_freelist(fl, p, old_size, new_size, true); 
    assert_true(aexpect.has_value);
    void *p2 = aexpect.u.value;
...
static void test_realloc_freelist_aligned_null_freelist(void **state) {
    (void)state;
    void_ptr_expect_t expect = realloc_freelist_aligned(NULL, NULL, 0u, 32u, false, 32u); 
    assert_false(expect.has_value);
}
// -------------------------------------------------------------------------------- 
...
static void test_realloc_freelist_aligned_null_ptr_allocates(void **state) {
    ...
    void_ptr_expect_t aexpect = realloc_freelist_aligned(fl, NULL, 0u, new_size, true, req_align); 
    assert_true(aexpect.has_value);
    void *p = aexpect.u.value;
    assert_true(is_freelist_ptr_sized(fl, p, new_size));
...
static void test_realloc_freelist_aligned_shrink_keeps_pointer(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_freelist_aligned(fl, old_size, req_align, false); 
    assert_true(aexpect.has_value);
    void *p = aexpect.u.value;
    ...
    aexpect = realloc_freelist_aligned(fl, p, old_size, new_size, true, req_align); 
    assert_true(aexpect.has_value);
    void *p2 = aexpect.u.value;
...
static void test_reset_freelist_basic_static(void **state) {
    ...
    /* Allocate one block */
    const size_t request = 128u;
    void_ptr_expect_t aexpect = alloc_freelist(fl, request, false); 
    assert_true(aexpect.has_value);
    void *p = aexpect.u.value;
    assert_non_null(p);
    assert_true(is_freelist_ptr_sized(fl, p, request));
    ...
    aexpect = alloc_freelist(fl, request, true); 
    assert_true(aexpect.has_value);
    void *p2 = aexpect.u.value;
...
static void test_reset_freelist_after_fragmentation(void **state) {
    ...
    void_ptr_expect_t aexpect = alloc_freelist(fl, a_size, false); 
    assert_true(aexpect.has_value);
    void *a = aexpect.u.value;

    aexpect = alloc_freelist(fl, b_size, false); 
    assert_true(aexpect.has_value);
    void *b = aexpect.u.value;

    aexpect = alloc_freelist(fl, c_size, false);
    assert_true(aexpect.has_value);
    void *c = aexpect.u.value;
    ...
    aexpect = alloc_freelist(fl, big_request, false); 
    assert_true(aexpect.has_value);
    void *big = aexpect.u.value;
    assert_non_null(big);
