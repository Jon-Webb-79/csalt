 ...
 *
 * Scans all elements using a signed 8-bit comparison and returns the smallest
 ...
 * int8_array_contains. The array is not modified. The scan dispatches to the
 ...
 *
 * @return int8_expect_t with has_value true and u.value == the minimum
 ...
 *     // arr contains [10, -20, 5].
 *
 *     int8_expect_t r = int8_array_min(arr);
 *     // r.has_value == true, r.u.value == -20.
 *
 *     return_int8_array(arr);
 * @endcode
 */
 ...
 *
 * Scans all elements using a signed 8-bit comparison and returns the largest
 ...
 *               contain at least one element.
 *
 * @return int8_expect_t with has_value true and u.value == the maximum
 *         element on success. On failure, has_value is false and u.error
 *         is one of:
 *         - NULL_POINTER  if array is NULL
 *         - EMPTY         if array->base.len == 0
 *
 * @code
 *     int8_expect_t r = int8_array_max(arr);
...
/**
 * @brief Compare two int8 arrays for exact equality using a SIMD-accelerated path.
 *
 ...
 *
 * @param a  Pointer to the first int8 array.
 * @param b  Pointer to the second int8 array.
 *
...
// ================================================================================
// Type alias and expected return type
// ================================================================================
 
/**
 * @brief A dictionary that maps null-terminated C-string keys to int8_t values.
 *
 * This is a typedef alias for @ref dict_t with the convention that
 * @c data_size is always @c sizeof(int8_t) and @c dtype is always
 * @c INT8_TYPE.  Do not use the raw @ref dict_t API directly on an
 * @c int8_dict_t — always go through the functions declared in this header
 ...
 * @endcode
 */
typedef dict_t int8_dict_t;
 
/**
 * @brief Expected return type for @ref init_int8_dict and
 *        @ref copy_int8_dict.
 *
...
/**
 * @brief Typed iterator callback for @ref foreach_int8_dict.
 *
...
/**
 * @brief Allocate and initialise a new int8_dict_t.
 *
 * The underlying @ref dict_t is created with @c data_size = sizeof(int8_t)
 * and @c dtype = INT8_TYPE.  The bucket count is rounded up to the next
 ...
 *                  must not be NULL.
 *
 * @return @ref int8_dict_expect_t with @c has_value true on success.
 ...
 *     allocator_vtable_t a = heap_allocator();
 *
 *     int8_dict_expect_t r = init_int8_dict(8, true, a);
 *     if (!r.has_value) {
 *         fprintf(stderr, "init failed: %d\n", r.u.error);
 *         return;
 *     }
 *     int8_dict_t* d = r.u.value;
 *
 *     insert_int8_dict(d, "delta", -5, a);
 *     insert_int8_dict(d, "gamma", 12, a);
 *
 *     return_int8_dict(d);
 * @endcode
 */
...
/**
 * @brief Release all memory owned by an int8_dict_t.
 *
 ...
 *
 * @code
 *     int8_dict_t* d = init_int8_dict(8, true, a).u.value;
 *     insert_int8_dict(d, "x", -42, a);
 *     // ... use the dict ...
 *     return_int8_dict(d);   // d must not be used after this point
 * @endcode
 */
...
/**
 * @brief Insert a null-terminated string key with an int8_t value.
 *
 ...
 * returned.  To overwrite an existing value use @ref update_int8_dict.
 *
 ...
 *     int8_dict_t* d = init_int8_dict(8, true, a).u.value;
 *
 *     insert_int8_dict(d, "temp",  -20, a);   // OK
 *     insert_int8_dict(d, "humid",  60, a);   // OK
 *
 *     // Duplicate key — returns INVALID_ARG, dict is unchanged
 *     error_code_t err = insert_int8_dict(d, "temp", 0, a);
...
/**
 * @brief Insert a bounded key (explicit length) with an int8_t value.
 *
 * Identical to @ref insert_int8_dict but the key length is supplied by the
 ...
 *
 * @code
 *     // Key is a sub-string: "delta_fine", take only "delta" (5 bytes)
 ...
 * @code
 *     int8_dict_t* d = init_int8_dict(8, true, a).u.value;
 *     insert_int8_dict(d, "gain", -6, a);
 *
 *     int8_t removed;
 *     error_code_t err = pop_int8_dict(d, "gain", &removed);
 *     assert(err == NO_ERROR && removed == -6);
 *
 *     // Key is gone — next pop returns NOT_FOUND
 *     err = pop_int8_dict(d, "gain", NULL);
 ...
 *
 * Identical to @ref pop_int8_dict but the key length is supplied explicitly.
 *
 * @param dict       Must not be NULL.
 ...
 *     insert_int8_dict_n(d, buf, 4, -6, a);   // key = "gain"
 *
 *     int8_t v;
 *     pop_int8_dict_n(d, buf, 4, &v);          // removes "gain", v == -6
 * @endcode
 */
 ...
 * @ref insert_int8_dict.
 *
 * @param dict   Must not be NULL.
 * @param key    Null-terminated C-string key.  Must not be NULL.
 * @param value  New int8_t value to store.
 *
 * @return NO_ERROR, NULL_POINTER, or NOT_FOUND.
 *
 * @code
 ...
 *     insert_int8_dict(d, "offset", 10, a);
 *
 *     update_int8_dict(d, "offset", -10);   // flip sign
 *
 *     int8_t v;
 *     get_int8_dict_value(d, "offset", &v);
 *     assert(v == -10);
 *
 *     return_int8_dict(d);
 * @endcode
 ...
 *
 * Identical to @ref update_int8_dict but the key length is supplied
 * explicitly.
 *
 * @param dict     Must not be NULL.
 * @param key      Pointer to the key bytes.  Must not be NULL.
 ...
 *     update_int8_dict_n(d, buf, 6, -10);      // update "offset" to -10
 * @endcode
 */
 ...
 * @code
 *     int8_dict_t* d = init_int8_dict(8, true, a).u.value;
 *     insert_int8_dict(d, "bias", -64, a);
 *
 *     int8_t v;
 *     if (get_int8_dict_value(d, "bias", &v) == NO_ERROR)
 *         printf("bias = %d\n", (int)v);
 *
 *     return_int8_dict(d);
 * @endcode
 ...
 *
 * Identical to @ref get_int8_dict_value but the key length is supplied
 * explicitly.
 *
 * @param dict       Must not be NULL.
 * @param key        Pointer to the key bytes.  Must not be NULL.
 ...
 *
 *     int8_t v;
 *     get_int8_dict_value_n(d, buf, 4, &v);     // v == -64
 * @endcode
 */
 ...
 * @param key   Null-terminated C-string key.  Must not be NULL.
 *
 * @return Pointer to the stored int8_t on success, NULL if not found or on
 *         error.
 *
 * @code
 ...
 *        a bounded key.
 *
 * Identical to @ref get_int8_dict_ptr but the key length is supplied
 * explicitly.
 *
 * @param dict     Must not be NULL.
 * @param key      Pointer to the key bytes.  Must not be NULL.
 ...
 *
 * @code
 *     insert_int8_dict(d, "enabled", 1, a);
 *
 *     if (has_int8_dict_key(d, "enabled"))
 *         printf("enabled is set\n");
 *
 *     if (!has_int8_dict_key(d, "disabled"))
 *         printf("disabled is not set\n");
 * @endcode
 */
 ...
 *
 * Identical to @ref has_int8_dict_key but the key length is supplied
 * explicitly.
 *
 * @param dict     Must not be NULL.
 * @param key      Pointer to the key bytes.  Must not be NULL.
 ...
 *
 * @code
 *     const char* buf = "enabled_flag";
 *     insert_int8_dict_n(d, buf, 7, 1, a);   // key = "enabled"
 *     assert(has_int8_dict_key_n(d, buf, 7));
 * @endcode
 */
 ...
 *     assert(int8_dict_hash_size(d) == 2);
 *
 *     clear_int8_dict(d);
 *     assert(int8_dict_hash_size(d) == 0);
 *
 *     // The dict is still usable after clear
 *     insert_int8_dict(d, "c", 3, a);
 * @endcode
 */
...
/**
 * @brief Allocate a deep copy of an int8_dict_t.
 *
...
/**
 * @brief Merge two int8_dict_t instances into a new dict.
 *
 ...
 *
 * @code
 *     int8_dict_t* da = init_int8_dict(8, true, a).u.value;
 *     int8_dict_t* db = init_int8_dict(8, true, a).u.value;
 *
 *     insert_int8_dict(da, "shared", -10, a);
 *     insert_int8_dict(da, "only_a",   1, a);
 *     insert_int8_dict(db, "shared",  99, a);
 *     insert_int8_dict(db, "only_b",   2, a);
 *
 *     // Merge without overwrite — "shared" keeps da's value of -10
 *     int8_dict_expect_t mr = merge_int8_dict(da, db, false, a);
 *     int8_t v;
 *     get_int8_dict_value(mr.u.value, "shared", &v);
 *     assert(v == -10);
 *
 ...
 * C-string pointer (into internal storage), its length, and the int8_t
 ...
 *
 *     int8_dict_t* d = init_int8_dict(8, true, a).u.value;
 *     insert_int8_dict(d, "x", -128, a);
 *     insert_int8_dict(d, "y",    0, a);
 *     insert_int8_dict(d, "z",  127, a);
 *
 *     foreach_int8_dict(d, print_entry, NULL);
...
// ================================================================================ 
// ================================================================================ 
// int8_matrix_t — type-safe int8 wrapper around matrix_t
//
// This section provides a int8-specialised interface to the generic matrix_t
// type.  int8_matrix_t is a typedef for matrix_t; all functions below fix
// the dtype to int8_TYPE and accept / return int8 values directly so that
// callers never need to pass a dtype_id_t or void* value.
//
// The wrapper covers every public function in c_matrix.h that either:
//   (a) takes a dtype_id_t argument (replaced with int8_TYPE), or
//   (b) takes or returns void* element pointers (replaced with int8 / int8*).
//
...
// int8_matrix_t* signature for consistency, but simply delegate.
// ================================================================================ 
// ================================================================================ 

typedef matrix_t int8_matrix_t;
...
/**
 * @brief Predicate function to determine semantic zero for int8 values.
 *
 ...
 *
 * @param value The int8 value to evaluate.
 *
 ...
 * @code{.c}
 * bool zero_or_neg_one(int8 v) {
 *     return (v == 0) || (v == 3);
 * }
 * @endcode
 */
typedef bool (*int8_zero_fn)(int8_t value);
...
/**
 * @brief Initialize a dense int8 matrix with zero-initialized storage.
 *
 * Creates a dense matrix of shape @p rows by @p cols with the dtype fixed to
 * int8_TYPE. All elements are initialised to zero.
 *
 ...
 *
 * @return int8_matrix_expect_t with has_value true on success.
 *
 * @code{.c}
 *     allocator_vtable_t a = heap_allocator();
...
/**
 * @brief Initialize an empty COO sparse int8 matrix.
 *
 * Creates a coordinate-list sparse matrix with storage for up to @p capacity
 * entries. The dtype is fixed to int8_TYPE.
 *
 * @param rows      Number of matrix rows. Must be > 0.
 * @param cols      Number of matrix columns. Must be > 0.
 ...
 *     int8_matrix_t* m = r.u.value;
 *
 *     push_back_int8_coo_matrix(m, 0, 5, 3);
 *     push_back_int8_coo_matrix(m, 42, 99, 7);
 *
 *     return_int8_matrix(m);
 * @endcode
 */
...
/**
 * @brief Release all storage owned by a int8 matrix.
 *
...
/**
 * @brief Read a int8 element at the given row and column.
 *
 ...
 * @param out  Receives the int8 value. Must not be NULL.
 *
 * @return NO_ERROR on success, or NULL_POINTER / INVALID_ARG / OUT_OF_BOUNDS.
 *
 * @code
 *     int8_t v = 0;
 *     get_int8_matrix(m, 1, 2, &v);
 * @endcode
 */
...
/**
 * @brief Write a int8 element at the given row and column.
 *
...
/**
 * @brief Reserve additional entry capacity for a int8 COO matrix.
 *
...
/**
 * @brief Append or overwrite a int8 entry in a COO matrix.
 *
 ...
 * @param row    Zero-based row index.
 * @param col    Zero-based column index.
 * @param value  The int8 value to insert.
 *
 * @return NO_ERROR on success, or an appropriate error_code_t.
 *
 * @code
...
/**
 * @brief Clear a int8 matrix, preserving its allocated storage.
 *
...
/**
 * @brief Convert a int8 matrix to a different storage format.
 *
 ...
 *
 * @code
 *     int8_matrix_expect_t csr =
 *         convert_int8_matrix(dense_m, CSR_MATRIX, a);
 * @endcode
 */
...
/**
 * @brief Fill a int8 matrix with a single repeated value.
 *
 * Nonzero fill is supported only for dense matrices. Supplying 0.0f resets
 * the matrix through clear_int8_matrix.
 *
 * @param mat    Matrix to fill.
 * @param value  The int8 fill value.
 *
 * @return NO_ERROR on success, or an appropriate error_code_t.
 *
 * @code
 *     fill_int8_matrix(m, 1);   // every element becomes 1.0
 *     fill_int8_matrix(m, 0);   // equivalent to clear
 * @endcode
 */
...
/**
 * @brief Test whether two int8 matrices have identical shape.
 */
...
/**
 * @brief Test whether a int8 matrix is square.
 */
...
/**
 * @brief Test whether a int8 matrix uses a sparse representation.
 */
...
/**
 * @brief Test whether a int8 matrix is logically all zeros.
 */
...
/**
 * @brief Compare two int8 matrices for numerical equality.
 *
 ...
 *
 * Equality is defined using standard int8ing-point comparison semantics:
 *
 * Matrices must:
 * - be non-NULL
 * - have the same dimensions (rows and columns)
 * - have dtype == INT8_TYPE
 *
 ...
 *       comparisons), use :c:func:`int8_matrix_equal_cmp`.
 *
 * @code{.c}
 * allocator_vtable_t alloc = heap_allocator();
 *
 * int8_matrix_expect_t r1 = init_int8_dense_matrix(2, 2, alloc);
 * int8_matrix_expect_t r2 = init_int8_dense_matrix(2, 2, alloc);
 *
 * int8_matrix_t* a = r1.u.value;
 * int8_matrix_t* b = r2.u.value;
 *
 * set_int8_matrix(a, 0, 0, 4);
 * set_int8_matrix(b, 0, 0,  0);
 *
 * assert_true(int8_matrix_equal(a, b)); // false
 *
 ...
 *
 * int8_matrix_expect_t dense_r = init_int8_dense_matrix(2, 2, alloc);
 * int8_matrix_t* dense = dense_r.u.value;
 *
 * set_int8_matrix(dense, 0, 1, 3);
 *
 * int8_matrix_expect_t csr_r =
 *     convert_int8_matrix(dense, CSR_MATRIX, alloc);
 *
 * int8_matrix_t* csr = csr_r.u.value;
 *
 * // Same logical values → equal
 * assert_true(int8_matrix_equal(dense, csr)); // true
 *
...
/**
 * @brief Test whether two int8 matrices are compatible for addition.
 *
...
/**
 * @brief Test whether two int8 matrices are compatible for multiplication.
 *
...
/**
 * @brief Swap two rows of a int8 matrix in place.
 *
...
/**
 * @brief Swap two columns of a int8 matrix in place.
 *
 * Dense and COO matrices are supported. CSR/CSC return OPERATION_UNAVAILABLE.
 *
 * @param mat  Matrix to modify.
...
/**
 * @brief Initialize a dense int8 identity matrix.
 *
 ...
 *
 * @code
 *     int8_matrix_expect_t r = init_int8_identity_matrix(3, a);
 *     // 3x3 identity matrix.
 * @endcode
 */
...
/**
 * @brief Initialize a dense int8 row vector (1 x length).
 *
...
/**
 * @brief Initialize a dense int8 column vector (length x 1).
 *
...
/**
 * @brief Test whether a int8 matrix has row-vector shape (1 x N).
 */
...
/**
 * @brief Test whether a int8 matrix has column-vector shape (N x 1).
 */
...
/**
 * @brief Test whether a int8 matrix has vector shape (row or column).
 */
...
/**
 * @brief Return the logical length of a vector-shaped int8 matrix.
 *
...
/**
 * @brief Convert a int8 matrix to a different storage format using an
 *        optional semantic zero-test.
 *
 * This function performs the same structural conversion as
 * :c:func:`convert_int8_matrix`, but allows the caller to define what
 * int8 values should be treated as zero during dense-to-sparse conversion.
 *
 ...
 *
 * @return int8_matrix_expect_t with has_value true on success, or
 ...
 *
 * @code{.c}
 * static bool zero_or_neg_one(int8 v) {
 *     return (v == 0) || (v == 3);
 * }
 *
 * int8_matrix_expect_t r =
 *     convert_int8_matrix_zero(dense_m, CSR_MATRIX, alloc, zero_or_neg_one);
 * @endcode
 *
...
/**
 * @brief Print an int8 matrix in a human-readable format.
 *
 ...
 *
 * @param mat
 * Pointer to the source @ref int8_matrix_t to print. Must not be NULL.
 *
 * @param stream
 ...
 *         - any error returned by get_int8_matrix during sparse traversal
 */
error_code_t print_int8_matrix(const int8_matrix_t* mat, FILE* stream);
// -------------------------------------------------------------------------------- 

/**
 * @brief Retrieve the minimum value from a uint8 matrix.
 *
 * This function determines the minimum element in a matrix of type
 * `uint8_matrix_t`. It delegates the index computation to the generic
 ...
 *
 * @param mat Pointer to the int8 matrix.
 *
 * @return int8_expect_t
 ...
 * - TYPE_MISMATCH  if the matrix dtype is not int8_TYPE.
 ...
 * @see matrix_min
 */
int8_expect_t int8_matrix_min(const int8_matrix_t* mat);
// -------------------------------------------------------------------------------- 

/**
 * @brief Retrieve the maximum value from a int8 matrix.
 *
 * This function determines the maximum element in a matrix of type
 * `int8_matrix_t`. It delegates the index computation to the generic
 ...
 * @see matrix_max
 */
int8_expect_t int8_matrix_max(const int8_matrix_t* mat);
// ================================================================================ 
// ================================================================================ 
#ifdef __cplusplus
}
