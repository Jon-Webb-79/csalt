...
/**
 * @brief Bracketed binary search of a sorted int64_t tensor.
 *
 * Locates the pair of adjacent indices that bracket value in a sorted
 * ascending tensor.  Unlike int64_tensor_bsearch which returns NOT_FOUND
...
/**
 * @brief Compare two int64_t tensors for equality.
 *
 * Compares the populated data buffers of two int64_t tensors using
 ...
 *
 * @code{.c}
 * int64_tensor_t* a = init_int64_array(4, false, heap_allocator()).u.value;
 * int64_tensor_t* b = init_int64_array(4, false, heap_allocator()).u.value;
 *
 * push_back_int64_array(a, -1);
 * push_back_int64_array(a,  2);
 * push_back_int64_array(b, -1);
 * push_back_int64_array(b,  2);
 *
 * // Value equality — signed content matches
 * bool eq = int64_tensors_equal(a, b, false);   // true
 *
 * // -1 and 127 have different bit patterns — not equal
 * set_int64_tensor_index(b, 0, 127);
 * eq = int64_tensors_equal(a, b, false);         // false
 *
