...
// ================================================================================
// - File:    c_uint16.h
// - Purpose: This file contains prototypes for uint16_t data structures and 
...
/**
 * @brief Type-safe wrapper for a tensor containing uint16_t elements.
 *
 ...
 * Use the typed wrapper functions (init_uint16_array, push_back_uint16_tensor,
 ...
 *
 * @code{.c}
 * uint16_tensor_expect_t r = init_uint16_array(8, true, heap_allocator());
 * if (!r.has_value) { handle(r.u.error); }
 * uint16_tensor_t* a = r.u.value;
 * @endcode
 */
typedef struct {
    bool has_value;
    union {
        uint16_tensor_t* value;
        error_code_t    error;
    } u;
} uint16_tensor_expect_t;
...
/**
 * @brief Initialise a fixed-shape N-dimensional tensor of uint16_t elements.
 *
 * Allocates the underlying tensor_t via init_tensor and then allocates a
 * thin uint16_tensor_t wrapper struct through the same allocator. All slots
 ...
 * return_uint16_tensor when the tensor is no longer needed.
 *
 ...
 *
 * @return uint16_tensor_expect_t with has_value true on success.
 *         On failure has_value is false and u.error is set.
 *
 * @code{.c}
 * // Create a 3x4 matrix of uint16_t values
 * size_t shape[] = { 3u, 4u };
 * uint16_tensor_expect_t r = init_uint16_tensor(2u, shape, heap_allocator());
 * if (!r.has_value) { fprintf(stderr, "%s\n", error_to_string(r.u.error)); }
 * uint16_tensor_t* mat = r.u.value;
 *
 * // Write to element (1, 2)
 * size_t idx[] = { 1u, 2u };
 * set_uint16_tensor_nd_index(mat, idx, 42u);
 *
 * // Read it back
 * uint16_t val;
 * get_uint16_tensor_nd_index(mat, idx, &val);   // val == 42
 *
...
/*
 * Allocator convention for uint16_tensor_t functions:
 *
 * init_uint16_array        — takes allocator_vtable_t by value. An allocator
 *                           is mandatory since there is no existing tensor to
 *                           inherit one from.
 *
 * copy_uint16_tensor       — takes allocator_vtable_t* (pointer). Pass NULL to
 * slice_uint16_tensor_array  inherit the allocator from the source tensor, or
 *                           a non-NULL pointer to use a different allocator.
 */
 
/**
 * @brief Initialise a dynamic 1-D array of uint16_t elements.
 *
 * Allocates the underlying tensor_t via init_tensor_array and then
 * allocates a thin uint16_tensor_t wrapper struct through the same
 * allocator. Both allocations must be returned by calling
 * return_uint16_tensor when the array is no longer needed.
 *
 ...
 *
 * @code{.c}
 * uint16_tensor_expect_t r = init_uint16_array(16, true, heap_allocator());
 * if (!r.has_value) { fprintf(stderr, "%s\n", error_to_string(r.u.error)); }
 * uint16_tensor_t* arr = r.u.value;
 * push_back_uint16_tensor(arr, 42u);
...
/**
 * @brief Return a uint16_tensor_t and its underlying tensor to the allocator.
 *
 ...
 * @endcode
 */
void return_uint16_tensor(uint16_tensor_t* t);
// -------------------------------------------------------------------------------- 

/**
 * @brief Create a deep copy of a uint16_tensor_t.
 *
 * Copies the underlying tensor via copy_tensor then allocates a new
 * uint16_tensor_t wrapper struct. Both allocations are returned by calling
 * return_uint16_tensor on the returned pointer. On partial failure the
 ...
 * affect the other.
 *
 * @param src     Pointer to the source uint16_tensor_t. Must not be NULL.
 ...
 * uint16_tensor_expect_t c = copy_uint16_tensor(arr, NULL);
 * if (!c.has_value) { fprintf(stderr, "%s\n", error_to_string(c.u.error)); }
 * uint16_tensor_t* copy = c.u.value;
 * // ...
 * return_uint16_tensor(copy);
 *
 * // Copy using a different allocator
 * allocator_vtable_t arena = arena_allocator(&my_arena);
 * uint16_tensor_expect_t c2 = copy_uint16_tensor(arr, &arena);
 * @endcode
 */
 ...
 *
 * @code{.c}
 * size_t n = uint16_tensor_size(arr);   // e.g. 3 after three push_back calls
 * @endcode
 */
static inline size_t uint16_tensor_size(const uint16_tensor_t* t) {
    if (t == NULL) return 0u;
 ...
 *
 * @code{.c}
 * size_t cap = uint16_tensor_alloc(arr);   // e.g. 16 after init_uint16_array(16,...)
 * @endcode
 */
static inline size_t uint16_tensor_alloc(const uint16_tensor_t* t) {
    if (t == NULL) return 0u;
 ...
 *
 * Always returns sizeof(uint16_t) == 1 for a correctly initialised
 * uint16_tensor_t. Returns 0 if t is NULL.
 *
 * @code{.c}
 * size_t esz = uint16_tensor_data_size(arr);   // 1
 * @endcode
 */
static inline size_t uint16_tensor_data_size(const uint16_tensor_t* t) {
    if (t == NULL) return 0u;
 ...
 *
 * Always returns UINT16_TYPE for a correctly initialised uint16_tensor_t.
 * Returns UNKNOWN_TYPE if t is NULL.
 *
 * @code{.c}
 * dtype_id_t id = uint16_tensor_dtype(arr);   // UINT16_TYPE
 * @endcode
 */
static inline dtype_id_t uint16_tensor_dtype(const uint16_tensor_t* t) {
 ...
 * uint16_tensor_expect_t r = init_uint16_array(8, false, heap_allocator());
 * uint16_tensor_t* arr = r.u.value;
 * bool empty = is_uint16_tensor_empty(arr);   // true before any push
 * push_back_uint16_tensor(arr, 1u);
 * empty = is_uint16_tensor_empty(arr);         // false after push
 * return_uint16_tensor(arr);
 * @endcode
 */
static inline bool is_uint16_tensor_empty(const uint16_tensor_t* t) {
 ...
 *
 * @code{.c}
 * uint16_tensor_expect_t r = init_uint16_array(2, false, heap_allocator());
 * uint16_tensor_t* arr = r.u.value;
 * push_back_uint16_tensor(arr, 1u);
 * push_back_uint16_tensor(arr, 2u);
 * bool full = is_uint16_tensor_full(arr);   // true — len == alloc == 2
 * return_uint16_tensor(arr);
 * @endcode
 */
static inline bool is_uint16_tensor_full(const uint16_tensor_t* t) {
    if (t == NULL) return true;
 ...
 *
 * @code{.c}
 * uint16_tensor_expect_t r = init_uint16_array(4, false, heap_allocator());
 * uint16_tensor_t* arr = r.u.value;
 * push_back_uint16_tensor(arr, 10u);
 * const uint16_t* p = arr->base->data;
 * bool valid = is_uint16_tensor_ptr(arr, p);   // true — points to element 0
 * return_uint16_tensor(arr);
 * @endcode
 */
static inline bool is_uint16_tensor_ptr(const uint16_tensor_t* t, const uint16_t* ptr) {
 ...
 *
 * Always returns 1 for a tensor initialised via init_uint16_array.
 * Returns 0 if t is NULL.
 *
 * @code{.c}
 * uint8_t ndim = uint16_tensor_ndim(arr);   // 1
 * @endcode
 */
static inline uint8_t uint16_tensor_ndim(const uint16_tensor_t* t) {
    if (t == NULL) return 0u;
 ...
 * uint16_tensor_expect_t r = init_uint16_array(8, false, heap_allocator());
 * uint16_tensor_t* arr = r.u.value;
 * size_t d0 = uint16_tensor_shape_dim(arr, 0u);   // 8
 * return_uint16_tensor(arr);
 * @endcode
 */
static inline size_t uint16_tensor_shape_dim(const uint16_tensor_t* t, uint16_t dim) {
    if (t == NULL) return 0u;
 ...
 * @code{.c}
 * size_t shape[1];
 * error_code_t err = uint16_tensor_shape(arr, shape, 1u);
 * // shape[0] now holds the single dimension size
 * @endcode
 */
static inline error_code_t uint16_tensor_shape(const uint16_tensor_t* t, size_t* out, uint16_t count) {
 ...
 *
 * @code{.c}
 * const size_t* sp = uint16_tensor_shape_ptr(arr);
 * if (sp) printf("dim0 = %zu\n", sp[0]);
 * @endcode
 */
static inline const size_t* uint16_tensor_shape_ptr(const uint16_tensor_t* t) {
 ...
 *
 * Strides are in bytes. For a contiguous 1-D uint16_t array,
 ...
 * @endcode
 */
static inline const size_t* uint16_tensor_strides_ptr(const uint16_tensor_t* t) {
    if (t == NULL) return NULL;
 ...
 * @code{.c}
 * char buf[32];
 * error_code_t err = uint16_tensor_shape_str(arr, buf, sizeof(buf));
 * if (err == NO_ERROR) printf("shape = %s\n", buf);   // e.g. "(8)"
 * @endcode
 */
static inline error_code_t uint16_tensor_shape_str(const uint16_tensor_t* t, 
                                                  char* buf, 
                                                  size_t buf_len) {
 ...
 * push_back_uint16_tensor(arr, 1u);
 * push_back_uint16_tensor(arr, 2u);
 * clear_uint16_tensor(arr);
 * size_t n = uint16_tensor_size(arr);   // 0
 * @endcode
 */
static inline error_code_t clear_uint16_tensor(uint16_tensor_t* t) {
 ...
 *
 * @code{.c}
 * uint16_tensor_expect_t r1 = init_uint16_array(8, false, heap_allocator());
 * uint16_tensor_expect_t r2 = init_uint16_array(4, false, heap_allocator());
 * uint16_tensor_t* dst = r1.u.value;
 * uint16_tensor_t* src = r2.u.value;
 * push_back_uint16_tensor(dst, 1u);
 * push_back_uint16_tensor(src, 2u);
 * push_back_uint16_tensor(src, 3u);
 * concat_uint16_tensor_array(dst, src);   // dst = [1, 2, 3]
 * return_uint16_tensor(dst);
 * return_uint16_tensor(src);
 * @endcode
 */
static inline error_code_t concat_uint16_tensor_array(uint16_tensor_t* dst,
                                                     const uint16_tensor_t* src) {
    if (dst == NULL || src == NULL) return NULL_POINTER;
    return concat_tensor_array(dst->base, src->base, UINT16_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 * The caller is responsible for calling return_uint16_tensor on the result.
 *
 ...
 * @code{.c}
 * // arr = [10, 20, 30, 40, 50]
 * uint16_tensor_expect_t s = slice_uint16_tensor_array(arr, 1u, 4u, NULL);
 * if (!s.has_value) { fprintf(stderr, "%s\n", error_to_string(s.u.error)); }
 * uint16_tensor_t* sl = s.u.value;   // sl = [20, 30, 40]
...
/**
 * @brief Reverse the populated elements of a uint16_t tensor in place.
 *
 ...
 * @code
 * // arr = [1, 2, 3, 4, 5]
 * error_code_t err = reverse_uint16_tensor(arr);
 * // arr = [5, 4, 3, 2, 1]
 * @endcode
 */
static inline error_code_t reverse_uint16_tensor(uint16_tensor_t* t) {
    if (t == NULL) return NULL_POINTER;
...
/**
 * @brief Sort the populated elements of a uint16_t tensor in place.
 *
 ...
 * fallback for partitions of fewer than 10 elements. The uint16_t
 ...
 * @code
 * // arr = [3, 1, 4, 1, 5, 9, 2, 6]
 * error_code_t err = sort_uint16_tensor(arr, FORWARD);
 * // arr = [1, 1, 2, 3, 4, 5, 6, 9]
 *
 * err = sort_uint16_tensor(arr, REVERSE);
 * // arr = [9, 6, 5, 4, 3, 2, 1, 1]
 * @endcode
 */
static inline error_code_t sort_uint16_tensor(uint16_tensor_t* t,
                                             direction_t dir) {
    if (t == NULL) return NULL_POINTER;
...
/**
 * @brief Binary search a sorted uint16_t tensor for a target value.
 *
 ...
 * data is unsorted.  Use sort_uint16_tensor before calling if the
 ...
 * data[*index] == value.  Use uint16_tensor_lsearch if the first
 ...
 * push_back_uint16_array(arr, 40u);
 * push_back_uint16_array(arr, 20u);
 * push_back_uint16_array(arr, 30u);
 *
 * // Sort before searching
 * sort_uint16_tensor(arr, FORWARD);
 * // arr = [10, 20, 30, 40]
 *
 * size_t idx = 0u;
 * error_code_t err = uint16_tensor_bsearch(arr, &idx, 30u);
 * // err == NO_ERROR, idx == 2
 *
 * err = uint16_tensor_bsearch(arr, &idx, 25u);
 * // err == NOT_FOUND, idx unchanged
 *
 * return_uint16_tensor(arr);
 * @endcode
...
/**
 * @brief Bracketed binary search of a sorted uint16_t tensor.
 *
 * Locates the pair of adjacent indices that bracket value in a sorted
 * ascending tensor.  Unlike uint16_tensor_bsearch which returns
 ...
 *
 * // Bracketed
 * r = uint16_tensor_bbsearch(arr, 25u);
...
/**
 * @brief Compare two uint16_t tensors for equality.
 *
 * Compares the populated data buffers of two uint16_t tensors using
 ...
 *
 * @code{.c}
 * uint16_tensor_t* a = init_uint16_array(8, false, heap_allocator()).u.value;
 * uint16_tensor_t* b = init_uint16_array(4, false, heap_allocator()).u.value;
 *
 * push_back_uint16_array(a, 1u);
 * push_back_uint16_array(a, 2u);
 * push_back_uint16_array(b, 1u);
 * push_back_uint16_array(b, 2u);
 *
 * // Value equality — content matches, capacity difference ignored
 * bool eq = uint16_tensors_equal(a, b, false);   // true
 *
 * // Structural equality — capacity differs (8 vs 4)
 * eq = uint16_tensors_equal(a, b, true);          // false
 *
 * return_uint16_tensor(a);
 * return_uint16_tensor(b);
 * @endcode
 */
bool uint16_tensors_equal(const uint16_tensor_t* one,
                         const uint16_tensor_t* two,
                         bool                  meta);
// -------------------------------------------------------------------------------- 

/**
 * @brief Find the minimum element in a uint16 tensor.
 *
 ...
 *
 * Note: SSE2 and SSSE3 lack an unsigned 16-bit min intrinsic, so those
 * paths use an XOR-bias technique (XOR each lane with 0x8000 to map
 * unsigned ordering onto signed ordering, reduce with _mm_min_epi16,
 * then XOR the result back).  SSE4.1 and later use native
 * _mm_min_epu16 and _mm_minpos_epu16 (PHMINPOSUW) for efficient
 * horizontal reduction.
 *
 * Works on both TENSOR_STRUCT and ARRAY_STRUCT modes — the scan always
 * covers exactly t->base->len elements.
 *
 * @param t      Pointer to the source uint16 tensor. Must not be NULL.
 * @param value  Pointer to a uint16_t to receive the minimum value.
 *               Must not be NULL.
 *
 * @return NO_ERROR on success, or one of:
 ...
 * @code{.c}
 * uint16_t min_val;
 * error_code_t err = min_uint16_tensor(arr, &min_val);
 * if (err != NO_ERROR) {
 *     fprintf(stderr, "%s\n", error_to_string(err));
 * }
 * printf("Minimum: %" PRIu16 "\n", min_val);
 * @endcode
 */
 ...
 *
 * @code{.c}
 * push_back_uint16_tensor(arr, 0u);   // arr = [0]
 * set_uint16_tensor_index(arr, 0u, 99u);   // arr = [99]
 * @endcode
 */
static inline error_code_t set_uint16_tensor_index(uint16_tensor_t* t,
                                                  size_t index,
                                                  uint16_t data) {
    if (t == NULL) return NULL_POINTER;
    return set_tensor_index(t->base, index, &data, UINT16_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 *
 * @code{.c}
 * // For a 3x3 uint16 matrix initialised via init_tensor
 * size_t idx[] = { 1u, 2u };
 * set_uint16_tensor_nd_index(mat, idx, 42u);   // mat[1][2] = 42
 * @endcode
 */
static inline error_code_t set_uint16_tensor_nd_index(uint16_tensor_t* t,
                                                     const size_t* idx,
                                                     uint16_t data) {
    if (t == NULL) return NULL_POINTER;
    return set_tensor_nd_index(t->base, idx, &data, UINT16_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 * @code{.c}
 * push_back_uint16_tensor(arr, 10u);
 * push_back_uint16_tensor(arr, 20u);
 * // arr = [10, 20], len == 2
 * @endcode
 */
static inline error_code_t push_back_uint16_array(uint16_tensor_t* t,
                                                 uint16_t data) {
    if (t == NULL) return NULL_POINTER;
    return push_back_tensor(t->base, &data, UINT16_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 * @code{.c}
 * push_back_uint16_tensor(arr, 10u);
 * push_front_uint16_tensor(arr, 99u);
 * // arr = [99, 10]
 * @endcode
 */
static inline error_code_t push_front_uint16_array(uint16_tensor_t* t,
                                                  uint16_t data) {
    if (t == NULL) return NULL_POINTER;
    return push_front_tensor(t->base, &data, UINT16_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 * @code{.c}
 * // arr = [10, 20, 30]
 * push_at_uint16_tensor(arr, 99u, 1u);
 * // arr = [10, 99, 20, 30]
 * @endcode
 */
static inline error_code_t push_at_uint16_array(uint16_tensor_t* t,
                                               uint16_t data,
                                               size_t index) {
    if (t == NULL) return NULL_POINTER;
    return push_at_tensor(t->base, &data, index, UINT16_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 * // arr = [10, 20, 30]
 * uint16_t val;
 * pop_back_uint16_tensor(arr, &val);   // val == 30, arr = [10, 20]
 * pop_back_uint16_tensor(arr, NULL);   // discard, arr = [10]
 * @endcode
 */
static inline error_code_t pop_back_uint16_array(uint16_tensor_t* t,
                                                uint16_t* out) {
    if (t == NULL) return NULL_POINTER;
    return pop_back_tensor(t->base, out, UINT16_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 * // arr = [10, 20, 30]
 * uint16_t val;
 * pop_front_uint16_tensor(arr, &val);   // val == 10, arr = [20, 30]
 * @endcode
 */
static inline error_code_t pop_front_uint16_array(uint16_tensor_t* t,
                                                 uint16_t* out) {
    if (t == NULL) return NULL_POINTER;
    return pop_front_tensor(t->base, out, UINT16_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 * // arr = [10, 20, 30, 40]
 * uint16_t val;
 * pop_at_uint16_tensor(arr, &val, 2u);   // val == 30, arr = [10, 20, 40]
 * @endcode
 */
static inline error_code_t pop_at_uint16_array(uint16_tensor_t* t,
                                              uint16_t* out,
                                              size_t index) {
    if (t == NULL) return NULL_POINTER;
    return pop_at_tensor(t->base, out, index, UINT16_TYPE);
}
// ================================================================================ 
// ================================================================================ 
// RETRIEVE DATA
 ...
 * // arr = [10, 20, 30]
 * uint16_t val;
 * get_uint16_tensor_index(arr, 1u, &val);   // val == 20
 * @endcode
 */
static inline error_code_t get_uint16_tensor_index(const uint16_tensor_t* t,
                                                  size_t index,
                                                  uint16_t* out) {
    if (t == NULL || out == NULL) return NULL_POINTER;
    return get_tensor_index(t->base, index, out, UINT16_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 *
 * @code{.c}
 * // For a 3x3 uint16 matrix
 * size_t idx[] = { 1u, 2u };
 * uint16_t val;
 * get_uint16_tensor_nd_index(mat, idx, &val);   // val = mat[1][2]
 * @endcode
 */
static inline error_code_t get_uint16_tensor_nd_index(const uint16_tensor_t* t,
                                                     const size_t* idx,
                                                     uint16_t* out) {
    if (t == NULL || idx == NULL || out == NULL) return NULL_POINTER;
    return get_tensor_nd_index(t->base, idx, out, UINT16_TYPE);
}
// ================================================================================ 
// ================================================================================ 
#ifdef __cplusplus
