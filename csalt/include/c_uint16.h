...
/**
 * @brief Add a scalar value to every element of a uint16 array.
 *
 ...
 *
 * @param array Pointer to the uint16 array to modify.
 ...
 * @retval TYPE_MISMATCH  If the underlying dtype does not match uint16_TYPE.
 *
 * @note
 * The addition uses standard unsigned 8-bit arithmetic. If the result of
 * adding @p value to an element exceeds uint16_MAX, it wraps around modulo 256.
 *
 * @warning
...
/**
 * @brief Retrieve the minimum value from a uint16 matrix.
 *
 * This function determines the minimum element in a matrix of type
 * `uint16_matrix_t`. It delegates the index computation to the generic
 ...
 *
 * @param mat Pointer to the uint16 matrix.
 *
 * @return uint16_expect_t
 ...
 * - TYPE_MISMATCH  if the matrix dtype is not uint16_TYPE.
...
/**
 * @brief Retrieve the maximum value from a uint16 matrix.
 *
