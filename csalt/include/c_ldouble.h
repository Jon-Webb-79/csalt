...
/**
 * @brief Return the element at @p index from an ::ldouble_v with bounds checking.
 *
 * On success, returns `vec->data[index]`. On failure, returns the sentinel
 * `LDBL_MAX` and sets `errno` to indicate the reason. This accessor does **not**
 * modify `vec->error`; callers should check `errno` (or validate inputs) to
 * distinguish a real value equal to `INT_MAX` from an error.
 *
 * @param vec    Poldoubleer to an ::ldouble_v (must be non-NULL and have `data != NULL`).
 * @param index  Zero-based index to access; must satisfy `index < vec->len`.
 *
 * @return
 * - The `long double` at position @p index on success.
 * - `LDBL_MAX` on failure (and `errno` is set).
 *
 * @par Errors
 ...
 * `LDBL_MAX` legitimately, check `errno` (or pre-validate inputs) to disambiguate.
 *
 * @warning
 ...
 * push_back_ldouble_vector(v, 20.0);
 * errno = 0;
 * ldouble x = ldouble_vector_index(v, 1);   // x == 20, errno == 0
 *
 * // Failure: out of bounds
 * errno = 0;
 * ldouble y = ldouble_vector_index(v, 5);   // y == LDBL_MAX, errno == ERANGE
 *
 * // Failure: NULL vector
 * errno = 0;
 * ldouble z = ldouble_vector_index(NULL, 0); // z == LDBL_MAX, errno == EINVAL
 * free_ldouble_vector(v);
 * @endcode
 */
static inline long double ldouble_vector_index(const ldouble_v* vec, size_t index) {
    if (!vec || !vec->data) {
        errno = EINVAL;
        return LDBL_MAX;
    }
...
/**
 * @brief Remove and return the first element of an ::ldouble_v, shifting the rest left.
 *
 ...
 * reallocation occurs.
 *
 * @param vec  Poldoubleer to an ::ldouble_v (must be non-NULL with `data != NULL` and `len > 0`).
 *
 * @return
 * - The removed `long double` value on success (and sets `vec->error = ::NO_ERROR`).
 * - `LDBL_MAX` on failure (no removal performed).
 *
 * @par Errors
 ...
 *
 * @note The error sentinel is `LDBL_MAX` (see `<limits.h>`). If your vector can
 * legitimately contain `LDBL_MAX`, distinguish success from failure by checking
 * `vec->error` (or `errno`) after the call.
 *
 * @warning
 * - This operation is O(n) due to the left shift; any saved poldoubleers/references to
 ...
 * push_back_ldouble_vector(v, 10.0);
 * push_back_ldouble_vector(v, 20.0);
 * push_back_ldouble_vector(v, 30.0);       // v = [10, 20, 30], len = 3
 *
 * ldouble first = pop_front_ldouble_vector(v); // first == 10, v -> [20, 30, 0], len = 2
 * if (first == LDBL_MAX && v->error != NO_ERROR) {
 *     perror("pop_front_ldouble_vector");
 * }
 * free_ldouble_vector(v);
 *
 * // Example: popping from an empty vector -> failure
 * ldouble_v* w = init_ldouble_vector(1);
 * ldouble val = pop_front_ldouble_vector(w);   // returns LDBL_MAX, w->error == INVALID_ARG
 * free_ldouble_vector(w);
 * @endcode
 */
...
/**
 * @brief Remove and return the element at @p index from an ::ldouble_v, shifting the tail left.
 *
 ...
 * - If the ldoubleernal byte count for `memmove` would overflow `size_t`:
 ...
 *
 * @warning
 * - This operation is O(n - index) due to the left shift; any saved poldoubleers/
 ...
 * ldouble_v* v = init_ldouble_vector(4);
 * push_back_ldouble_vector(v, 10); // [10]
 * push_back_ldouble_vector(v, 20); // [10,20]
 * push_back_ldouble_vector(v, 30); // [10,20,30]
 * ldouble removed = pop_any_ldouble_vector(v, 1); // removes 20; v -> [10,30,0], len=2
 * if (removed == DBL_MAX && v->error != NO_ERROR) {
 *     perror("pop_any_ldouble_vector");
 * }
 * free_ldouble_vector(v);
 *
 * // Example 2: remove last element (no shift)
 * ldouble_v* w = init_ldouble_vector(2);
 * push_back_ldouble_vector(w, 1.0);  // [1]
 * push_back_ldouble_vector(w, 2.0);  // [1,2]
 * (void)pop_any_ldouble_vector(w, w->len - 1); // removes 2; w -> [1,0], len=1
 * free_ldouble_vector(w);
 *
 * // Example 3: invalid index
 * ldouble_v* z = init_ldouble_vector(1);
 * push_back_ldouble_vector(z, 5.0);  // [5]
 * ldouble bad = pop_any_ldouble_vector(z, 3); // OUT_OF_BOUNDS -> returns LDBL_MAX
 * free_ldouble_vector(z);
 * @endcode
 */
...
/**
 * @brief Reverse the elements of an ::ldouble_v in place.
 *
 * Reorders the vector so that the first element becomes the last and vice versa,
 * swapping pairs from the ends toward the center (uses ::swap_ldouble). Works for
 ...
 *
 * @code{.c}
 * ldouble_v* v = init_ldouble_vector(5);
 * push_back_ldouble_vector(v, 1.0);
 * push_back_ldouble_vector(v, 2.0);
 * push_back_ldouble_vector(v, 3.0);
 * push_back_ldouble_vector(v, 4.0);
 * // v->data = [1.0,2.0,3.0,4.0], len = 4
 *
 * reverse_ldouble_vector(v);
 * // v->data = [4.0,3.0,2.0,1.0], len = 4
 *
 * // Error cases
 * errno = 0;
 * reverse_ldouble_vector(NULL);    // errno == EINVAL
 * ldouble_v* w = init_ldouble_vector(1);
 * // w->len == 0
 * reverse_ldouble_vector(w);       // errno == ENODATA
 * free_ldouble_vector(v);
 * free_ldouble_vector(w);
 * @endcode
 */
...
/**
 * @brief Sort an ::ldouble_v in place in ascending or descending order.
 *
 ...
 *
 * @param vec        Poldoubleer to an ::ldouble_v to sort. Must be non-NULL and have
 ...
 * // Ascending sort
 * ldouble_v* v = init_ldouble_vector(8);
 * push_back_ldouble_vector(v, 5.0);
 * push_back_ldouble_vector(v, 2.0);
 * push_back_ldouble_vector(v, 9.0);
 * push_back_ldouble_vector(v, 2.0);
 * sort_ldouble_vector(v, FORWARD);  // v->data: [2.0, 2.0, 5.0, 9.0]
 *
 * // Descending sort
 * sort_ldouble_vector(v, REVERSE);  // v->data: [9.0, 5.0, 2.0, 2.0]
 * free_ldouble_vector(v);
 *
 * // Error case
 * errno = 0;
 * sort_ldouble_vector(NULL, FORWARD); // errno == EINVAL
 * @endcode
 */
...
/**
 * @brief Shrink an ::ldouble_v’s capacity to match its current length.
 *
 ...
 * poldoubleer may change (reallocation), `len` is unchanged, and `error` is set to
 * ::NO_ERROR.
 *
 * @param vec  Poldoubleer to an ::ldouble_v.
 *
 * @return Void.
 *
 ...
 *
 * @warning
 * - On success, any saved poldoubleers ldoubleo `vec->data` become invalid if `realloc`
 *   moves the buffer. Always refresh poldoubleers after trimming.
 ...
 * ldouble_v* v = init_ldouble_vector(16);    // alloc = 16, len = 0
 * push_back_ldouble_vector(v, 1.0);
 * push_back_ldouble_vector(v, 2.0);        // len = 2
 * trim_ldouble_vector(v);                // success -> alloc = 2, len = 2
 * free_ldouble_vector(v);
 *
 * // Already tight: reports INVALID_ARG (no-op)
 * ldouble_v* t = init_ldouble_vector(2);
 * push_back_ldouble_vector(t, 10.0);
 * push_back_ldouble_vector(t, 20.0);       // len = alloc = 2
 * trim_ldouble_vector(t);                 // fails: t->error == INVALID_ARG
 * free_ldouble_vector(t);
 *
 * // Static vector: not allowed
 * {
 *     ldouble_v s = init_ldouble_array(8);    // STATIC storage
 *     push_back_ldouble_vector(&s, 5.0);
 *     trim_ldouble_vector(&s);            // fails: s.error == INVALID_ARG
 * }
 * @endcode
 */
