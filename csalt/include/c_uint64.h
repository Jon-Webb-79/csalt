...
/**
 * @brief Type-safe wrapper for a tensor containing uint64_t elements.
 *
 ...
 * Use the typed wrapper functions (init_uint64_array, push_back_uint64_tensor,
 ...
 *
 * @code{.c}
 * uint64_tensor_expect_t r = init_uint64_array(8, true, heap_allocator());
 * if (!r.has_value) { handle(r.u.error); }
 * uint64_tensor_t* a = r.u.value;
 * @endcode
 */
typedef struct {
    bool has_value;
    union {
        uint64_tensor_t* value;
        error_code_t    error;
    } u;
} uint64_tensor_expect_t;
...
/**
 * @brief Initialise a fixed-shape N-dimensional tensor of uint64_t elements.
 *
 * Allocates the underlying tensor_t via init_tensor and then allocates a
 * thin uint64_tensor_t wrapper struct through the same allocator. All slots
 ...
 * return_uint64_tensor when the tensor is no longer needed.
 *
 ...
 *
 * @return uint64_tensor_expect_t with has_value true on success.
 *         On failure has_value is false and u.error is set.
 *
 * @code{.c}
 * // Create a 3x4 matrix of uint64_t values
 * size_t shape[] = { 3u, 4u };
 * uint64_tensor_expect_t r = init_uint64_tensor(2u, shape, heap_allocator());
 * if (!r.has_value) { fprintf(stderr, "%s\n", error_to_string(r.u.error)); }
 * uint64_tensor_t* mat = r.u.value;
 *
 * // Write to element (1, 2)
 * size_t idx[] = { 1u, 2u };
 * set_uint64_tensor_nd_index(mat, idx, 42u);
 *
 * // Read it back
 * uint64_t val;
 * get_uint64_tensor_nd_index(mat, idx, &val);   // val == 42
 *
...
/*
 * Allocator convention for uint64_tensor_t functions:
 *
 * init_uint64_array        — takes allocator_vtable_t by value. An allocator
 *                           is mandatory since there is no existing tensor to
 *                           inherit one from.
 *
 * copy_uint64_tensor       — takes allocator_vtable_t* (pointer). Pass NULL to
 * slice_uint64_tensor_array  inherit the allocator from the source tensor, or
 *                           a non-NULL pointer to use a different allocator.
 */
 
/**
 * @brief Initialise a dynamic 1-D array of uint64_t elements.
 *
 * Allocates the underlying tensor_t via init_tensor_array and then
 * allocates a thin uint64_tensor_t wrapper struct through the same
 * allocator. Both allocations must be returned by calling
 * return_uint64_tensor when the array is no longer needed.
 *
 ...
 *
 * @code{.c}
 * uint64_tensor_expect_t r = init_uint64_array(16, true, heap_allocator());
 * if (!r.has_value) { fprintf(stderr, "%s\n", error_to_string(r.u.error)); }
 * uint64_tensor_t* arr = r.u.value;
 * push_back_uint64_tensor(arr, 42u);
...
/**
 * @brief Return a uint64_tensor_t and its underlying tensor to the allocator.
 *
 ...
 *
 * @code{.c}
 * uint64_tensor_expect_t r = init_uint64_array(8, false, heap_allocator());
 * uint64_tensor_t* arr = r.u.value;
 * // ... use arr ...
 * return_uint64_tensor(arr);
 ...
 * @endcode
 */
void return_uint64_tensor(uint64_tensor_t* t);
// -------------------------------------------------------------------------------- 

/**
 * @brief Create a deep copy of a uint64_tensor_t.
 *
 * Copies the underlying tensor via copy_tensor then allocates a new
 * uint64_tensor_t wrapper struct. Both allocations are returned by calling
 * return_uint64_tensor on the returned pointer. On partial failure the
 ...
 * affect the other.
 *
 * @param src     Pointer to the source uint64_tensor_t. Must not be NULL.
 ...
 * uint64_tensor_expect_t c = copy_uint64_tensor(arr, NULL);
 * if (!c.has_value) { fprintf(stderr, "%s\n", error_to_string(c.u.error)); }
 * uint64_tensor_t* copy = c.u.value;
 * // ...
 * return_uint64_tensor(copy);
 *
 * // Copy using a different allocator
 * allocator_vtable_t arena = arena_allocator(&my_arena);
 * uint64_tensor_expect_t c2 = copy_uint64_tensor(arr, &arena);
 * @endcode
 */
 ...
 *
 * @code{.c}
 * size_t n = uint64_tensor_size(arr);   // e.g. 3 after three push_back calls
 * @endcode
 */
static inline size_t uint64_tensor_size(const uint64_tensor_t* t) {
    if (t == NULL) return 0u;
 ...
 *
 * @code{.c}
 * size_t cap = uint64_tensor_alloc(arr);   // e.g. 16 after init_uint64_array(16,...)
 * @endcode
 */
static inline size_t uint64_tensor_alloc(const uint64_tensor_t* t) {
    if (t == NULL) return 0u;
 ...
 *
 * Always returns sizeof(uint64_t) == 1 for a correctly initialised
 * uint64_tensor_t. Returns 0 if t is NULL.
 *
 * @code{.c}
 * size_t esz = uint64_tensor_data_size(arr);   // 1
 * @endcode
 */
static inline size_t uint64_tensor_data_size(const uint64_tensor_t* t) {
    if (t == NULL) return 0u;
 ...
 *
 * Always returns UINT64_TYPE for a correctly initialised uint64_tensor_t.
 * Returns UNKNOWN_TYPE if t is NULL.
 *
 * @code{.c}
 * dtype_id_t id = uint64_tensor_dtype(arr);   // UINT64_TYPE
 * @endcode
 */
static inline dtype_id_t uint64_tensor_dtype(const uint64_tensor_t* t) {
 ...
 * uint64_tensor_expect_t r = init_uint64_array(8, false, heap_allocator());
 * uint64_tensor_t* arr = r.u.value;
 * bool empty = is_uint64_tensor_empty(arr);   // true before any push
 * push_back_uint64_tensor(arr, 1u);
 * empty = is_uint64_tensor_empty(arr);         // false after push
 * return_uint64_tensor(arr);
 * @endcode
 */
static inline bool is_uint64_tensor_empty(const uint64_tensor_t* t) {
 ...
 *
 * @code{.c}
 * uint64_tensor_expect_t r = init_uint64_array(2, false, heap_allocator());
 * uint64_tensor_t* arr = r.u.value;
 * push_back_uint64_tensor(arr, 1u);
 * push_back_uint64_tensor(arr, 2u);
 * bool full = is_uint64_tensor_full(arr);   // true — len == alloc == 2
 * return_uint64_tensor(arr);
 * @endcode
 */
static inline bool is_uint64_tensor_full(const uint64_tensor_t* t) {
    if (t == NULL) return true;
 ...
 *
 * @code{.c}
 * uint64_tensor_expect_t r = init_uint64_array(4, false, heap_allocator());
 * uint64_tensor_t* arr = r.u.value;
 * push_back_uint64_tensor(arr, 10u);
 * const uint64_t* p = arr->base->data;
 * bool valid = is_uint64_tensor_ptr(arr, p);   // true — points to element 0
 * return_uint64_tensor(arr);
 * @endcode
 */
static inline bool is_uint64_tensor_ptr(const uint64_tensor_t* t, const uint64_t* ptr) {
 ...
 *
 * Always returns 1 for a tensor initialised via init_uint64_array.
 * Returns 0 if t is NULL.
 *
 * @code{.c}
 * uint8_t ndim = uint64_tensor_ndim(arr);   // 1
 * @endcode
 */
static inline uint8_t uint64_tensor_ndim(const uint64_tensor_t* t) {
    if (t == NULL) return 0u;
 ...
 * uint64_tensor_expect_t r = init_uint64_array(8, false, heap_allocator());
 * uint64_tensor_t* arr = r.u.value;
 * size_t d0 = uint64_tensor_shape_dim(arr, 0u);   // 8
 * return_uint64_tensor(arr);
 * @endcode
 */
static inline size_t uint64_tensor_shape_dim(const uint64_tensor_t* t, uint64_t dim) {
    if (t == NULL) return 0u;
 ...
 * @code{.c}
 * size_t shape[1];
 * error_code_t err = uint64_tensor_shape(arr, shape, 1u);
 * // shape[0] now holds the single dimension size
 * @endcode
 */
static inline error_code_t uint64_tensor_shape(const uint64_tensor_t* t, size_t* out, uint64_t count) {
 ...
 *
 * @code{.c}
 * const size_t* sp = uint64_tensor_shape_ptr(arr);
 * if (sp) printf("dim0 = %zu\n", sp[0]);
 * @endcode
 */
static inline const size_t* uint64_tensor_shape_ptr(const uint64_tensor_t* t) {
 ...
 *
 * Strides are in bytes. For a contiguous 1-D uint64_t array,
 ...
 * @endcode
 */
static inline const size_t* uint64_tensor_strides_ptr(const uint64_tensor_t* t) {
    if (t == NULL) return NULL;
 ...
 * @code{.c}
 * char buf[32];
 * error_code_t err = uint64_tensor_shape_str(arr, buf, sizeof(buf));
 * if (err == NO_ERROR) printf("shape = %s\n", buf);   // e.g. "(8)"
 * @endcode
 */
static inline error_code_t uint64_tensor_shape_str(const uint64_tensor_t* t, 
                                                  char* buf, 
                                                  size_t buf_len) {
 ...
 * push_back_uint64_tensor(arr, 1u);
 * push_back_uint64_tensor(arr, 2u);
 * clear_uint64_tensor(arr);
 * size_t n = uint64_tensor_size(arr);   // 0
 * @endcode
 */
static inline error_code_t clear_uint64_tensor(uint64_tensor_t* t) {
 ...
 *
 * @code{.c}
 * uint64_tensor_expect_t r1 = init_uint64_array(8, false, heap_allocator());
 * uint64_tensor_expect_t r2 = init_uint64_array(4, false, heap_allocator());
 * uint64_tensor_t* dst = r1.u.value;
 * uint64_tensor_t* src = r2.u.value;
 * push_back_uint64_tensor(dst, 1u);
 * push_back_uint64_tensor(src, 2u);
 * push_back_uint64_tensor(src, 3u);
 * concat_uint64_tensor_array(dst, src);   // dst = [1, 2, 3]
 * return_uint64_tensor(dst);
 * return_uint64_tensor(src);
 * @endcode
 */
static inline error_code_t concat_uint64_tensor_array(uint64_tensor_t* dst,
                                                     const uint64_tensor_t* src) {
    if (dst == NULL || src == NULL) return NULL_POINTER;
    return concat_tensor_array(dst->base, src->base, UINT64_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 * The caller is responsible for calling return_uint64_tensor on the result.
 *
 ...
 * @code{.c}
 * // arr = [10, 20, 30, 40, 50]
 * uint64_tensor_expect_t s = slice_uint64_tensor_array(arr, 1u, 4u, NULL);
 * if (!s.has_value) { fprintf(stderr, "%s\n", error_to_string(s.u.error)); }
 * uint64_tensor_t* sl = s.u.value;   // sl = [20, 30, 40]
...
/**
 * @brief Reverse the populated elements of a uint64_t tensor in place.
 *
 ...
 * @code
 * // arr = [1, 2, 3, 4, 5]
 * error_code_t err = reverse_uint64_tensor(arr);
 * // arr = [5, 4, 3, 2, 1]
 * @endcode
 */
static inline error_code_t reverse_uint64_tensor(uint64_tensor_t* t) {
    if (t == NULL) return NULL_POINTER;
...
/**
 * @brief Sort the populated elements of a uint64_t tensor in place.
 *
 ...
 * fallback for partitions of fewer than 10 elements. The uint64_t
 ...
 * @code
 * // arr = [3, 1, 4, 1, 5, 9, 2, 6]
 * error_code_t err = sort_uint64_tensor(arr, FORWARD);
 * // arr = [1, 1, 2, 3, 4, 5, 6, 9]
 *
 * err = sort_uint64_tensor(arr, REVERSE);
 * // arr = [9, 6, 5, 4, 3, 2, 1, 1]
 * @endcode
 */
static inline error_code_t sort_uint64_tensor(uint64_tensor_t* t,
                                             direction_t dir) {
    if (t == NULL) return NULL_POINTER;
    return sort_tensor(t->base, uint64_cmp, dir);
}
// -------------------------------------------------------------------------------- 

/**
 * @brief Search a uint64_t tensor for the first occurrence of a value.
 *
 ...
 *
 * size_t idx = 0u;
 * error_code_t err = find_uint64_tensor_value(arr, &idx, 200u);
 * // err == NO_ERROR, idx == 1  (first occurrence)
 *
 * err = find_uint64_tensor_value(arr, &idx, 999u);
...
/**
 * @brief Binary search a sorted uint64_t tensor for a target value.
 *
 ...
 * data is unsorted.  Use sort_uint64_tensor before calling if the
 ...
 * data[*index] == value.  Use uint64_tensor_lsearch if the first
 ...
 * push_back_uint64_array(arr, 40u);
 * push_back_uint64_array(arr, 20u);
 * push_back_uint64_array(arr, 30u);
 *
 * // Sort before searching
 * sort_uint64_tensor(arr, FORWARD);
 * // arr = [10, 20, 30, 40]
 *
 * size_t idx = 0u;
 * error_code_t err = uint64_tensor_bsearch(arr, &idx, 30u);
 * // err == NO_ERROR, idx == 2
 *
 * err = uint64_tensor_bsearch(arr, &idx, 25u);
 * // err == NOT_FOUND, idx unchanged
 *
 * return_uint64_tensor(arr);
 * @endcode
 ...
 *
 * @code{.c}
 * push_back_uint64_tensor(arr, 0u);   // arr = [0]
 * set_uint64_tensor_index(arr, 0u, 99u);   // arr = [99]
 * @endcode
 */
static inline error_code_t set_uint64_tensor_index(uint64_tensor_t* t,
                                                  size_t index,
                                                  uint64_t data) {
    if (t == NULL) return NULL_POINTER;
    return set_tensor_index(t->base, index, &data, UINT64_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 *
 * @code{.c}
 * // For a 3x3 uint64 matrix initialised via init_tensor
 * size_t idx[] = { 1u, 2u };
 * set_uint64_tensor_nd_index(mat, idx, 42u);   // mat[1][2] = 42
 * @endcode
 */
static inline error_code_t set_uint64_tensor_nd_index(uint64_tensor_t* t,
                                                     const size_t* idx,
                                                     uint64_t data) {
    if (t == NULL) return NULL_POINTER;
    return set_tensor_nd_index(t->base, idx, &data, UINT64_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 * @code{.c}
 * push_back_uint64_tensor(arr, 10u);
 * push_back_uint64_tensor(arr, 20u);
 * // arr = [10, 20], len == 2
 * @endcode
 */
static inline error_code_t push_back_uint64_array(uint64_tensor_t* t,
                                                 uint64_t data) {
    if (t == NULL) return NULL_POINTER;
    return push_back_tensor(t->base, &data, UINT64_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 * @code{.c}
 * push_back_uint64_tensor(arr, 10u);
 * push_front_uint64_tensor(arr, 99u);
 * // arr = [99, 10]
 * @endcode
 */
static inline error_code_t push_front_uint64_array(uint64_tensor_t* t,
                                                  uint64_t data) {
    if (t == NULL) return NULL_POINTER;
    return push_front_tensor(t->base, &data, UINT64_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 * @code{.c}
 * // arr = [10, 20, 30]
 * push_at_uint64_tensor(arr, 99u, 1u);
 * // arr = [10, 99, 20, 30]
 * @endcode
 */
static inline error_code_t push_at_uint64_array(uint64_tensor_t* t,
                                               uint64_t data,
                                               size_t index) {
    if (t == NULL) return NULL_POINTER;
    return push_at_tensor(t->base, &data, index, UINT64_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 * // arr = [10, 20, 30]
 * uint64_t val;
 * pop_back_uint64_tensor(arr, &val);   // val == 30, arr = [10, 20]
 * pop_back_uint64_tensor(arr, NULL);   // discard, arr = [10]
 * @endcode
 */
static inline error_code_t pop_back_uint64_array(uint64_tensor_t* t,
                                                uint64_t* out) {
    if (t == NULL) return NULL_POINTER;
    return pop_back_tensor(t->base, out, UINT64_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 * // arr = [10, 20, 30]
 * uint64_t val;
 * pop_front_uint64_tensor(arr, &val);   // val == 10, arr = [20, 30]
 * @endcode
 */
static inline error_code_t pop_front_uint64_array(uint64_tensor_t* t,
                                                 uint64_t* out) {
    if (t == NULL) return NULL_POINTER;
    return pop_front_tensor(t->base, out, UINT64_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 * // arr = [10, 20, 30, 40]
 * uint64_t val;
 * pop_at_uint64_tensor(arr, &val, 2u);   // val == 30, arr = [10, 20, 40]
 * @endcode
 */
static inline error_code_t pop_at_uint64_array(uint64_tensor_t* t,
                                              uint64_t* out,
                                              size_t index) {
    if (t == NULL) return NULL_POINTER;
    return pop_at_tensor(t->base, out, index, UINT64_TYPE);
}
// ================================================================================ 
// ================================================================================ 
// RETRIEVE DATA
 ...
 * // arr = [10, 20, 30]
 * uint64_t val;
 * get_uint64_tensor_index(arr, 1u, &val);   // val == 20
 * @endcode
 */
static inline error_code_t get_uint64_tensor_index(const uint64_tensor_t* t,
                                                  size_t index,
                                                  uint64_t* out) {
    if (t == NULL || out == NULL) return NULL_POINTER;
    return get_tensor_index(t->base, index, out, UINT64_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 *
 * @code{.c}
 * // For a 3x3 uint64 matrix
 * size_t idx[] = { 1u, 2u };
 * uint64_t val;
 * get_uint64_tensor_nd_index(mat, idx, &val);   // val = mat[1][2]
 * @endcode
 */
static inline error_code_t get_uint64_tensor_nd_index(const uint64_tensor_t* t,
                                                     const size_t* idx,
                                                     uint64_t* out) {
    if (t == NULL || idx == NULL || out == NULL) return NULL_POINTER;
    return get_tensor_nd_index(t->base, idx, out, UINT64_TYPE);
}
// ================================================================================ 
// ================================================================================ 
#ifdef __cplusplus
