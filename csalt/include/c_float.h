...
/**
 * @brief Find the minimum element in a float tensor.
 *
 ...
 *
 * All ISAs use separate NaN tracking because IEEE 754 MINPS/minNum
 * semantics return the non-NaN operand when one input is NaN, which
 * would silently drop NaN from the accumulation.
 *
 * Works on both TENSOR_STRUCT and ARRAY_STRUCT modes — the scan always
 * covers exactly t->base->len elements.
 *
 * @param t      Pointer to the source float tensor. Must not be NULL.
 * @param value  Pointer to a float to receive the minimum value.
 ...
 * @code{.c}
 * float min_val;
 * error_code_t err = min_float_tensor(arr, &min_val);
 * if (err != NO_ERROR) {
 *     fprintf(stderr, "%s\n", error_to_string(err));
