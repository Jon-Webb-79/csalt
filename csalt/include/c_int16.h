 ...
 *
 * Scans all elements using a signed 16-bit comparison and returns the smallest
 ...
 * int16_array_contains. The array is not modified. The scan dispatches to the
 ...
 *
 * @return int16_expect_t with has_value true and u.value == the minimum
 ...
 *
 *     int16_expect_t r = int16_array_min(arr);
 ...
 *
 * Scans all elements using a signed 16-bit comparison and returns the largest
 ...
 *               contain at least one element.
 *
 * @return int16_expect_t with has_value true and u.value == the maximum
 *         element on success. On failure, has_value is false and u.error
 *         is one of:
 *         - NULL_POINTER  if array is NULL
 *         - EMPTY         if array->base.len == 0
 *
 * @code
 *     int16_expect_t r = int16_array_max(arr);
...
/**
 * @brief A dictionary that maps null-terminated C-string keys to int16_t values (range INT16_MIN..INT16_MAX).
 *
 * This is a typedef alias for @ref dict_t with the convention that
 * @c data_size is always @c sizeof(int16_t) and @c dtype is always
 * @c INT16_TYPE.  Do not use the raw @ref dict_t API directly on a
 * @c int16_dict_t — always go through the functions declared in this header
 ...
 *
 *     insert_int16_dict(d, "width",  1920u, a);
 *     insert_int16_dict(d, "height", 1080u, a);
 *     insert_int16_dict(d, "depth",    32u, a);
 *
 *     int16_t v;
 *     get_int16_dict_value(d, "width", &v);   // v == 1920
 *
 *     return_int16_dict(d);
 * @endcode
 */
typedef dict_t int16_dict_t;
 
/**
 * @brief Expected return type for @ref init_int16_dict and
 *        @ref copy_int16_dict.
 *
...
/**
 * @brief Typed iterator callback for @ref foreach_int16_dict.
 *
 ...
 * @param value     The int16_t value associated with @p key.
 * @param user_data Caller-supplied context pointer passed unchanged from
 *                  @ref foreach_int16_dict; may be NULL.
 *
 * @code
 ...
 * @endcode
 */
typedef void (*int16_dict_iter_fn)(const char* key,
                                    size_t      key_len,
                                    int16_t    value,
                                    void*       user_data);
...
/**
 * @brief Allocate and initialise a new int16_dict_t.
 *
 * The underlying @ref dict_t is created with @c data_size = sizeof(int16_t)
 * and @c dtype = INT16_TYPE.  The bucket count is rounded up to the next
 ...
 *                  must not be NULL.
 *
 * @return @ref int16_dict_expect_t with @c has_value true on success.
 ...
 *     allocator_vtable_t a = heap_allocator();
 *
 *     int16_dict_expect_t r = init_int16_dict(8, true, a);
 *     if (!r.has_value) {
 *         fprintf(stderr, "init failed: %d\n", r.u.error);
 *         return;
 *     }
 *     int16_dict_t* d = r.u.value;
 *
 *     insert_int16_dict(d, "port",  8080u, a);
 *     insert_int16_dict(d, "range", 1024u, a);
 *
 *     return_int16_dict(d);
 * @endcode
 */
...
/**
 * @brief Release all memory owned by a int16_dict_t.
 *
 ...
 *
 * @code
 *     int16_dict_t* d = init_int16_dict(8, true, a).u.value;
 *     insert_int16_dict(d, "x", 1000u, a);
 *     // ... use the dict ...
 *     return_int16_dict(d);   // d must not be used after this point
 * @endcode
 */
...
/**
 * @brief Insert a null-terminated string key with a int16_t value.
 *
 ...
 * returned.  To overwrite an existing value use @ref update_int16_dict.
 *
 ...
 *     int16_dict_t* d = init_int16_dict(8, true, a).u.value;
 *
 *     insert_int16_dict(d, "width",  1920u, a);   // OK
 *     insert_int16_dict(d, "height", 1080u, a);   // OK
 *
 *     // Duplicate key — returns INVALID_ARG, dict is unchanged
 *     error_code_t err = insert_int16_dict(d, "width", 800u, a);
...
/**
 * @brief Insert a bounded key (explicit length) with a int16_t value.
 *
 * Identical to @ref insert_int16_dict but the key length is supplied by the
 ...
 *     insert_int16_dict_n(d, buf, 5, 1920u, a);
 *
 *     int16_t v;
 *     get_int16_dict_value_n(d, buf, 5, &v);   // v == 1920
 * @endcode
 */
 ...
 *
 *     int16_t removed;
 *     error_code_t err = pop_int16_dict(d, "port", &removed);
 *     assert(err == NO_ERROR && removed == 8080u);
 *
 *     // Key is gone — next pop returns NOT_FOUND
 *     err = pop_int16_dict(d, "port", NULL);
 ...
 *
 * Identical to @ref pop_int16_dict but the key length is supplied explicitly.
 *
 * @param dict       Must not be NULL.
 ...
 *     insert_int16_dict_n(d, buf, 4, 8080u, a);   // key = "port"
 *
 *     int16_t v;
 *     pop_int16_dict_n(d, buf, 4, &v);             // removes "port", v == 8080
 * @endcode
 */
 ...
 * @ref insert_int16_dict.
 *
 * @param dict   Must not be NULL.
 * @param key    Null-terminated C-string key.  Must not be NULL.
 * @param value  New int16_t value to store.
 *
 * @return NO_ERROR, NULL_POINTER, or NOT_FOUND.
 *
 * @code
 ...
 *     insert_int16_dict(d, "port", 8080u, a);
 *
 *     update_int16_dict(d, "port", 443u);
 *
 *     int16_t v;
 *     get_int16_dict_value(d, "port", &v);
 *     assert(v == 443u);
 *
 *     return_int16_dict(d);
 * @endcode
 ...
 *
 * Identical to @ref update_int16_dict but the key length is supplied
 * explicitly.
 *
 * @param dict     Must not be NULL.
 * @param key      Pointer to the key bytes.  Must not be NULL.
 ...
 *     const char* buf = "port_http";
 *     insert_int16_dict_n(d, buf, 4, 8080u, a);   // key = "port"
 *     update_int16_dict_n(d, buf, 4, 443u);        // update "port" to 443
 * @endcode
 */
 ...
 *
 *     int16_t v;
 *     if (get_int16_dict_value(d, "width", &v) == NO_ERROR)
 *         printf("width = %u\n", (unsigned)v);
 *
 *     return_int16_dict(d);
 * @endcode
 ...
 *
 * Identical to @ref get_int16_dict_value but the key length is supplied
 * explicitly.
 *
 * @param dict       Must not be NULL.
 * @param key        Pointer to the key bytes.  Must not be NULL.
 ...
 * @code
 *     const char* buf = "width_max";
 *     insert_int16_dict_n(d, buf, 5, 1920u, a);   // key = "width"
 *
 *     int16_t v;
 *     get_int16_dict_value_n(d, buf, 5, &v);       // v == 1920
 * @endcode
 ...
 * @param key   Null-terminated C-string key.  Must not be NULL.
 *
 * @return Pointer to the stored int16_t on success, NULL if not found or on
 *         error.
 *
 * @code
 *     insert_int16_dict(d, "port", 8080u, a);
 *
 *     const int16_t* p = get_int16_dict_ptr(d, "port");
 *     if (p) printf("port = %u\n", (unsigned)*p);
 ...
 *        a bounded key.
 *
 * Identical to @ref get_int16_dict_ptr but the key length is supplied
 * explicitly.
 *
 * @param dict     Must not be NULL.
 * @param key      Pointer to the key bytes.  Must not be NULL.
 ...
 *     insert_int16_dict_n(d, buf, 4, 8080u, a);   // key = "port"
 *
 *     const int16_t* p = get_int16_dict_ptr_n(d, buf, 4);
 *     if (p) printf("port = %u\n", (unsigned)*p);
 * @endcode
 */
 ...
 *
 * @code
 *     insert_int16_dict(d, "active", 1u, a);
 *
 *     if (has_int16_dict_key(d, "active"))
 ...
 *
 * Identical to @ref has_int16_dict_key but the key length is supplied
 * explicitly.
 *
 * @param dict     Must not be NULL.
 * @param key      Pointer to the key bytes.  Must not be NULL.
 ...
 *     insert_int16_dict_n(d, buf, 6, 1u, a);   // key = "active"
 *     assert(has_int16_dict_key_n(d, buf, 6));
 * @endcode
 */
 ...
 *
 * @code
 *     insert_int16_dict(d, "a", 100u, a);
 *     insert_int16_dict(d, "b", 200u, a);
 *     assert(int16_dict_hash_size(d) == 2);
 *
 *     clear_int16_dict(d);
 *     assert(int16_dict_hash_size(d) == 0);
 *
 *     // The dict is still usable after clear
 *     insert_int16_dict(d, "c", 300u, a);
 * @endcode
 */
...
/**
 * @brief Allocate a deep copy of a int16_dict_t.
 *
 ...
 *
 * @code
 *     int16_dict_t* orig = init_int16_dict(8, true, a).u.value;
 *     insert_int16_dict(orig, "x", 1000u, a);
 *     insert_int16_dict(orig, "y", 2000u, a);
 *
 *     int16_dict_expect_t cr = copy_int16_dict(orig, a);
 *     assert(cr.has_value);
 *     int16_dict_t* copy = cr.u.value;
 *
 *     // The copy is independent — mutating orig does not affect copy
 *     update_int16_dict(orig, "x", 9999u);
 *     int16_t v;
 *     get_int16_dict_value(copy, "x", &v);
 *     assert(v == 1000u);
 *
 *     return_int16_dict(copy);
...
/**
 * @brief Merge two int16_dict_t instances into a new dict.
 *
 ...
 *
 * @code
 *     int16_dict_t* da = init_int16_dict(8, true, a).u.value;
 *     int16_dict_t* db = init_int16_dict(8, true, a).u.value;
 *
 *     insert_int16_dict(da, "shared", 100u, a);
 *     insert_int16_dict(da, "only_a",   1u, a);
 *     insert_int16_dict(db, "shared", 999u, a);
 *     insert_int16_dict(db, "only_b",   2u, a);
 *
 *     // Merge without overwrite — "shared" keeps da's value of 100
 *     int16_dict_expect_t mr = merge_int16_dict(da, db, false, a);
 *     int16_t v;
 *     get_int16_dict_value(mr.u.value, "shared", &v);
 *     assert(v == 100u);
 *
 ...
 * C-string pointer (into internal storage), its length, and the int16_t
...
// ================================================================================ 
// ================================================================================ 
// int16_matrix_t — type-safe int16 wrapper around matrix_t
//
// This section provides a int16-specialised interface to the generic matrix_t
// type.  int16_matrix_t is a typedef for matrix_t; all functions below fix
// the dtype to int16_TYPE and accept / return int16 values directly so that
// callers never need to pass a dtype_id_t or void* value.
//
// The wrapper covers every public function in c_matrix.h that either:
//   (a) takes a dtype_id_t argument (replaced with int16_TYPE), or
//   (b) takes or returns void* element pointers (replaced with int16 / int16*).
//
...
// int16_matrix_t* signature for consistency, but simply delegate.
// ================================================================================ 
// ================================================================================ 

typedef matrix_t int16_matrix_t;

typedef struct {
    bool has_value;
    union {
        int16_matrix_t* value;
        error_code_t    error;
    } u;
} int16_matrix_expect_t;
...
/**
 * @brief Predicate function to determine semantic zero for int16 values.
 *
 ...
 *
 * - convert_int16_matrix_zero()
 *
 * @param value The int16 value to evaluate.
 *
 ...
 * @code{.c}
 * bool zero_or_neg_one(int16 v) {
 *     return (v == 0) || (v == 3);
 * }
 * @endcode
 */
typedef bool (*int16_zero_fn)(int16_t value);
...
/**
 * @brief Initialize a dense int16 matrix with zero-initialized storage.
 *
 * Creates a dense matrix of shape @p rows by @p cols with the dtype fixed to
 * int16_TYPE. All elements are initialised to zero.
 *
 ...
 *
 * @return int16_matrix_expect_t with has_value true on success.
 *
 * @code{.c}
 *     allocator_vtable_t a = heap_allocator();
 ...
 * @endcode
 */
int16_matrix_expect_t init_int16_dense_matrix(size_t             rows,
                                              size_t             cols,
                                              allocator_vtable_t alloc_v);
...
/**
 * @brief Initialize an empty COO sparse int16 matrix.
 *
 * Creates a coordinate-list sparse matrix with storage for up to @p capacity
 * entries. The dtype is fixed to int16_TYPE.
 *
 * @param rows      Number of matrix rows. Must be > 0.
 * @param cols      Number of matrix columns. Must be > 0.
 ...
 *     int16_matrix_t* m = r.u.value;
 *
 *     push_back_int16_coo_matrix(m, 0, 5, 3);
 *     push_back_int16_coo_matrix(m, 42, 99, 7);
 *
 *     return_int16_matrix(m);
 * @endcode
 */
int16_matrix_expect_t init_int16_coo_matrix(size_t             rows,
                                            size_t             cols,
                                            size_t             capacity,
                                            bool               growth,
                                            allocator_vtable_t alloc_v);
...
/**
 * @brief Read a int16 element at the given row and column.
 *
 ...
 * @param out  Receives the int16 value. Must not be NULL.
 *
 * @return NO_ERROR on success, or NULL_POINTER / INVALID_ARG / OUT_OF_BOUNDS.
 *
 * @code
 *     int16_t v = 0;
 *     get_int16_matrix(m, 1, 2, &v);
 * @endcode
 */
error_code_t get_int16_matrix(const int16_matrix_t* mat,
                              size_t                row,
                              size_t                col,
                              int16_t*              out);
...
/**
 * @brief Write a int16 element at the given row and column.
 *
 ...
 *
 * @code
 *     set_int16_matrix(m, 0, 0, 42u);
 * @endcode
 */
error_code_t set_int16_matrix(int16_matrix_t* mat,
                              size_t          row,
                              size_t          col,
                              int16_t         value);
...
/**
 * @brief Reserve additional entry capacity for a int16 COO matrix.
 *
 ...
 * @return NO_ERROR on success, or an appropriate error_code_t.
 */
error_code_t reserve_int16_coo_matrix(int16_matrix_t* mat,
                                      size_t          capacity);

// -------------------------------------------------------------------------------- 

/**
 * @brief Append or overwrite a int16 entry in a COO matrix.
 *
 ...
 * @param row    Zero-based row index.
 * @param col    Zero-based column index.
 * @param value  The int16 value to insert.
 *
 * @return NO_ERROR on success, or an appropriate error_code_t.
 *
 * @code
 ...
 * @endcode
 */
error_code_t push_back_int16_coo_matrix(int16_matrix_t* mat,
                                        size_t          row,
                                        size_t          col,
                                        int16_t         value);
 ...
 * @return NO_ERROR on success, or an appropriate error_code_t.
 */
error_code_t sort_int16_coo_matrix(int16_matrix_t* mat);
...
/**
 * @brief Clear a int16 matrix, preserving its allocated storage.
 *
 ...
 * @return NO_ERROR on success, or an appropriate error_code_t.
 */
error_code_t clear_int16_matrix(int16_matrix_t* mat);

// -------------------------------------------------------------------------------- 

/**
 * @brief Deep-copy a int16 matrix.
 *
 ...
 * @return int16_matrix_expect_t with has_value true on success.
 */
int16_matrix_expect_t copy_int16_matrix(const int16_matrix_t* src,
                                      allocator_vtable_t    alloc_v);

// -------------------------------------------------------------------------------- 

/**
 * @brief Convert a int16 matrix to a different storage format.
 *
 ...
 *
 * @code
 *     int16_matrix_expect_t csr =
 *         convert_int16_matrix(dense_m, CSR_MATRIX, a);
 * @endcode
 */
int16_matrix_expect_t convert_int16_matrix(const int16_matrix_t* src,
 ...
 * @return int16_matrix_expect_t with has_value true on success.
 */
int16_matrix_expect_t transpose_int16_matrix(const int16_matrix_t* src,
                                           allocator_vtable_t    alloc_v);
...
/**
 * @brief Fill a int16 matrix with a single repeated value.
 *
 * Nonzero fill is supported only for dense matrices. Supplying 0.0f resets
 * the matrix through clear_int16_matrix.
 *
 * @param mat    Matrix to fill.
 * @param value  The int16 fill value.
 *
 * @return NO_ERROR on success, or an appropriate error_code_t.
 *
 * @code
 *     fill_int16_matrix(m, 1);   // every element becomes 1.0
 *     fill_int16_matrix(m, 0);   // equivalent to clear
 * @endcode
 */
error_code_t fill_int16_matrix(int16_matrix_t* mat,
                              int16_t         value);
// ================================================================================
...
/**
 * @brief Test whether two int16 matrices have identical shape.
 */
bool int16_matrix_has_same_shape(const int16_matrix_t* a,
                                const int16_matrix_t* b);

// -------------------------------------------------------------------------------- 

/**
 * @brief Test whether a int16 matrix is square.
 */
bool int16_matrix_is_square(const int16_matrix_t* mat);

// -------------------------------------------------------------------------------- 

/**
 * @brief Test whether a int16 matrix uses a sparse representation.
 */
bool int16_matrix_is_sparse(const int16_matrix_t* mat);

// -------------------------------------------------------------------------------- 

/**
 * @brief Test whether a int16 matrix is logically all zeros.
 */
bool int16_matrix_is_zero(const int16_matrix_t* mat);

// -------------------------------------------------------------------------------- 

/**
 * @brief Compare two int16 matrices for numerical equality.
 *
 ...
 *
 * Equality is defined using standard int16ing-point comparison semantics:
 *
 * Matrices must:
 * - be non-NULL
 * - have the same dimensions (rows and columns)
 * - have dtype == int16_TYPE
 *
 ...
 *       comparisons), use :c:func:`int16_matrix_equal_cmp`.
 *
 * @code{.c}
 * allocator_vtable_t alloc = heap_allocator();
 *
 * int16_matrix_expect_t r1 = init_int16_dense_matrix(2, 2, alloc);
 * int16_matrix_expect_t r2 = init_int16_dense_matrix(2, 2, alloc);
 *
 * int16_matrix_t* a = r1.u.value;
 * int16_matrix_t* b = r2.u.value;
 *
 * set_int16_matrix(a, 0, 0, 4);
 * set_int16_matrix(b, 0, 0,  0);
 *
 * assert_true(int16_matrix_equal(a, b)); // false
 *
 ...
 *
 * int16_matrix_expect_t dense_r = init_int16_dense_matrix(2, 2, alloc);
 * int16_matrix_t* dense = dense_r.u.value;
 *
 * set_int16_matrix(dense, 0, 1, 3);
 *
 * int16_matrix_expect_t csr_r =
 *     convert_int16_matrix(dense, CSR_MATRIX, alloc);
 *
 * int16_matrix_t* csr = csr_r.u.value;
 *
 * // Same logical values → equal
 * assert_true(int16_matrix_equal(dense, csr)); // true
 *
 * return_int16_matrix(csr);
 * return_int16_matrix(dense);
 * @endcode
 */
bool int16_matrix_equal(const int16_matrix_t* a,
                       const int16_matrix_t* b);
// -------------------------------------------------------------------------------- 

/**
 * @brief Test whether two int16 matrices are compatible for addition.
 *
 * Requires equal shape and equal dtype.
 */
bool int16_matrix_is_add_compatible(const int16_matrix_t* a,
                                   const int16_matrix_t* b);

// -------------------------------------------------------------------------------- 

/**
 * @brief Test whether two int16 matrices are compatible for multiplication.
 *
 * Requires a->cols == b->rows and equal dtype.
 */
bool int16_matrix_is_multiply_compatible(const int16_matrix_t* a,
                                        const int16_matrix_t* b);
...
/**
 * @brief Swap two rows of a int16 matrix in place.
 *
 ...
 * @return NO_ERROR on success, or an appropriate error_code_t.
 */
error_code_t swap_int16_matrix_rows(int16_matrix_t* mat,
                                   size_t          r1,
                                   size_t          r2);
...
/**
 * @brief Swap two columns of a int16 matrix in place.
 *
 * Dense and COO matrices are supported. CSR/CSC return OPERATION_UNAVAILABLE.
 *
 * @param mat  Matrix to modify.
 ...
 * @return NO_ERROR on success, or an appropriate error_code_t.
 */
error_code_t swap_int16_matrix_cols(int16_matrix_t* mat,
                                   size_t          c1,
                                   size_t          c2);
...
/**
 * @brief Initialize a dense int16 identity matrix.
 *
 ...
 *
 * @code
 *     int16_matrix_expect_t r = init_int16_identity_matrix(3, a);
 *     // 3x3 identity matrix.
 * @endcode
 */
int16_matrix_expect_t init_int16_identity_matrix(size_t             n,
                                               allocator_vtable_t alloc_v);

// -------------------------------------------------------------------------------- 

/**
 * @brief Initialize a dense int16 row vector (1 x length).
 *
 ...
 * @return int16_matrix_expect_t with has_value true on success.
 */
int16_matrix_expect_t init_int16_row_vector(size_t             length,
                                          allocator_vtable_t alloc_v);

// -------------------------------------------------------------------------------- 

/**
 * @brief Initialize a dense int16 column vector (length x 1).
 *
 ...
 * @return int16_matrix_expect_t with has_value true on success.
 */
int16_matrix_expect_t init_int16_col_vector(size_t             length,
                                          allocator_vtable_t alloc_v);
...
/**
 * @brief Test whether a int16 matrix has row-vector shape (1 x N).
 */
bool int16_matrix_is_row_vector(const int16_matrix_t* mat);

// -------------------------------------------------------------------------------- 

/**
 * @brief Test whether a int16 matrix has column-vector shape (N x 1).
 */
bool int16_matrix_is_col_vector(const int16_matrix_t* mat);

// -------------------------------------------------------------------------------- 

/**
 * @brief Test whether a int16 matrix has vector shape (row or column).
 */
bool int16_matrix_is_vector(const int16_matrix_t* mat);

// -------------------------------------------------------------------------------- 

/**
 * @brief Return the logical length of a vector-shaped int16 matrix.
 *
 * For row vectors returns cols; for column vectors returns rows.
 * Non-vector matrices return 0.
 */
size_t int16_matrix_vector_length(const int16_matrix_t* mat);
...
/**
 * @brief Convert a int16 matrix to a different storage format using an
 *        optional semantic zero-test.
 *
 * This function performs the same structural conversion as
 * :c:func:`convert_int16_matrix`, but allows the caller to define what
 * int16 values should be treated as zero during dense-to-sparse conversion.
 *
 ...
 *
 * @return int16_matrix_expect_t with has_value true on success, or
 ...
 *
 * @code{.c}
 * static bool zero_or_neg_one(int16 v) {
 *     return (v == 0) || (v == 3);
 * }
 *
 * int16_matrix_expect_t r =
 *     convert_int16_matrix_zero(dense_m, CSR_MATRIX, alloc, zero_or_neg_one);
 * @endcode
 *
 * @note This function is primarily intended for domain-specific sparse
 *       conversion rules where exact zero is not sufficient.
 */
int16_matrix_expect_t convert_int16_matrix_zero(const int16_matrix_t* src,
                                              matrix_format_t       target,
                                              allocator_vtable_t    alloc_v,
                                              int16_zero_fn         is_zero);
// -------------------------------------------------------------------------------- 

/**
 * @brief Print an int16 matrix in a human-readable format.
 *
 ...
 *
 * @param mat
 * Pointer to the source @ref int16_matrix_t to print. Must not be NULL.
 *
 * @param stream
 ...
 *         - any error returned by get_int16_matrix during sparse traversal
 *
 * @note
 ...
 */
error_code_t print_int16_matrix(const int16_matrix_t* mat, FILE* stream);
// -------------------------------------------------------------------------------- 

/**
 * @brief Retrieve the minimum value from a int16 matrix.
 *
 * This function determines the minimum element in a matrix of type
 * `int16_matrix_t`. It delegates the index computation to the generic
 ...
 * @see matrix_min
 */
int16_expect_t int16_matrix_min(const int16_matrix_t* mat);
// -------------------------------------------------------------------------------- 

/**
 * @brief Retrieve the maximum value from a int16 matrix.
 *
