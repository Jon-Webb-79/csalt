...
/**
 * @brief A type-safe dynamic array of int32_t elements.
 *
 * Wraps array_t with the dtype fixed to INT32_TYPE at initialization.
 * All operations enforce INT32_TYPE at the call site so the caller never
...
/**
 * @brief Expected return type for int32_array_t initialization and copy/slice.
 */
typedef struct {
    bool has_value;
    union {
...
/**
 * @brief Initialize a new heap-allocated int32_array_t.
 *
 * Allocates both the array_t struct and its internal data buffer through
 * alloc_v. The element type is fixed to INT32_TYPE and the element size is
 ...
 * return_int32_array when the array is no longer needed.
 *
 ...
 *     int32_array_expect_t result = init_int32_array(32, false, alloc);
 *     if (!result.has_value) {
 *         fprintf(stderr, "init failed: %d\n", result.u.error);
 *         return;
 *     }
 *     int32_array_t* arr = result.u.value;
 *
...
/**
 * @brief Release all memory owned by a int32_array_t back to its allocator.
 *
 ...
 * @code{.c}
 *     allocator_vtable_t alloc = heap_allocator();
 *     int32_array_expect_t result = init_int32_array(8, false, alloc);
...
/**
 * @brief Append one int32_t to the end of the array.
 *
 ...
 * @code{.c}
 *     allocator_vtable_t alloc = heap_allocator();
 *     int32_array_expect_t result = init_int32_array(4, false, alloc);
 *     if (!result.has_value) { return; }
 *     int32_array_t* arr = result.u.value;
 *
 *     push_back_int32_array(arr, 1000);
 ...
 *
 *     int32_t out = 0;
 *     get_int32_array_index(arr, 2, &out);  // out == 3000
 *
 *     return_int32_array(arr);
 * @endcode
 */
...
/**
 * @brief Prepend one int32_t to the front of the array.
 *
 ...
 * is an O(n) operation — prefer push_back_int32_array for performance-sensitive
 * append-heavy workloads.
 *
 * @param array  Pointer to the target array. Must not be NULL.
 * @param value  The int32_t value to prepend.
 *
 * @return NO_ERROR on success, or one of:
 *         - NULL_POINTER      if array is NULL
 *         - CAPACITY_OVERFLOW if the array is full and growth is false
...
/**
 * @brief Insert one int32_t at an arbitrary position in the array.
 *
 ...
 * push_front_int32_array; inserting at index == len is equivalent to
 * push_back_int32_array. Both boundary cases are accepted without error.
 ...
 * @param value  The int32_t value to insert.
 *
 * @return NO_ERROR on success, or one of:
 *         - NULL_POINTER      if array is NULL
 ...
 *
 *     // Boundary cases:
 *     push_at_int32_array(arr, 0,  500);  // prepend -> [500, 1000, 2000, 3000]
 *     push_at_int32_array(arr, 4, 4000);  // append  -> [500, 1000, 2000, 3000, 4000]
 *
 *     return_int32_array(arr);
 * @endcode
 */
...
/**
 * @brief Retrieve the int32_t at a given index, copying it into out.
 *
 ...
 * @param out    Pointer to a int32_t that receives the copied value.
 *               Must not be NULL.
 *
 * @return NO_ERROR on success, or one of:
 ...
 *
 *     int32_t val = 0;
 *     get_int32_array_index(arr, 0, &val);  // val == 1000
 *     get_int32_array_index(arr, 1, &val);  // val == 2000
 *     get_int32_array_index(arr, 2, &val);  // val == 3000
 *
 *     // Reading past the end is an error regardless of buffer capacity.
 *     error_code_t err = get_int32_array_index(arr, 3, &val);
 ...
 *
 *     int32_t val = 0;
 *     pop_back_int32_array(arr, &val);  // val == 3000, len == 2
 *     pop_back_int32_array(arr, &val);  // val == 2000, len == 1
 *     pop_back_int32_array(arr, NULL);  // discard,    len == 0
 *
 *     // Popping from an empty array is an error.
 *     error_code_t err = pop_back_int32_array(arr, &val);
 ...
 * pop_back_int32_array for performance-sensitive drain loops. The array must
 ...
 *
 *     int32_t val = 0;
 *     pop_front_int32_array(arr, &val);  // val == 1000, arr is [2000, 3000]
 *     pop_front_int32_array(arr, &val);  // val == 2000, arr is [3000]
 *     pop_front_int32_array(arr, NULL);  // discard,    arr is []
 *
 *     return_int32_array(arr);
 * @endcode
 */
 ...
 * index 0 is equivalent to pop_front_int32_array; removing from index
 * len - 1 is equivalent to pop_back_int32_array. Both boundary cases are
 ...
 *
 *     int32_t val = 0;
 *     pop_any_int32_array(arr, &val, 1);  // val == 2000, arr is [1000, 3000, 4000]
 *     pop_any_int32_array(arr, &val, 0);  // val == 1000, arr is [3000, 4000]
 *     pop_any_int32_array(arr, &val, 1);  // val == 4000, arr is [3000]
 *
 *     return_int32_array(arr);
 * @endcode
 */
 ...
 * directly. Use clear_int32_array when you need a guaranteed clean slate.
 *
 * @param array  Pointer to the target array. Must not be NULL.
 *
 ...
 * reallocation. Use push_at_int32_array to insert a new element and shift
 * existing ones.
 *
 * @param array  Pointer to the target array. Must not be NULL.
 * @param index  Zero-based position to overwrite. Must be < array->base.len.
 * @param value  The replacement int32_t value.
 *
 * @return NO_ERROR on success, or one of:
 *         - NULL_POINTER  if array is NULL
 *         - OUT_OF_BOUNDS if index >= array->base.len
 ...
 *
 * Allocates a new int32_array_t struct and a new data buffer through alloc_v,
 ...
 * for calling return_int32_array on the returned array when it is no longer
 * needed.
 *
 ...
 *     int32_array_expect_t result = init_int32_array(8, false, alloc);
 *     if (!result.has_value) { return; }
 *     int32_array_t* src = result.u.value;
 *
 *     push_back_int32_array(src, 1000);
 *     push_back_int32_array(src, 2000);
 *     push_back_int32_array(src, 3000);
 *
 *     int32_array_expect_t copy_result = copy_int32_array(src, alloc);
 *     if (!copy_result.has_value) { return_int32_array(src); return; }
 *     int32_array_t* dst = copy_result.u.value;
 *     // src and dst both contain [1000, 2000, 3000] but own separate buffers.
 *
 *     set_int32_array_index(src, 0, 9999);
 ...
 *     push_back_int32_array(dst, 2000);
 *     push_back_int32_array(src, 3000);
 *     push_back_int32_array(src, 4000);
 *
 *     concat_int32_array(dst, src);
 ...
 *
 * Allocates a new int32_array_t and data buffer through alloc_v, then copies
 ...
 * is exclusive. The caller is responsible for calling return_int32_array on
 ...
 * stack depth O(log n). The comparator is a built-in signed 32-bit
 ...
 *     push_back_int32_array(arr, -2000);
 *     push_back_int32_array(arr,  1000);
 *     // arr contains [2000, -1000, 0, -2000, 1000].
 *
 *     sort_int32_array(arr, FORWARD);
 *     // arr is now [-2000, -1000, 0, 1000, 2000].
 *
 *     sort_int32_array(arr, REVERSE);
 *     // arr is now [2000, 1000, 0, -1000, -2000].
 *
 *     return_int32_array(arr);
 * @endcode
 ...
 * @param value  The int32_t value to search for.
 ...
 *
 *     size_expect_t r = int32_array_contains(arr, 3000, 0, 5);
 *     // r.has_value == true, r.u.value == 1  (first occurrence).
 *
 *     r = int32_array_contains(arr, 3000, 2, 5);
 *     // r.has_value == true, r.u.value == 3  (search starts after index 1).
 *
 *     r = int32_array_contains(arr, 9999, 0, 5);
 ...
 *
 * Uses a built-in signed 32-bit comparator — the caller does not need to
 ...
 *     size_expect_t r = int32_array_binary_search(arr, 1000, true);
 *     // r.has_value == true, r.u.value == 0 (first of two 100s).
 *
 *     // arr is now sorted; sort == false is safe for subsequent calls.
 *     r = int32_array_binary_search(arr, 3000, false);
 *     // r.has_value == true, r.u.value == 3.
 *
 *     r = int32_array_binary_search(arr, 9999, false);
 *     // r.has_value == false, r.u.error == NOT_FOUND.
 *
 *     return_int32_array(arr);
 * @endcode
 ...
 *
 * Uses a built-in signed 32-bit comparator. When sort is true the array
 ...
 *
 * @param array  Pointer to the array to search. Must not be NULL.
 * @param value  The int32_t value to bracket.
 * @param sort   If true, sort the array in place before searching.
 *               If false, the array must already be sorted ascending.
 *
 ...
 *     push_back_int32_array(arr,  1000);
 *     push_back_int32_array(arr,  3000);
 *     // arr contains [-3000, -1000, 1000, 3000] (already sorted).
 *
 *     // Value exists exactly — lower == upper, both at first occurrence.
 *     bracket_expect_t r = int32_array_binary_bracket(arr, -1000, false);
 *     // r.has_value == true, r.u.value.lower == 1, r.u.value.upper == 1.
 *
 *     // Value falls between elements.
 *     r = int32_array_binary_bracket(arr, 0, false);
 *     // r.has_value == true
 *     // r.u.value.lower == 2  (first element >= 0, which is 1000 at index 2)
 *     // r.u.value.upper == 1  (last  element <= 0, which is -1000 at index 1)
 *
 *     // Value outside range returns NOT_FOUND.
 *     r = int32_array_binary_bracket(arr, -5000, false);
 *     // r.has_value == false, r.u.error == NOT_FOUND.
 *
 *     return_int32_array(arr);
 * @endcode
 ...
 *
 * @code{.c}
 *     // int32_array_size(arr) == 0 after init.
 *     push_back_int32_array(arr, 1000);
 *     push_back_int32_array(arr, 2000);
 *     // int32_array_size(arr) == 2.
 *
 *     pop_back_int32_array(arr, NULL);
 *     // int32_array_size(arr) == 1.
 * @endcode
 */
 ...
 * is int32_array_alloc(arr) - int32_array_size(arr). The call is O(1) and
 ...
 *
 * @code{.c}
 *     // int32_array_alloc(arr) == 4 after init with capacity 4.
 *     push_back_int32_array(arr, 1000);
 *     push_back_int32_array(arr, 2000);
 *     push_back_int32_array(arr, 3000);
 *     push_back_int32_array(arr, 4000);
 ...
 *
 *     push_back_int32_array(arr, 5000);  // growth == true
 ...
 *
 * Reads array->base.data_size, which is fixed to 4 for all int32_array_t
 * instances — a int32_t is always four bytes. The call is O(1) and does not
 * modify the array.
 *
 * @param array  Pointer to the array. May be NULL.
 *
 ...
 *
 * @code{.c}
 *     size_t elem_bytes = int32_array_data_size(arr);  // always 4
 *
 *     // Total buffer size in bytes:
 *     size_t total_bytes = int32_array_alloc(arr) * int32_array_data_size(arr);
 * @endcode
 */
 ...
 *
 * @code{.c}
 *     // is_int32_array_empty(arr) == true immediately after init.
 *     push_back_int32_array(arr, 1000);
 *     // is_int32_array_empty(arr) == false.
 *
 *     int32_t val = 0;
 *     while (!is_int32_array_empty(arr))
 *         pop_back_int32_array(arr, &val);
 *     // is_int32_array_empty(arr) == true again.
 * @endcode
 */
 ...
 *     push_back_int32_array(arr, 1000);
 *     push_back_int32_array(arr, 2000);
 *     // is_int32_array_full(arr) == false (capacity 3, one slot remains).
 *
 *     push_back_int32_array(arr, 3000);
 *     // is_int32_array_full(arr) == true, size == alloc == 3.
 * @endcode
 */
 ...
 *     // arr contains [1000, 2000, 3000], len == 3, alloc == 8.
 *
 *     const int32_t* p = (const int32_t*)arr->base.data + 1;
 *
 *     is_int32_array_ptr(arr, p);                               // true
 *     is_int32_array_ptr(arr, (const int32_t*)arr->base.data); // true  — first
 *     is_int32_array_ptr(arr, (const int32_t*)arr->base.data + 2); // true — last
 *     is_int32_array_ptr(arr, (const int32_t*)arr->base.data + 3); // false — spare
 *     is_int32_array_ptr(arr, NULL);                            // false
 *     is_int32_array_ptr(NULL, p);                              // false
 * @endcode
 */
 ...
 *
 * Scans all elements using a signed 32-bit comparison and returns the smallest
 * value found. When two or more elements share the minimum value the value of
 * the first occurrence is returned, consistent with the behaviour of
 * int32_array_contains. The array is not modified. The scan dispatches to the
 * best SIMD horizontal-reduction path available at compile time and falls back
 * to a scalar loop on unsupported targets.
 *
 * @param array  Pointer to the array to scan. Must not be NULL and must
 *               contain at least one element.
 *
 * @return int32_expect_t with has_value true and u.value == the minimum
 ...
 *     int32_array_t* arr = result.u.value;
 *
 *     push_back_int32_array(arr,  70000);
 *     push_back_int32_array(arr, -10000);
 *     push_back_int32_array(arr,  40000);
 *     // arr contains [70000, -10000, 40000].
 *
 *     int32_expect_t r = int32_array_min(arr);
 ...
 *
 * Scans all elements using a signed 32-bit comparison and returns the largest
 * value found. When two or more elements share the maximum value the value of
 * the first occurrence is returned, consistent with the behaviour of
 * int32_array_contains. The array is not modified. The scan dispatches to the
 * best SIMD horizontal-reduction path available at compile time and falls back
 * to a scalar loop on unsupported targets.
 ...
 *               contain at least one element.
 *
 * @return int32_expect_t with has_value true and u.value == the maximum
 *         element on success. On failure, has_value is false and u.error
 *         is one of:
 *         - NULL_POINTER  if array is NULL
 *         - EMPTY         if array->base.len == 0
 *
 * @code
 *     int32_expect_t r = int32_array_max(arr);
...
/**
 * @brief A dictionary that maps null-terminated C-string keys to int32_t values.
 *
 * This is a typedef alias for @ref dict_t with the convention that
 * @c data_size is always @c sizeof(int32_t) (4 bytes) and @c dtype is
 * always @c INT32_TYPE.  Do not use the raw @ref dict_t API directly on a
 * @c int32_dict_t — always go through the functions declared in this header
 ...
 *
 *     insert_int32_dict(d, "width",  1920u, a);
 *     insert_int32_dict(d, "height", 1080u, a);
 *     insert_int32_dict(d, "depth",    32u, a);
 *
 *     int32_t v;
 *     get_int32_dict_value(d, "width", &v);   // v == 1920
 *
 *     return_int32_dict(d);
 * @endcode
 */
...
/**
 * @brief Expected return type for @ref init_int32_dict and
 *        @ref copy_int32_dict.
 *
...
/**
 * @brief Typed iterator callback for @ref foreach_int32_dict.
 *
 ...
 * @param value     The int32_t value associated with @p key.
 * @param user_data Caller-supplied context pointer passed unchanged from
 *                  @ref foreach_int32_dict; may be NULL.
 *
 * @code
 ...
 * @endcode
 */
typedef void (*int32_dict_iter_fn)(const char* key,
                                    size_t      key_len,
                                    int32_t    value,
                                    void*       user_data);
...
/**
 * @brief Allocate and initialise a new int32_dict_t.
 *
 * The underlying @ref dict_t is created with @c data_size = sizeof(int32_t)
 * and @c dtype = INT32_TYPE.  The bucket count is rounded up to the next
 ...
 *                  must not be NULL.
 *
 * @return @ref int32_dict_expect_t with @c has_value true on success.
 ...
 *     allocator_vtable_t a = heap_allocator();
 *
 *     int32_dict_expect_t r = init_int32_dict(8, true, a);
 *     if (!r.has_value) {
 *         fprintf(stderr, "init failed: %d\n", r.u.error);
 *         return;
 *     }
 *     int32_dict_t* d = r.u.value;
 *
 *     insert_int32_dict(d, "port",  8080u, a);
 *     insert_int32_dict(d, "range", 1024u, a);
 *
 *     return_int32_dict(d);
 * @endcode
 */
...
/**
 * @brief Release all memory owned by a int32_dict_t.
 *
 ...
 *
 * @code
 *     int32_dict_t* d = init_int32_dict(8, true, a).u.value;
 *     insert_int32_dict(d, "x", 1000u, a);
 *     // ... use the dict ...
 *     return_int32_dict(d);   // d must not be used after this point
 * @endcode
 */
...
/**
 * @brief Insert a null-terminated string key with a int32_t value.
 *
 ...
 * returned.  To overwrite an existing value use @ref update_int32_dict.
 *
 ...
 *     int32_dict_t* d = init_int32_dict(8, true, a).u.value;
 *
 *     insert_int32_dict(d, "width",  1920u, a);   // OK
 *     insert_int32_dict(d, "height", 1080u, a);   // OK
 *
 *     // Duplicate key — returns INVALID_ARG, dict is unchanged
 *     error_code_t err = insert_int32_dict(d, "width", 800u, a);
...
/**
 * @brief Insert a bounded key (explicit length) with a int32_t value.
 *
 * Identical to @ref insert_int32_dict but the key length is supplied by the
 ...
 *     insert_int32_dict_n(d, buf, 5, 1920u, a);
 *
 *     int32_t v;
 *     get_int32_dict_value_n(d, buf, 5, &v);   // v == 1920
 * @endcode
 */
 ...
 *
 *     int32_t removed;
 *     error_code_t err = pop_int32_dict(d, "port", &removed);
 *     assert(err == NO_ERROR && removed == 8080u);
 *
 *     // Key is gone — next pop returns NOT_FOUND
 *     err = pop_int32_dict(d, "port", NULL);
 ...
 *
 * Identical to @ref pop_int32_dict but the key length is supplied explicitly.
 *
 * @param dict       Must not be NULL.
 ...
 *     insert_int32_dict_n(d, buf, 4, 8080u, a);   // key = "port"
 *
 *     int32_t v;
 *     pop_int32_dict_n(d, buf, 4, &v);             // removes "port", v == 8080
 * @endcode
 */
 ...
 * @ref insert_int32_dict.
 *
 * @param dict   Must not be NULL.
 * @param key    Null-terminated C-string key.  Must not be NULL.
 * @param value  New int32_t value to store.
 *
 * @return NO_ERROR, NULL_POINTER, or NOT_FOUND.
 *
 * @code
 ...
 *     insert_int32_dict(d, "port", 8080u, a);
 *
 *     update_int32_dict(d, "port", 443u);
 *
 *     int32_t v;
 *     get_int32_dict_value(d, "port", &v);
 *     assert(v == 443u);
 *
 *     return_int32_dict(d);
 * @endcode
 ...
 *
 * Identical to @ref update_int32_dict but the key length is supplied
 * explicitly.
 *
 * @param dict     Must not be NULL.
 * @param key      Pointer to the key bytes.  Must not be NULL.
 ...
 *     const char* buf = "port_http";
 *     insert_int32_dict_n(d, buf, 4, 8080u, a);   // key = "port"
 *     update_int32_dict_n(d, buf, 4, 443u);        // update "port" to 443
 * @endcode
 */
 ...
 *
 *     int32_t v;
 *     if (get_int32_dict_value(d, "width", &v) == NO_ERROR)
 *         printf("width = %u\n", (unsigned)v);
 *
 *     return_int32_dict(d);
 * @endcode
 ...
 *
 * Identical to @ref get_int32_dict_value but the key length is supplied
 * explicitly.
 *
 * @param dict       Must not be NULL.
 * @param key        Pointer to the key bytes.  Must not be NULL.
 ...
 * @code
 *     const char* buf = "width_max";
 *     insert_int32_dict_n(d, buf, 5, 1920u, a);   // key = "width"
 *
 *     int32_t v;
 *     get_int32_dict_value_n(d, buf, 5, &v);       // v == 1920
 * @endcode
 ...
 * @param key   Null-terminated C-string key.  Must not be NULL.
 *
 * @return Pointer to the stored int32_t on success, NULL if not found or on
 *         error.
 *
 * @code
 *     insert_int32_dict(d, "port", 8080u, a);
 *
 *     const int32_t* p = get_int32_dict_ptr(d, "port");
 *     if (p) printf("port = %u\n", (unsigned)*p);
 ...
 *        a bounded key.
 *
 * Identical to @ref get_int32_dict_ptr but the key length is supplied
 * explicitly.
 *
 * @param dict     Must not be NULL.
 * @param key      Pointer to the key bytes.  Must not be NULL.
 ...
 *     insert_int32_dict_n(d, buf, 4, 8080u, a);   // key = "port"
 *
 *     const int32_t* p = get_int32_dict_ptr_n(d, buf, 4);
 *     if (p) printf("port = %u\n", (unsigned)*p);
 * @endcode
 */
 ...
 *
 * @code
 *     insert_int32_dict(d, "active", 1u, a);
 *
 *     if (has_int32_dict_key(d, "active"))
 ...
 *
 * Identical to @ref has_int32_dict_key but the key length is supplied
 * explicitly.
 *
 * @param dict     Must not be NULL.
 * @param key      Pointer to the key bytes.  Must not be NULL.
 ...
 *     insert_int32_dict_n(d, buf, 6, 1u, a);   // key = "active"
 *     assert(has_int32_dict_key_n(d, buf, 6));
 * @endcode
 */
 ...
 *
 * @code
 *     insert_int32_dict(d, "a", 100u, a);
 *     insert_int32_dict(d, "b", 200u, a);
 *     assert(int32_dict_hash_size(d) == 2);
 *
 *     clear_int32_dict(d);
 *     assert(int32_dict_hash_size(d) == 0);
 *
 *     // The dict is still usable after clear
 *     insert_int32_dict(d, "c", 300u, a);
 * @endcode
 */
...
/**
 * @brief Allocate a deep copy of a int32_dict_t.
 *
 ...
 *
 * @code
 *     int32_dict_t* orig = init_int32_dict(8, true, a).u.value;
 *     insert_int32_dict(orig, "x", 1000u, a);
 *     insert_int32_dict(orig, "y", 2000u, a);
 *
 *     int32_dict_expect_t cr = copy_int32_dict(orig, a);
 *     assert(cr.has_value);
 *     int32_dict_t* copy = cr.u.value;
 *
 *     // The copy is independent — mutating orig does not affect copy
 *     update_int32_dict(orig, "x", 9999u);
 *     int32_t v;
 *     get_int32_dict_value(copy, "x", &v);
 *     assert(v == 1000u);
 *
 *     return_int32_dict(copy);
...
/**
 * @brief Merge two int32_dict_t instances into a new dict.
 *
 ...
 *
 * @code
 *     int32_dict_t* da = init_int32_dict(8, true, a).u.value;
 *     int32_dict_t* db = init_int32_dict(8, true, a).u.value;
 *
 *     insert_int32_dict(da, "shared", 100u, a);
 *     insert_int32_dict(da, "only_a",   1u, a);
 *     insert_int32_dict(db, "shared", 999u, a);
 *     insert_int32_dict(db, "only_b",   2u, a);
 *
 *     // Merge without overwrite — "shared" keeps da's value of 100
 *     int32_dict_expect_t mr = merge_int32_dict(da, db, false, a);
 *     int32_t v;
 *     get_int32_dict_value(mr.u.value, "shared", &v);
 *     assert(v == 100u);
 *
 ...
 * C-string pointer (into internal storage), its length, and the int32_t
...
// ================================================================================ 
// ================================================================================ 
// int32_matrix_t — type-safe int32 wrapper around matrix_t
//
// This section provides a int32-specialised interface to the generic matrix_t
// type.  int32_matrix_t is a typedef for matrix_t; all functions below fix
// the dtype to int32_TYPE and accept / return int32 values directly so that
// callers never need to pass a dtype_id_t or void* value.
//
// The wrapper covers every public function in c_matrix.h that either:
//   (a) takes a dtype_id_t argument (replaced with int32_TYPE), or
//   (b) takes or returns void* element pointers (replaced with int32 / int32*).
//
...
// int32_matrix_t* signature for consistency, but simply delegate.
// ================================================================================ 
// ================================================================================ 

typedef matrix_t int32_matrix_t;

typedef struct {
    bool has_value;
    union {
        int32_matrix_t* value;
        error_code_t    error;
    } u;
} int32_matrix_expect_t;
...
/**
 * @brief Predicate function to determine semantic zero for int32 values.
 *
 ...
 *
 * @param value The int32 value to evaluate.
 *
 ...
 * @code{.c}
 * bool zero_or_neg_one(int32 v) {
 *     return (v == 0) || (v == 3);
 * }
 * @endcode
 */
typedef bool (*int32_zero_fn)(int32_t value);
...
/**
 * @brief Initialize a dense int32 matrix with zero-initialized storage.
 *
 * Creates a dense matrix of shape @p rows by @p cols with the dtype fixed to
 * int32_TYPE. All elements are initialised to zero.
 *
 ...
 *
 * @return int32_matrix_expect_t with has_value true on success.
 *
 * @code{.c}
 *     allocator_vtable_t a = heap_allocator();
...
/**
 * @brief Initialize an empty COO sparse int32 matrix.
 *
 * Creates a coordinate-list sparse matrix with storage for up to @p capacity
 * entries. The dtype is fixed to int32_TYPE.
 *
 * @param rows      Number of matrix rows. Must be > 0.
 * @param cols      Number of matrix columns. Must be > 0.
 ...
 *     int32_matrix_t* m = r.u.value;
 *
 *     push_back_int32_coo_matrix(m, 0, 5, 3);
 *     push_back_int32_coo_matrix(m, 42, 99, 7);
 *
 *     return_int32_matrix(m);
 * @endcode
 */
...
/**
 * @brief Release all storage owned by a int32 matrix.
 *
...
/**
 * @brief Read a int32 element at the given row and column.
 *
 ...
 * @param out  Receives the int32 value. Must not be NULL.
 *
 * @return NO_ERROR on success, or NULL_POINTER / INVALID_ARG / OUT_OF_BOUNDS.
 *
 * @code
 *     int32_t v = 0;
 *     get_int32_matrix(m, 1, 2, &v);
 * @endcode
 */
...
/**
 * @brief Write a int32 element at the given row and column.
 *
...
/**
 * @brief Reserve additional entry capacity for a int32 COO matrix.
 *
...
/**
 * @brief Append or overwrite a int32 entry in a COO matrix.
 *
 ...
 * @param row    Zero-based row index.
 * @param col    Zero-based column index.
 * @param value  The int32 value to insert.
 *
 * @return NO_ERROR on success, or an appropriate error_code_t.
 *
 * @code
...
/**
 * @brief Clear a int32 matrix, preserving its allocated storage.
 *
...
/**
 * @brief Deep-copy a int32 matrix.
 *
...
/**
 * @brief Convert a int32 matrix to a different storage format.
 *
 ...
 *
 * @code
 *     int32_matrix_expect_t csr =
 *         convert_int32_matrix(dense_m, CSR_MATRIX, a);
 * @endcode
 */
...
/**
 * @brief Compute the transpose of a int32 matrix.
 *
...
/**
 * @brief Fill a int32 matrix with a single repeated value.
 *
 * Nonzero fill is supported only for dense matrices. Supplying 0.0f resets
 * the matrix through clear_int32_matrix.
 *
 * @param mat    Matrix to fill.
 * @param value  The int32 fill value.
 *
 * @return NO_ERROR on success, or an appropriate error_code_t.
 *
 * @code
 *     fill_int32_matrix(m, 1);   // every element becomes 1.0
 *     fill_int32_matrix(m, 0);   // equivalent to clear
 * @endcode
 */
...
/**
 * @brief Test whether two int32 matrices have identical shape.
 */
...
/**
 * @brief Test whether a int32 matrix is square.
 */
...
/**
 * @brief Test whether a int32 matrix uses a sparse representation.
 */
...
/**
 * @brief Test whether a int32 matrix is logically all zeros.
 */
...
/**
 * @brief Compare two int32 matrices for numerical equality.
 *
 ...
 *
 * Equality is defined using standard int32ing-point comparison semantics:
 *
 * Matrices must:
 * - be non-NULL
 * - have the same dimensions (rows and columns)
 * - have dtype == int32_TYPE
 *
 ...
 *       comparisons), use :c:func:`int32_matrix_equal_cmp`.
 *
 * @code{.c}
 * allocator_vtable_t alloc = heap_allocator();
 *
 * int32_matrix_expect_t r1 = init_int32_dense_matrix(2, 2, alloc);
 * int32_matrix_expect_t r2 = init_int32_dense_matrix(2, 2, alloc);
 *
 * int32_matrix_t* a = r1.u.value;
 * int32_matrix_t* b = r2.u.value;
 *
 * set_int32_matrix(a, 0, 0, 4);
 * set_int32_matrix(b, 0, 0,  0);
 *
 * assert_true(int32_matrix_equal(a, b)); // false
 *
 ...
 *
 * int32_matrix_expect_t dense_r = init_int32_dense_matrix(2, 2, alloc);
 * int32_matrix_t* dense = dense_r.u.value;
 *
 * set_int32_matrix(dense, 0, 1, 3);
 *
 * int32_matrix_expect_t csr_r =
 *     convert_int32_matrix(dense, CSR_MATRIX, alloc);
 *
 * int32_matrix_t* csr = csr_r.u.value;
 *
 * // Same logical values → equal
 * assert_true(int32_matrix_equal(dense, csr)); // true
 *
...
/**
 * @brief Test whether two int32 matrices are compatible for addition.
 *
...
/**
 * @brief Test whether two int32 matrices are compatible for multiplication.
 *
...
/**
 * @brief Swap two rows of a int32 matrix in place.
 *
...
/**
 * @brief Swap two columns of a int32 matrix in place.
 *
 * Dense and COO matrices are supported. CSR/CSC return OPERATION_UNAVAILABLE.
 *
 * @param mat  Matrix to modify.
...
/**
 * @brief Initialize a dense int32 identity matrix.
 *
 ...
 *
 * @code
 *     int32_matrix_expect_t r = init_int32_identity_matrix(3, a);
 *     // 3x3 identity matrix.
 * @endcode
 */
...
/**
 * @brief Initialize a dense int32 row vector (1 x length).
 *
...
/**
 * @brief Initialize a dense int32 column vector (length x 1).
 *
...
/**
 * @brief Test whether a int32 matrix has row-vector shape (1 x N).
 */
...
/**
 * @brief Test whether a int32 matrix has column-vector shape (N x 1).
 */
...
/**
 * @brief Test whether a int32 matrix has vector shape (row or column).
 */
...
/**
 * @brief Return the logical length of a vector-shaped int32 matrix.
 *
...
/**
 * @brief Convert a int32 matrix to a different storage format using an
 *        optional semantic zero-test.
 *
 * This function performs the same structural conversion as
 * :c:func:`convert_int32_matrix`, but allows the caller to define what
 * int32 values should be treated as zero during dense-to-sparse conversion.
 *
 ...
 *
 * @return int32_matrix_expect_t with has_value true on success, or
 ...
 *
 * @code{.c}
 * static bool zero_or_neg_one(int32 v) {
 *     return (v == 0) || (v == 3);
 * }
 *
 * int32_matrix_expect_t r =
 *     convert_int32_matrix_zero(dense_m, CSR_MATRIX, alloc, zero_or_neg_one);
 * @endcode
 *
...
/**
 * @brief Print an int32 matrix in a human-readable format.
 *
 ...
 *         - NULL_POINTER
 *         - ILLEGAL_STATE
 *         - error from get_int32_matrix
 */
error_code_t print_int32_matrix(const int32_matrix_t* mat, FILE* stream);
// -------------------------------------------------------------------------------- 

/**
 * @brief Retrieve the minimum value from a int32 matrix.
 *
 * This function determines the minimum element in a matrix of type
 * `int32_matrix_t`. It delegates the index computation to the generic
...
/**
 * @brief Retrieve the maximum value from a int32 matrix.
 *
