...
/**
 * @brief Type-safe wrapper for a tensor containing int32_t elements.
 *
 ...
 * Use the typed wrapper functions (init_int32_array, push_back_int32_tensor,
 ...
 * int32_tensor_t* a = r.u.value;
 * @endcode
 */
typedef struct {
    bool has_value;
    union {
        int32_tensor_t* value;
        error_code_t    error;
    } u;
} int32_tensor_expect_t;
...
/**
 * @brief Initialise a fixed-shape N-dimensional tensor of int32_t elements.
 *
 * Allocates the underlying tensor_t via init_tensor and then allocates a
 * thin int32_tensor_t wrapper struct through the same allocator. All slots
 ...
 * return_int32_tensor when the tensor is no longer needed.
 *
 ...
 *
 * @return int32_tensor_expect_t with has_value true on success.
 *         On failure has_value is false and u.error is set.
 *
 * @code{.c}
 * // Create a 3x4 matrix of int32_t values
 * size_t shape[] = { 3u, 4u };
 * int32_tensor_expect_t r = init_int32_tensor(2u, shape, heap_allocator());
 * if (!r.has_value) { fprintf(stderr, "%s\n", error_to_string(r.u.error)); }
 * int32_tensor_t* mat = r.u.value;
 *
 * // Write to element (1, 2)
 * size_t idx[] = { 1u, 2u };
 * set_int32_tensor_nd_index(mat, idx, 42);
 *
 * // Read it back
 * int32_t val;
 * get_int32_tensor_nd_index(mat, idx, &val);   // val == 42
 *
...
/*
 * Allocator convention for int32_tensor_t functions:
 *
 * init_int32_array        — takes allocator_vtable_t by value. An allocator
 *                           is mandatory since there is no existing tensor to
 *                           inherit one from.
 *
 * copy_int32_tensor       — takes allocator_vtable_t* (pointer). Pass NULL to
 * slice_int32_tensor_array  inherit the allocator from the source tensor, or
 *                           a non-NULL pointer to use a different allocator.
 */
 
/**
 * @brief Initialise a dynamic 1-D array of int32_t elements.
 *
 * Allocates the underlying tensor_t via init_tensor_array and then
 * allocates a thin int32_tensor_t wrapper struct through the same
 * allocator. Both allocations must be returned by calling
 * return_int32_tensor when the array is no longer needed.
 *
 ...
 *
 * @code{.c}
 * int32_tensor_expect_t r = init_int32_array(16, true, heap_allocator());
 * if (!r.has_value) { fprintf(stderr, "%s\n", error_to_string(r.u.error)); }
 * int32_tensor_t* arr = r.u.value;
 * push_back_int32_tensor(arr, 42);
...
/**
 * @brief Return a int32_tensor_t and its underlying tensor to the allocator.
 *
 ...
 *
 * @code{.c}
 * int32_tensor_expect_t r = init_int32_array(8, false, heap_allocator());
 * int32_tensor_t* arr = r.u.value;
 * // ... use arr ...
 * return_int32_tensor(arr);
 ...
 * @endcode
 */
void return_int32_tensor(int32_tensor_t* t);
// -------------------------------------------------------------------------------- 

/**
 * @brief Create a deep copy of a int32_tensor_t.
 *
 * Copies the underlying tensor via copy_tensor then allocates a new
 * int32_tensor_t wrapper struct. Both allocations are returned by calling
 * return_int32_tensor on the returned pointer. On partial failure the
 ...
 * affect the other.
 *
 * @param src     Pointer to the source int32_tensor_t. Must not be NULL.
 ...
 * int32_tensor_expect_t c = copy_int32_tensor(arr, NULL);
 * if (!c.has_value) { fprintf(stderr, "%s\n", error_to_string(c.u.error)); }
 * int32_tensor_t* copy = c.u.value;
 * // ...
 * return_int32_tensor(copy);
 *
 * // Copy using a different allocator
 * allocator_vtable_t arena = arena_allocator(&my_arena);
 * int32_tensor_expect_t c2 = copy_int32_tensor(arr, &arena);
 * @endcode
 */
 ...
 *
 * @code{.c}
 * size_t n = int32_tensor_size(arr);   // e.g. 3 after three push_back calls
 * @endcode
 */
static inline size_t int32_tensor_size(const int32_tensor_t* t) {
    if (t == NULL) return 0u;
 ...
 *
 * @code{.c}
 * size_t cap = int32_tensor_alloc(arr);   // e.g. 16 after init_int32_array(16,...)
 * @endcode
 */
static inline size_t int32_tensor_alloc(const int32_tensor_t* t) {
    if (t == NULL) return 0u;
 ...
 *
 * Always returns sizeof(int32_t) == 1 for a correctly initialised
 * int32_tensor_t. Returns 0 if t is NULL.
 *
 * @code{.c}
 * size_t esz = int32_tensor_data_size(arr);   // 1
 * @endcode
 */
static inline size_t int32_tensor_data_size(const int32_tensor_t* t) {
    if (t == NULL) return 0u;
 ...
 *
 * Always returns INT32_TYPE for a correctly initialised int32_tensor_t.
 * Returns UNKNOWN_TYPE if t is NULL.
 *
 * @code{.c}
 * dtype_id_t id = int32_tensor_dtype(arr);   // INT32_TYPE
 * @endcode
 */
static inline dtype_id_t int32_tensor_dtype(const int32_tensor_t* t) {
 ...
 * int32_tensor_expect_t r = init_int32_array(8, false, heap_allocator());
 * int32_tensor_t* arr = r.u.value;
 * bool empty = is_int32_tensor_empty(arr);   // true before any push
 * push_back_int32_tensor(arr, 1);
 * empty = is_int32_tensor_empty(arr);         // false after push
 * return_int32_tensor(arr);
 * @endcode
 */
static inline bool is_int32_tensor_empty(const int32_tensor_t* t) {
 ...
 *
 * @code{.c}
 * int32_tensor_expect_t r = init_int32_array(2, false, heap_allocator());
 * int32_tensor_t* arr = r.u.value;
 * push_back_int32_tensor(arr, 1);
 * push_back_int32_tensor(arr, 2);
 * bool full = is_int32_tensor_full(arr);   // true — len == alloc == 2
 * return_int32_tensor(arr);
 * @endcode
 */
static inline bool is_int32_tensor_full(const int32_tensor_t* t) {
    if (t == NULL) return true;
 ...
 *
 * @code{.c}
 * int32_tensor_expect_t r = init_int32_array(4, false, heap_allocator());
 * int32_tensor_t* arr = r.u.value;
 * push_back_int32_tensor(arr, 10);
 * const int32_t* p = arr->base->data;
 * bool valid = is_int32_tensor_ptr(arr, p);   // true — points to element 0
 * return_int32_tensor(arr);
 * @endcode
 */
static inline bool is_int32_tensor_ptr(const int32_tensor_t* t, const int32_t* ptr) {
 ...
 *
 * Always returns 1 for a tensor initialised via init_int32_array.
 * Returns 0 if t is NULL.
 *
 * @code{.c}
 * uint8_t ndim = int32_tensor_ndim(arr);   // 1
 * @endcode
 */
static inline uint8_t int32_tensor_ndim(const int32_tensor_t* t) {
    if (t == NULL) return 0u;
 ...
 * int32_tensor_expect_t r = init_int32_array(8, false, heap_allocator());
 * int32_tensor_t* arr = r.u.value;
 * size_t d0 = int32_tensor_shape_dim(arr, 0u);   // 8
 * return_int32_tensor(arr);
 * @endcode
 */
static inline size_t int32_tensor_shape_dim(const int32_tensor_t* t, int32_t dim) {
    if (t == NULL) return 0u;
 ...
 * @code{.c}
 * size_t shape[1];
 * error_code_t err = int32_tensor_shape(arr, shape, 1u);
 * // shape[0] now holds the single dimension size
 * @endcode
 */
static inline error_code_t int32_tensor_shape(const int32_tensor_t* t, size_t* out, int32_t count) {
 ...
 *
 * @code{.c}
 * const size_t* sp = int32_tensor_shape_ptr(arr);
 * if (sp) printf("dim0 = %zu\n", sp[0]);
 * @endcode
 */
static inline const size_t* int32_tensor_shape_ptr(const int32_tensor_t* t) {
 ...
 *
 * Strides are in bytes. For a contiguous 1-D int32_t array,
 ...
 * @endcode
 */
static inline const size_t* int32_tensor_strides_ptr(const int32_tensor_t* t) {
    if (t == NULL) return NULL;
 ...
 * @code{.c}
 * char buf[32];
 * error_code_t err = int32_tensor_shape_str(arr, buf, sizeof(buf));
 * if (err == NO_ERROR) printf("shape = %s\n", buf);   // e.g. "(8)"
 * @endcode
 */
static inline error_code_t int32_tensor_shape_str(const int32_tensor_t* t, 
                                                  char* buf, 
                                                  size_t buf_len) {
 ...
 * push_back_int32_tensor(arr, 1);
 * push_back_int32_tensor(arr, 2);
 * clear_int32_tensor(arr);
 * size_t n = int32_tensor_size(arr);   // 0
 * @endcode
 */
static inline error_code_t clear_int32_tensor(int32_tensor_t* t) {
 ...
 *
 * @code{.c}
 * int32_tensor_expect_t r1 = init_int32_array(8, false, heap_allocator());
 * int32_tensor_expect_t r2 = init_int32_array(4, false, heap_allocator());
 * int32_tensor_t* dst = r1.u.value;
 * int32_tensor_t* src = r2.u.value;
 * push_back_int32_tensor(dst, 1);
 * push_back_int32_tensor(src, 2);
 * push_back_int32_tensor(src, 3);
 * concat_int32_tensor_array(dst, src);   // dst = [1, 2, 3]
 * return_int32_tensor(dst);
 * return_int32_tensor(src);
 * @endcode
 */
static inline error_code_t concat_int32_tensor_array(int32_tensor_t* dst,
                                                     const int32_tensor_t* src) {
    if (dst == NULL || src == NULL) return NULL_POINTER;
    return concat_tensor_array(dst->base, src->base, INT32_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 * The caller is responsible for calling return_int32_tensor on the result.
 *
 ...
 * @code{.c}
 * // arr = [10, 20, 30, 40, 50]
 * int32_tensor_expect_t s = slice_int32_tensor_array(arr, 1u, 4u, NULL);
 * if (!s.has_value) { fprintf(stderr, "%s\n", error_to_string(s.u.error)); }
 * int32_tensor_t* sl = s.u.value;   // sl = [20, 30, 40]
...
/**
 * @brief Reverse the populated elements of a int32_t tensor in place.
 *
 ...
 * @code
 * // arr = [1, 2, 3, 4, 5]
 * error_code_t err = reverse_int32_tensor(arr);
 * // arr = [5, 4, 3, 2, 1]
 * @endcode
 */
static inline error_code_t reverse_int32_tensor(int32_tensor_t* t) {
    if (t == NULL) return NULL_POINTER;
...
static int int32_cmp(const void* a, const void* b) {
    int32_t ua = *(const int32_t*)a;
    int32_t ub = *(const int32_t*)b;
    return (ua > ub) - (ua < ub);
}
// -------------------------------------------------------------------------------- 

/**
 * @brief Sort the populated elements of a int32_t tensor in place.
 *
 ...
 * fallback for partitions of fewer than 10 elements. The int32_t
 ...
 * @code
 * // arr = [3, 1, 4, 1, 5, 9, 2, 6]
 * error_code_t err = sort_int32_tensor(arr, FORWARD);
 * // arr = [1, 1, 2, 3, 4, 5, 6, 9]
 *
 * err = sort_int32_tensor(arr, REVERSE);
 * // arr = [9, 6, 5, 4, 3, 2, 1, 1]
 * @endcode
 */
static inline error_code_t sort_int32_tensor(int32_tensor_t* t,
                                             direction_t dir) {
    if (t == NULL) return NULL_POINTER;
    return sort_tensor(t->base, int32_cmp, dir);
}
// -------------------------------------------------------------------------------- 

/**
 * @brief Search a int32_t tensor for the first occurrence of a value.
 *
 ...
 *
 * size_t idx = 0u;
 * error_code_t err = find_int32_tensor_value(arr, &idx, 20u);
 * // err == NO_ERROR, idx == 1  (first occurrence)
 *
 * err = find_int32_tensor_value(arr, &idx, 99u);
...
/**
 * @brief Binary search a sorted int32_t tensor for a target value.
 *
 ...
 * data is unsorted.  Use sort_int32_tensor before calling if the
 ...
 * data[*index] == value.  Use int32_tensor_lsearch if the first
 ...
 * sort_int32_tensor(arr, FORWARD);
 * // arr = [10, 20, 30, 40]
 *
 * size_t idx = 0u;
 * error_code_t err = int32_tensor_bsearch(arr, &idx, 30);
 * // err == NO_ERROR, idx == 2
 *
 * err = int32_tensor_bsearch(arr, &idx, 25);
 * // err == NOT_FOUND, idx unchanged
 *
 * return_int32_tensor(arr);
 * @endcode
...
/**
 * @brief Bracketed binary search of a sorted int32_t tensor.
 *
 * Locates the pair of adjacent indices that bracket value in a sorted
 * ascending tensor.  Unlike int32_tensor_bsearch which returns NOT_FOUND
...
/**
 * @brief Compare two int32_t tensors for equality.
 *
 * Compares the populated data buffers of two int32_t tensors using
 ...
 *
 * @code{.c}
 * int32_tensor_t* a = init_int32_array(4, false, heap_allocator()).u.value;
 * int32_tensor_t* b = init_int32_array(4, false, heap_allocator()).u.value;
 *
 * push_back_int32_array(a, -1);
 * push_back_int32_array(a,  2);
 * push_back_int32_array(b, -1);
 * push_back_int32_array(b,  2);
 *
 * // Value equality — signed content matches
 * bool eq = int32_tensors_equal(a, b, false);   // true
 *
 * // -1 and 127 have different bit patterns — not equal
 * set_int32_tensor_index(b, 0, 127);
 * eq = int32_tensors_equal(a, b, false);         // false
 *
...
/**
 * @brief Find the minimum element in an int32 tensor.
 *
 ...
 * for the type (INT32_MIN, -2147483648) is encountered.
 *
 * Note: SSE2 and SSSE3 lack _mm_min_epi32, so those paths emulate
 * signed min via _mm_cmplt_epi32 (which is natively signed) and a
 * bitwise blend — no bias trick is needed.  SSE4.1 and later use
 * native _mm_min_epi32 / _mm256_min_epi32 / _mm512_min_epi32.
 *
 * Works on both TENSOR_STRUCT and ARRAY_STRUCT modes — the scan always
 * covers exactly t->base->len elements.
 *
 * @param t      Pointer to the source int32 tensor. Must not be NULL.
 * @param value  Pointer to an int32_t to receive the minimum value.
 *               Must not be NULL.
 *
 * @return NO_ERROR on success, or one of:
 ...
 * @code{.c}
 * int32_t min_val;
 * error_code_t err = min_int32_tensor(arr, &min_val);
 * if (err != NO_ERROR) {
 *     fprintf(stderr, "%s\n", error_to_string(err));
 * }
 ...
 *
 * @code{.c}
 * push_back_int32_tensor(arr, 0);   // arr = [0]
 * set_int32_tensor_index(arr, 0u, 99);   // arr = [99]
 * @endcode
 */
static inline error_code_t set_int32_tensor_index(int32_tensor_t* t,
                                                  size_t index,
                                                  int32_t data) {
    if (t == NULL) return NULL_POINTER;
    return set_tensor_index(t->base, index, &data, INT32_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 *
 * @code{.c}
 * // For a 3x3 int32 matrix initialised via init_tensor
 * size_t idx[] = { 1u, 2u };
 * set_int32_tensor_nd_index(mat, idx, 42u);   // mat[1][2] = 42
 * @endcode
 */
static inline error_code_t set_int32_tensor_nd_index(int32_tensor_t* t,
                                                     const size_t* idx,
                                                     int32_t data) {
    if (t == NULL) return NULL_POINTER;
    return set_tensor_nd_index(t->base, idx, &data, INT32_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 * @code{.c}
 * push_back_int32_tensor(arr, 10);
 * push_back_int32_tensor(arr, 20);
 * // arr = [10, 20], len == 2
 * @endcode
 */
static inline error_code_t push_back_int32_array(int32_tensor_t* t,
                                                 int32_t data) {
    if (t == NULL) return NULL_POINTER;
    return push_back_tensor(t->base, &data, INT32_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 * @code{.c}
 * push_back_int32_tensor(arr, 10);
 * push_front_int32_tensor(arr, 99);
 * // arr = [99, 10]
 * @endcode
 */
static inline error_code_t push_front_int32_array(int32_tensor_t* t,
                                                  int32_t data) {
    if (t == NULL) return NULL_POINTER;
    return push_front_tensor(t->base, &data, INT32_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 * @code{.c}
 * // arr = [10, 20, 30]
 * push_at_int32_tensor(arr, 99u, 1);
 * // arr = [10, 99, 20, 30]
 * @endcode
 */
static inline error_code_t push_at_int32_array(int32_tensor_t* t,
                                               int32_t data,
                                               size_t index) {
    if (t == NULL) return NULL_POINTER;
    return push_at_tensor(t->base, &data, index, INT32_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 * // arr = [10, 20, 30]
 * int32_t val;
 * pop_back_int32_tensor(arr, &val);   // val == 30, arr = [10, 20]
 * pop_back_int32_tensor(arr, NULL);   // discard, arr = [10]
 * @endcode
 */
static inline error_code_t pop_back_int32_array(int32_tensor_t* t,
                                                int32_t* out) {
    if (t == NULL) return NULL_POINTER;
    return pop_back_tensor(t->base, out, INT32_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 * // arr = [10, 20, 30]
 * int32_t val;
 * pop_front_int32_tensor(arr, &val);   // val == 10, arr = [20, 30]
 * @endcode
 */
static inline error_code_t pop_front_int32_array(int32_tensor_t* t,
                                                 int32_t* out) {
    if (t == NULL) return NULL_POINTER;
    return pop_front_tensor(t->base, out, INT32_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 * // arr = [10, 20, 30, 40]
 * int32_t val;
 * pop_at_int32_tensor(arr, &val, 2u);   // val == 30, arr = [10, 20, 40]
 * @endcode
 */
static inline error_code_t pop_at_int32_array(int32_tensor_t* t,
                                              int32_t* out,
                                              size_t index) {
    if (t == NULL) return NULL_POINTER;
    return pop_at_tensor(t->base, out, index, INT32_TYPE);
}
// ================================================================================ 
// ================================================================================ 
// RETRIEVE DATA
 ...
 * // arr = [10, 20, 30]
 * int32_t val;
 * get_int32_tensor_index(arr, 1u, &val);   // val == 20
 * @endcode
 */
static inline error_code_t get_int32_tensor_index(const int32_tensor_t* t,
                                                  size_t index,
                                                  int32_t* out) {
    if (t == NULL || out == NULL) return NULL_POINTER;
    return get_tensor_index(t->base, index, out, INT32_TYPE);
}
// -------------------------------------------------------------------------------- 
 ...
 *
 * @code{.c}
 * // For a 3x3 int32 matrix
 * size_t idx[] = { 1u, 2u };
 * int32_t val;
 * get_int32_tensor_nd_index(mat, idx, &val);   // val = mat[1][2]
 * @endcode
 */
static inline error_code_t get_int32_tensor_nd_index(const int32_tensor_t* t,
                                                     const size_t* idx,
                                                     int32_t* out) {
    if (t == NULL || idx == NULL || out == NULL) return NULL_POINTER;
    return get_tensor_nd_index(t->base, idx, out, INT32_TYPE);
}
// ================================================================================ 
// ================================================================================ 
#ifdef __cplusplus
