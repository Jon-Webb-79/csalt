...
/**
 * @brief Return the current logical length of an ::double_v.
 *
 ...
 *
 * @code{.c}
 * double_v* v = init_double_vector(8);
 * push_back_double_vector(v, 1.0);
 * push_back_double_vector(v, 2.0);
 * errno = 0;
 * size_t n = double_vector_size(v);     // n == 2, errno == 0
 *
 * errno = 0;
 * size_t bad = double_vector_size(NULL); // bad == SIZE_MAX, errno == EINVAL
 * free_double_vector(v);
 * @endcode
 */
static inline size_t double_vector_size(const double_v* vec) {
    if (!vec || !vec->data) {
        errno = EINVAL;
        return SIZE_MAX;
    }
...
/**
 * @brief Reverse the elements of an ::double_v in place.
 *
 * Reorders the vector so that the first element becomes the last and vice versa,
 * swapping pairs from the ends toward the center (uses ::swap_double). Works for
 ...
 *
 * @code{.c}
 * double_v* v = init_double_vector(5);
 * push_back_double_vector(v, 1.0);
 * push_back_double_vector(v, 2.0);
 * push_back_double_vector(v, 3.0);
 * push_back_double_vector(v, 4.0);
 * // v->data = [1.0,2.0,3.0,4.0], len = 4
 *
 * reverse_double_vector(v);
 * // v->data = [4.0,3.0,2.0,1.0], len = 4
 *
 * // Error cases
 * errno = 0;
 * reverse_double_vector(NULL);    // errno == EINVAL
 * double_v* w = init_double_vector(1);
 * // w->len == 0
 * reverse_double_vector(w);       // errno == ENODATA
 * free_double_vector(v);
 * free_double_vector(w);
 * @endcode
 */
...
/**
 * @brief Sort an ::double_v in place in ascending or descending order.
 *
 ...
 *
 * @param vec        Podoubleer to an ::double_v to sort. Must be non-NULL and have
 ...
 * // Ascending sort
 * double_v* v = init_double_vector(8);
 * push_back_double_vector(v, 5.0);
 * push_back_double_vector(v, 2.0);
 * push_back_double_vector(v, 9.0);
 * push_back_double_vector(v, 2.0);
 * sort_double_vector(v, FORWARD);  // v->data: [2.0, 2.0, 5.0, 9.0]
 *
 * // Descending sort
 * sort_double_vector(v, REVERSE);  // v->data: [9.0, 5.0, 2.0, 2.0]
 * free_double_vector(v);
 *
 * // Error case
 * errno = 0;
 * sort_double_vector(NULL, FORWARD); // errno == EINVAL
 * @endcode
 */
