...
/**
 * @brief Search a uint8_t tensor for the first occurrence of a value.
 *
 ...
 *
 * size_t idx = 0u;
 * error_code_t err = find_uint8_tensor_value(arr, &idx, 20u);
 * // err == NO_ERROR, idx == 1  (first occurrence)
 *
 * err = find_uint8_tensor_value(arr, &idx, 99u);
...
/**
 * @brief Binary search a sorted uint8_t tensor for a target value.
 *
 ...
 * data is unsorted.  Use sort_uint8_tensor before calling if the
 ...
 * data[*index] == value.  Use uint8_tensor_lsearch if the first
 ...
 * sort_uint8_tensor(arr, FORWARD);
 * // arr = [10, 20, 30, 40]
 *
 * size_t idx = 0u;
 * error_code_t err = uint8_tensor_bsearch(arr, &idx, 30u);
 * // err == NO_ERROR, idx == 2
 *
 * err = uint8_tensor_bsearch(arr, &idx, 25u);
 * // err == NOT_FOUND, idx unchanged
 *
 * return_uint8_tensor(arr);
 * @endcode
...
/**
 * @brief Bracketed binary search of a sorted uint8_t tensor.
 *
 * Locates the pair of adjacent indices that bracket value in a sorted
 * ascending tensor.  Unlike uint8_tensor_bsearch which returns
 ...
 *
 * // Bracketed
 * r = uint8_tensor_bbsearch(arr, 25u);
...
/**
 * @brief Compare two uint8_t tensors for equality.
 *
 * Compares the populated data buffers of two uint8_t tensors using
 ...
 *
 * @code{.c}
 * uint8_tensor_t* a = init_uint8_array(8, false, heap_allocator()).u.value;
 * uint8_tensor_t* b = init_uint8_array(4, false, heap_allocator()).u.value;
 *
 * push_back_uint8_array(a, 1u);
 * push_back_uint8_array(a, 2u);
 * push_back_uint8_array(b, 1u);
 * push_back_uint8_array(b, 2u);
 *
 * // Value equality — content matches, capacity difference ignored
 * bool eq = uint8_tensors_equal(a, b, false);   // true
 *
 * // Structural equality — capacity differs (8 vs 4)
 * eq = uint8_tensors_equal(a, b, true);          // false
 *
 * return_uint8_tensor(a);
 * return_uint8_tensor(b);
 * @endcode
 */
bool uint8_tensors_equal(const uint8_tensor_t* one,
                         const uint8_tensor_t* two,
                         bool                  meta);
// -------------------------------------------------------------------------------- 

/**
 * @brief Find the minimum element in a uint8 tensor.
 *
 ...
 *
 * @param t      Pointer to the source uint8 tensor. Must not be NULL.
 * @param value  Pointer to a uint8_t to receive the minimum value.
 *               Must not be NULL.
 *
 * @return NO_ERROR on success, or one of:
 ...
 * @code{.c}
 * uint8_t min_val;
 * error_code_t err = min_uint8_tensor(arr, &min_val);
 * if (err != NO_ERROR) {
 *     fprintf(stderr, "%s\n", error_to_string(err));
 * }
 * printf("Minimum: %" PRIu8 "\n", min_val);
 * @endcode
 */
