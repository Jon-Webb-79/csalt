...
/**
 * @brief Compare two uint32_t tensors for equality.
 *
 * Compares the populated data buffers of two uint32_t tensors using
 ...
 *
 * @code{.c}
 * uint32_tensor_t* a = init_uint32_array(8, false, heap_allocator()).u.value;
 * uint32_tensor_t* b = init_uint32_array(4, false, heap_allocator()).u.value;
 *
 * push_back_uint32_array(a, 1u);
 * push_back_uint32_array(a, 2u);
 * push_back_uint32_array(b, 1u);
 * push_back_uint32_array(b, 2u);
 *
 * // Value equality — content matches, capacity difference ignored
 * bool eq = uint32_tensors_equal(a, b, false);   // true
 *
 * // Structural equality — capacity differs (8 vs 4)
 * eq = uint32_tensors_equal(a, b, true);          // false
 *
