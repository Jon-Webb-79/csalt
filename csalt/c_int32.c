...
bool int32_array_equal(const int32_array_t* a,
    ...
    return simd_int32_arrays_equal(
        (const int32_t*)a->base.data,
...
static error_code_t _print_int32_dense_matrix(const int32_matrix_t* mat,
                                              FILE*                 stream) {
    ...
    for (size_t i = 0u; i < mat->rows; ++i) {
        ...
        for (size_t j = 0u; j < mat->cols; ++j) {
            ...
            if (fprintf(stream, "%" PRId32, data[idx]) < 0) {
...
static error_code_t _print_int32_sparse_row_major(const int32_matrix_t* mat,
                                                  FILE*                 stream) {
    if (mat == NULL || stream == NULL) return NULL_POINTER;
    ...
    for (size_t i = 0u; i < mat->rows; ++i) {
        for (size_t j = 0u; j < mat->cols; ++j) {
            ...
            error_code_t err = get_int32_matrix(mat, i, j, &value);
            if (err != NO_ERROR) return err;
...
static error_code_t _print_int32_coo_matrix(const int32_matrix_t* mat,
                                            FILE*                 stream) {
    if (mat == NULL || stream == NULL) return NULL_POINTER;
    if (mat->format != COO_MATRIX)     return ILLEGAL_STATE;
    return _print_int32_sparse_row_major(mat, stream);
}

// -----------------------------------------------------------------------------

static error_code_t _print_int32_csr_matrix(const int32_matrix_t* mat,
                                            FILE*                 stream) {
    if (mat == NULL || stream == NULL) return NULL_POINTER;
...
static error_code_t _print_int32_csc_matrix(const int32_matrix_t* mat,
                                            FILE*                 stream) {
    if (mat == NULL || stream == NULL) return NULL_POINTER;
...
error_code_t print_int32_matrix(const int32_matrix_t* mat, FILE* stream) {
    ...
    switch (mat->format) {
        case DENSE_MATRIX:
            return _print_int32_dense_matrix(mat, stream);

        case COO_MATRIX:
            return _print_int32_coo_matrix(mat, stream);

        case CSR_MATRIX:
            return _print_int32_csr_matrix(mat, stream);

        case CSC_MATRIX:
            return _print_int32_csc_matrix(mat, stream);
...
int32_expect_t int32_matrix_min(const int32_matrix_t* mat) {
    ...
    switch (mat->format) {
        case DENSE_MATRIX: {
            const int32_t* vals = (const int32_t*)mat->rep.dense.data;
            int32_t val = vals[idx.u.value];
            return (int32_expect_t){ .has_value = true, .u.value = val };
        }

        case COO_MATRIX: {
            const int32_t* vals = (const int32_t*)mat->rep.coo.values;
            int32_t val = vals[idx.u.value];
            return (int32_expect_t){ .has_value = true, .u.value = val };
        }

        case CSR_MATRIX: {
            const int32_t* vals = (const int32_t*)mat->rep.csr.values;
            int32_t val = vals[idx.u.value];
            return (int32_expect_t){ .has_value = true, .u.value = val };
        }

        case CSC_MATRIX: {
            const int32_t* vals = (const int32_t*)mat->rep.csc.values;
            int32_t val = vals[idx.u.value];
            return (int32_expect_t){ .has_value = true, .u.value = val };
        }
...
int32_expect_t int32_matrix_max(const int32_matrix_t* mat) {
    ...
    size_expect_t idx = matrix_max(mat, _cmp_int32, INT32_TYPE);
    if (!idx.has_value) {
        return (int32_expect_t){ .has_value = false, .u.error = idx.u.error };
    }
